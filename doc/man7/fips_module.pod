...
OPENSSL_BOX_EVP_MD_meth_new(), OPENSSL_BOX_EVP_CIPHER_meth_new(), OPENSSL_BOX_EVP_PKEY_meth_new(), RSA_meth_new(),
...
standards you can specify the property query C<fips=yes> like this:
    ...
    sha256 = OPENSSL_BOX_EVP_MD_fetch(NULL, "SHA2-256", "fips=yes");
...
default provider:
    ...
    sha256 = OPENSSL_BOX_EVP_MD_fetch(NULL, "SHA2-256", "provider=default");
...
base providers. The other library context will just use the default provider.
     ...
     */
    if (!OPENSSL_BOX_EVP_set_default_properties(fips_libctx, "fips=yes"))
        goto err;
    ...
    fipssha256 = OPENSSL_BOX_EVP_MD_fetch(fips_libctx, "SHA2-256", NULL);
    if (fipssha256 == NULL)
        goto err;

    /* Get a non-FIPS validated digest */
    nonfipssha256 = OPENSSL_BOX_EVP_MD_fetch(nonfips_libctx, "SHA2-256", NULL);
    ...
    err:
    OPENSSL_BOX_EVP_MD_free(fipssha256);
    OPENSSL_BOX_EVP_MD_free(nonfipssha256);
...
OpenSSL 1.1.1 or earlier such as L<OPENSSL_BOX_i2d_PrivateKey(3)>. However the appropriate
...
To go from an B<EVP_MD_CTX> to an B<EVP_MD>, use L<OPENSSL_BOX_EVP_MD_CTX_md(3)> .
To go from the B<EVP_MD> to its B<OSSL_PROVIDER>,
use L<OPENSSL_BOX_EVP_MD_get0_provider(3)>.
...
See L<OPENSSL_BOX_EVP_EncryptInit(3)/PARAMETERS> for further information.
