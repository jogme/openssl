...
used with OPENSSL_BOX_EVP_MAC_fetch():
...
The following parameter can be set with OPENSSL_BOX_EVP_MAC_CTX_set_params():
...
If required this parameter should be set before OPENSSL_BOX_EVP_MAC_init()
...
OPENSSL_BOX_EVP_MAC_CTX_get_params():

=over 4

=item "size" (B<OSSL_MAC_PARAM_SIZE>) <unsigned integer>

The "size" parameter can also be retrieved with with OPENSSL_BOX_EVP_MAC_CTX_get_mac_size().
...
OPENSSL_BOX_EVP_MAC_CTX_get_block_size().
...
This may be used after calling OPENSSL_BOX_EVP_MAC_final().
