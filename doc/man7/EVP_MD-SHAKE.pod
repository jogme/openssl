...
This parameter must be set before calling either OPENSSL_BOX_EVP_DigestFinal_ex() or
OPENSSL_BOX_EVP_DigestFinal(), since these functions were not designed to handle variable
length output. It is recommended to either use OPENSSL_BOX_EVP_DigestSqueeze() or
OPENSSL_BOX_EVP_DigestFinalXOF() instead.
...
See L<OPENSSL_BOX_EVP_DigestInit(3)/PARAMETERS> for further information related to parameters

=head1 NOTES

For SHAKE-128, to ensure the maximum security strength of 128 bits, the output
length passed to OPENSSL_BOX_EVP_DigestFinalXOF() should be at least 32.

For SHAKE-256, to ensure the maximum security strength of 256 bits, the output
length passed to OPENSSL_BOX_EVP_DigestFinalXOF() should be at least 64.

=head1 SEE ALSO

L<OPENSSL_BOX_EVP_MD_CTX_set_params(3)>, L<provider-digest(7)>, L<OSSL_PROVIDER-default(7)>
