...
OPENSSL_BOX_EVP_PKEY_verify_message_init() and OPENSSL_BOX_EVP_PKEY_verify() are the only supported
...
=head2 LMS signature verification
 ...
 ctx = OPENSSL_BOX_EVP_PKEY_CTX_new_from_pkey(libctx, pub, propq);
 sig = OPENSSL_BOX_EVP_SIGNATURE_fetch(libctx, "LMS", propq);
 OPENSSL_BOX_EVP_PKEY_verify_message_init(ctx, sig, NULL);
 /*
  * OPENSSL_BOX_EVP_PKEY_verify_message_update() and OPENSSL_BOX_EVP_PKEY_verify_message_final()
  * are not supported
  */
 ret = OPENSSL_BOX_EVP_PKEY_verify(ctx, sigdata, sigdata_len, msg, msglen);
 /*
