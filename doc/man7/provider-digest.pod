...
them available to applications via the API functions L<OPENSSL_BOX_EVP_DigestInit_ex(3)>,
L<OPENSSL_BOX_EVP_DigestUpdate(3)> and L<OPENSSL_BOX_EVP_DigestFinal(3)> (and other related functions).
...
context.
The OSSL_FUNC_digest_copyctx function is used in the OPENSSL_BOX_EVP_MD_CTX_copy_ex function to
speed up HMAC operations in the PBKDF2.
This function is optional, and dupctx will be used if there is no OPENSSL_BOX_EVP_MD_CTX_copy_ex
function.
...
The OPENSSL_BOX_EVP_Q_digest(), OPENSSL_BOX_EVP_Digest() and OPENSSL_BOX_EVP_DigestFinal_ex() API calls do not
...
L<EVP_MD-NULL(7)>,
L<life_cycle-digest(7)>, L<OPENSSL_BOX_EVP_DigestInit(3)>
