...
Use OPENSSL_BOX_EVP_PKEY_CTX_set_params() after calling OPENSSL_BOX_EVP_PKEY_keygen_init().
...
The following parameters are gettable using OPENSSL_BOX_EVP_PKEY_get_octet_string_param(),
and settable when using OPENSSL_BOX_EVP_PKEY_fromdata().
...
An B<EVP_PKEY> context can be obtained by calling:
    ...
    EVP_PKEY_CTX *pctx =
        OPENSSL_BOX_EVP_PKEY_CTX_new_from_name(NULL, "SLH-DSA-SHA2-128f", NULL);

An B<SLH-DSA> key can be generated like this:

    pkey = OPENSSL_BOX_EVP_PKEY_Q_keygen(NULL, NULL, "SLH-DSA-SHA2-128f");
