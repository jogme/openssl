...
This value must always be set when creating a raw key using L<OPENSSL_BOX_EVP_PKEY_fromdata(3)>.
...
For RSA keys, L<OPENSSL_BOX_EVP_PKEY_param_check(3)> and L<OPENSSL_BOX_EVP_PKEY_param_check_quick(3)>
both return 1 unconditionally.

For RSA keys, L<OPENSSL_BOX_EVP_PKEY_public_check(3)> conforms to the SP800-56Br1 I<public key
...
For RSA keys, L<OPENSSL_BOX_EVP_PKEY_public_check_quick(3)> is the same as
L<OPENSSL_BOX_EVP_PKEY_public_check(3)>.

For RSA keys, L<OPENSSL_BOX_EVP_PKEY_private_check(3)> conforms to the SP800-56Br1
I<private key test>.

For RSA keys, L<OPENSSL_BOX_EVP_PKEY_pairwise_check(3)> conforms to the
...
An B<EVP_PKEY> context can be obtained by calling:
    ...
    EVP_PKEY_CTX *pctx =
        OPENSSL_BOX_EVP_PKEY_CTX_new_from_name(NULL, "RSA", NULL);
...
An B<RSA> key can be generated with key generation parameters:
    ...
    OSSL_PARAM params[3];
    EVP_PKEY *pkey = NULL;
    EVP_PKEY_CTX *pctx = OPENSSL_BOX_EVP_PKEY_CTX_new_from_name(NULL, "RSA", NULL);
    ...
    OPENSSL_BOX_EVP_PKEY_generate(pctx, &pkey);
    OPENSSL_BOX_EVP_PKEY_print_private(bio_out, pkey, 0, NULL);
    OPENSSL_BOX_EVP_PKEY_CTX_free(pctx);
