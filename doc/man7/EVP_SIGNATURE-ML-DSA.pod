...
L<OPENSSL_BOX_EVP_SIGNATURE_fetch(3)> can be used to explicitely fetch one of the 3
algorithms which can then be used with L<OPENSSL_BOX_EVP_PKEY_sign_message_init(3)>,
L<OPENSSL_BOX_EVP_PKEY_sign(3)>, L<OPENSSL_BOX_EVP_PKEY_verify_message_init(3)>, and
L<OPENSSL_BOX_EVP_PKEY_verify(3)> to perform one-shot message signing or signature verification.
...
it may be set by passing an OSSL_PARAM array to L<OPENSSL_BOX_EVP_PKEY_sign_message_init(3)>
or L<OPENSSL_BOX_EVP_PKEY_verify_message_init(3)>
...
They can be set by passing an OSSL_PARAM array to L<OPENSSL_BOX_EVP_PKEY_sign_init_ex2(3)>.
...
For backwards compatability reasons OPENSSL_BOX_EVP_DigestSignInit_ex(), OPENSSL_BOX_EVP_DigestSign(),
...
To sign a message using an ML-DSA EVP_PKEY structure:
    ...
    void do_sign(EVP_PKEY *key, unsigned char *msg, size_t msg_len)
    {
        ...
        EVP_PKEY_CTX *sctx = OPENSSL_BOX_EVP_PKEY_CTX_new_from_pkey(NULL, pkey, NULL);
        EVP_SIGNATURE *sig_alg = OPENSSL_BOX_EVP_SIGNATURE_fetch(NULL, "ML-DSA-65", NULL);

        OPENSSL_BOX_EVP_PKEY_sign_message_init(sctx, sig_alg, params);
        /* Calculate the required size for the signature by passing a NULL buffer. */
        OPENSSL_BOX_EVP_PKEY_sign(sctx, NULL, &sig_len, msg, msg_len);
        sig = OPENSSL_zalloc(sig_len);
        OPENSSL_BOX_EVP_PKEY_sign(sctx, sig, &sig_len, msg, msg_len);
        ...
        OPENSSL_free(sig);
