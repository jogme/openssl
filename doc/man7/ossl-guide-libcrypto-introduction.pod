...
the encryption functions detailed on the L<OPENSSL_BOX_EVP_EncryptInit(3)> page. Or to
perform a digest operation using SHA256 then you would use the digesting
functions on the L<OPENSSL_BOX_EVP_DigestInit(3)> page.
...
where C<APINAME> is the name of the operation. For example L<OPENSSL_BOX_EVP_MD_fetch(3)>
...
functions that use them. For example the L<OPENSSL_BOX_EVP_DigestInit_ex(3)> function takes
as a parameter an B<EVP_MD> object which may have been returned from an earlier
call to L<OPENSSL_BOX_EVP_MD_fetch(3)>.

=head2 Implicit fetching

OpenSSL has a number of functions that return an algorithm object with no
associated implementation, such as L<OPENSSL_BOX_EVP_sha256(3)>, L<OPENSSL_BOX_EVP_aes_128_cbc(3)>,
L<OPENSSL_BOX_EVP_get_cipherbyname(3)> or L<OPENSSL_BOX_EVP_get_digestbyname(3)>. These are present for
compatibility with OpenSSL before version 3.0 where explicit fetching was not
available.

When they are used with functions like L<OPENSSL_BOX_EVP_DigestInit_ex(3)> or
L<OPENSSL_BOX_EVP_CipherInit_ex(3)>, the actual implementation to be used is
...
L<OPENSSL_BOX_EVP_DigestSignInit(3)>, all fetch the implementations implicitly. Usually the
...
Prior to OpenSSL 3.0, functions such as OPENSSL_BOX_EVP_sha256() which return a "const"
...
=item L<OPENSSL_BOX_EVP_MD_fetch(3)>

Fetch a message digest/hashing algorithm implementation.

=item L<OPENSSL_BOX_EVP_CIPHER_fetch(3)>
...
=item L<OPENSSL_BOX_EVP_MAC_fetch(3)>

Fetch a Message Authentication Code (MAC) algorithm implementation.

=item L<OPENSSL_BOX_EVP_KEM_fetch(3)>
...
=item L<OPENSSL_BOX_EVP_RAND_fetch(3)>
...
that some algorithms have aliases. So "SHA256" and "SHA2-256" are synonymous:
 ...
 EVP_MD *md = OPENSSL_BOX_EVP_MD_fetch(NULL, "SHA2-256", NULL);
 ...
 OPENSSL_BOX_EVP_MD_free(md);

Fetch any available implementation of AES-128-CBC in the default context:

 EVP_CIPHER *cipher = OPENSSL_BOX_EVP_CIPHER_fetch(NULL, "AES-128-CBC", NULL);
 ...
...
context:
 ...
 EVP_MD *md = OPENSSL_BOX_EVP_MD_fetch(NULL, "SHA2-256", "provider=default");
 ...
 OPENSSL_BOX_EVP_MD_free(md);
...
the default context:
 ...
 EVP_MD *md = OPENSSL_BOX_EVP_MD_fetch(NULL, "SHA2-256", "provider=?fips");
 ...
 OPENSSL_BOX_EVP_MD_free(md);
...
library context:
 ...
 EVP_MD *md = OPENSSL_BOX_EVP_MD_fetch(libctx, "SHA2-256", "provider=default");
 ...
 OPENSSL_BOX_EVP_MD_free(md);
...
implementation of WHIRLPOOL from it:
 ...
 EVP_MD *md = OPENSSL_BOX_EVP_MD_fetch(NULL, "WHIRLPOOL", "provider=legacy");
 ...
 OPENSSL_BOX_EVP_MD_free(md);
...
other providers:
 ...
 EVP_MD *md_whirlpool = OPENSSL_BOX_EVP_MD_fetch(NULL, "whirlpool", NULL);
 EVP_MD *md_sha256 = OPENSSL_BOX_EVP_MD_fetch(NULL, "SHA2-256", NULL);
 ...
 OPENSSL_BOX_EVP_MD_free(md_whirlpool);
 OPENSSL_BOX_EVP_MD_free(md_sha256);
...
L<ossl-guide-libraries-introduction(7)/DEMO APPLICATIONS>).
 ...
 int main(void)
 {
      ...
      * be used for multiple calls to other operations such as OPENSSL_BOX_EVP_DigestInit_ex().
      */
     sha256 = OPENSSL_BOX_EVP_MD_fetch(NULL, "SHA256", NULL);
     if (sha256 == NULL)
         goto err;

    /* Initialise the digest operation */
    if (!OPENSSL_BOX_EVP_DigestInit_ex(ctx, sha256, NULL))
        goto err;

     /*
      * Pass the message to be digested. This can be passed in over multiple
      * OPENSSL_BOX_EVP_DigestUpdate calls if necessary
      */
     if (!OPENSSL_BOX_EVP_DigestUpdate(ctx, msg, sizeof(msg)))
         goto err;

     /* Allocate the output buffer */
     outdigest = OPENSSL_malloc(OPENSSL_BOX_EVP_MD_get_size(sha256));
     if (outdigest == NULL)
         goto err;

     /* Now calculate the digest itself */
     if (!OPENSSL_BOX_EVP_DigestFinal_ex(ctx, outdigest, &len))
         goto err;
...
using the EVP APIs (for example see L<OPENSSL_BOX_EVP_PKEY_Q_keygen(3)>). However it is often
