...
L<OPENSSL_BOX_EVP_DigestSignInit(3)> and L<OPENSSL_BOX_EVP_DigestVerifyInit(3)>, see
L</ED25519 and ED448 Signature Parameters> below.

These instances are also explicitly fetchable as algorithms using
L<OPENSSL_BOX_EVP_SIGNATURE_fetch(3)>, which can be used with
L<OPENSSL_BOX_EVP_PKEY_sign_init_ex2(3)>, L<OPENSSL_BOX_EVP_PKEY_verify_init_ex2(3)>,
L<OPENSSL_BOX_EVP_PKEY_sign_message_init(3)> and L<OPENSSL_BOX_EVP_PKEY_verify_message_init(3)>.
...
passing an OSSL_PARAM array to OPENSSL_BOX_EVP_DigestSignInit_ex().
...
When using L<OPENSSL_BOX_EVP_DigestSignInit(3)> or L<OPENSSL_BOX_EVP_DigestVerifyInit(3)>, the
...
When using L<OPENSSL_BOX_EVP_PKEY_sign_init_ex2(3)>, L<OPENSSL_BOX_EVP_PKEY_verify_init_ex2(3)>,
L<OPENSSL_BOX_EVP_PKEY_sign_message_init(3)> or L<OPENSSL_BOX_EVP_PKEY_verify_message_init(3)>, the
...
other signature algorithms using, for example, OPENSSL_BOX_EVP_DigestUpdate().
The message to sign or verify must be passed using the one-shot
OPENSSL_BOX_EVP_DigestSign() and OPENSSL_BOX_EVP_DigestVerify() functions.
...
When calling OPENSSL_BOX_EVP_DigestSignInit() or OPENSSL_BOX_EVP_DigestVerifyInit(), the
...
L<OPENSSL_BOX_EVP_PKEY_new_raw_private_key(3)> or loaded from a PKCS#8 private key file
...
L<OPENSSL_BOX_EVP_PKEY_new_raw_public_key(3)> or loaded from a SubjectPublicKeyInfo
...
To sign a message using an ED25519 EVP_PKEY structure:
    ...
    void do_sign(EVP_PKEY *ed_key, unsigned char *msg, size_t msg_len)
    {
        ...
        OPENSSL_BOX_EVP_DigestSignInit_ex(md_ctx, NULL, NULL, NULL, NULL, ed_key, params);
        /* Calculate the required size for the signature by passing a NULL buffer. */
        OPENSSL_BOX_EVP_DigestSign(md_ctx, NULL, &sig_len, msg, msg_len);
        sig = OPENSSL_zalloc(sig_len);

        OPENSSL_BOX_EVP_DigestSign(md_ctx, sig, &sig_len, msg, msg_len);
        ...
        OPENSSL_free(sig);
