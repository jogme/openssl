...
See EVP_PKEY_set1_encoded_public_key() and OPENSSL_BOX_EVP_PKEY_get1_encoded_public_key().
...
With the OpenSSL FIPS provider, L<OPENSSL_BOX_EVP_PKEY_param_check(3)> and
L<OPENSSL_BOX_EVP_PKEY_param_check_quick(3)> behave in the following way: the parameters
...
value for backwards compatibility, however the L<OPENSSL_BOX_EVP_PKEY_param_check(3)> will
test the I<p> value for being a prime (and a safe prime if I<q> is missing)
which can take significant time. The L<OPENSSL_BOX_EVP_PKEY_param_check_quick(3)> avoids
the prime tests.

L<OPENSSL_BOX_EVP_PKEY_public_check(3)> conforms to SP800-56Ar3
I<FFC Full Public-Key Validation>.

L<OPENSSL_BOX_EVP_PKEY_public_check_quick(3)> conforms to SP800-56Ar3
I<FFC Partial Public-Key Validation> when the key is an approved named safe
prime group, otherwise it is the same as L<OPENSSL_BOX_EVP_PKEY_public_check(3)>.

L<OPENSSL_BOX_EVP_PKEY_private_check(3)> tests that the private key is in the correct range
...
L<OPENSSL_BOX_EVP_PKEY_pairwise_check(3)> conforms to SP800-56Ar3
...
An B<EVP_PKEY> context can be obtained by calling:
    ...
    EVP_PKEY_CTX *pctx = OPENSSL_BOX_EVP_PKEY_CTX_new_from_name(NULL, "DH", NULL);
...
A B<DH> key can be generated with a named safe prime group by calling:
    ...
    OPENSSL_BOX_EVP_PKEY_keygen_init(pctx);
    OPENSSL_BOX_EVP_PKEY_CTX_set_params(pctx, params);
    OPENSSL_BOX_EVP_PKEY_generate(pctx, &pkey);
    ...
...
B<DHX> domain parameters can be generated according to B<FIPS186-4> by calling:
    ...
    pctx = OPENSSL_BOX_EVP_PKEY_CTX_new_from_name(NULL, "DHX", NULL);
    OPENSSL_BOX_EVP_PKEY_paramgen_init(pctx);
    ...
    OPENSSL_BOX_EVP_PKEY_generate(pctx, &param_key);

    OPENSSL_BOX_EVP_PKEY_print_params(bio_out, param_key, 0, NULL);
    ...
...
A B<DH> key can be generated using domain parameters by calling:
    ...
    EVP_PKEY *key = NULL;
    EVP_PKEY_CTX *gctx = OPENSSL_BOX_EVP_PKEY_CTX_new_from_pkey(NULL, param_key, NULL);

    OPENSSL_BOX_EVP_PKEY_keygen_init(gctx);
    OPENSSL_BOX_EVP_PKEY_generate(gctx, &key);
    OPENSSL_BOX_EVP_PKEY_print_private(bio_out, key, 0, NULL);
    ...
...
OPENSSL_BOX_EVP_PKEY_todata(), OSSL_PARAM_merge(), and OPENSSL_BOX_EVP_PKEY_fromdata() are useful
...
the actual validation. In production code the return values should be checked.
    ...
    OPENSSL_BOX_EVP_PKEY_todata(received_domp, OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS,
                    &domain_params);
    ...
    ctx = OPENSSL_BOX_EVP_PKEY_CTX_new_from_name(NULL, "DHX", NULL);
    OPENSSL_BOX_EVP_PKEY_fromdata_init(ctx);
    OPENSSL_BOX_EVP_PKEY_fromdata(ctx, &complete_domp, OSSL_KEYMGMT_SELECT_ALL,
                      merged_params);

    validate_ctx = OPENSSL_BOX_EVP_PKEY_CTX_new_from_pkey(NULL, complete_domp, NULL);
    if (OPENSSL_BOX_EVP_PKEY_param_check(validate_ctx) > 0)
        /* validation_passed(); */
    else
        /* validation_failed(); */
    ...
    OPENSSL_BOX_EVP_PKEY_CTX_free(validate_ctx);
    OPENSSL_BOX_EVP_PKEY_free(complete_domp);
