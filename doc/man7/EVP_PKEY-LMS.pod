...
The following parameters are used by OPENSSL_BOX_EVP_PKEY_fromdata() and by the
...
An B<EVP_PKEY> context can be obtained by calling:
    ...
    EVP_PKEY_CTX *ctx = OPENSSL_BOX_EVP_PKEY_CTX_new_from_name(libctx, "LMS", propq);
...
To load a LMS key from XDR encoded "data" of size "datalen":
    ...
    ret = OPENSSL_BOX_EVP_PKEY_fromdata_init(ctx)
    ret = OPENSSL_BOX_EVP_PKEY_fromdata(ctx, &key, EVP_PKEY_PUBLIC_KEY, params);
