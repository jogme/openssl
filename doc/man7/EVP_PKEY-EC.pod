...
Sets or Gets the type of group check done when OPENSSL_BOX_EVP_PKEY_param_check() is called.
...
OPENSSL_BOX_EVP_PKEY_fromdata() and OPENSSL_BOX_EVP_PKEY_todata() functions.

Note, in particular, that the choice of point compression format used for
encoding the exported value via OPENSSL_BOX_EVP_PKEY_todata() depends on the underlying
...
For EC keys, L<OPENSSL_BOX_EVP_PKEY_param_check(3)> behaves in the following way:
...
For EC keys, L<OPENSSL_BOX_EVP_PKEY_param_check_quick(3)> is equivalent to
L<OPENSSL_BOX_EVP_PKEY_param_check(3)>.

For EC keys, L<OPENSSL_BOX_EVP_PKEY_public_check(3)> and L<OPENSSL_BOX_EVP_PKEY_public_check_quick(3)>
conform to SP800-56Ar3 I<ECC Full Public-Key Validation> and
I<ECC Partial Public-Key Validation> respectively.

For EC Keys, L<OPENSSL_BOX_EVP_PKEY_private_check(3)> and L<OPENSSL_BOX_EVP_PKEY_pairwise_check(3)>
...
An B<EVP_PKEY> context can be obtained by calling:
    ...
    EVP_PKEY_CTX *pctx =
        OPENSSL_BOX_EVP_PKEY_CTX_new_from_name(NULL, "EC", NULL);
...
or like this:
    ...
    OPENSSL_BOX_EVP_PKEY_CTX_set_params(gctx, params);
