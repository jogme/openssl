...
"BLAKE2B-512" supports the following OPENSSL_BOX_EVP_MD_CTX_set_params() key
described in L<OPENSSL_BOX_EVP_DigestInit(3)/PARAMETERS>.
...
are settable for an B<EVP_MD_CTX> with L<OPENSSL_BOX_EVP_DigestInit_ex2(3)> or
L<OPENSSL_BOX_EVP_MD_CTX_set_params(3)>:

=over 4

=item "size" (B<OSSL_DIGEST_PARAM_SIZE>) <unsigned integer>

Sets a different digest length for the L<OPENSSL_BOX_EVP_DigestFinal(3)> output.
...
L<OPENSSL_BOX_EVP_DigestInit_ex2(3)> call to have an immediate effect. When set with
L<OPENSSL_BOX_EVP_MD_CTX_set_params(3)> it will have an effect only if the B<EVP_MD_CTX>
