...
properties, to be used with OPENSSL_BOX_EVP_MAC_fetch():
...
OPENSSL_BOX_EVP_MAC_CTX_set_params().
Furthermore, the "size" parameter can be retrieved with
OPENSSL_BOX_EVP_MAC_CTX_get_params(), or with OPENSSL_BOX_EVP_MAC_CTX_get_mac_size().
The length of the "size" parameter should not exceed that of a B<size_t>.
Likewise, the "block-size" parameter can be retrieved with
OPENSSL_BOX_EVP_MAC_CTX_get_params(), or with OPENSSL_BOX_EVP_MAC_CTX_get_block_size().
...
Sets the MAC key.
Setting this parameter is identical to passing a I<key> to L<OPENSSL_BOX_EVP_MAC_init(3)>.
...
the OPENSSL_BOX_EVP_MAC_init() call.
The "xof" and "size" parameters can be set at any time before OPENSSL_BOX_EVP_MAC_final().
The "key" parameter is set as part of the OPENSSL_BOX_EVP_MAC_init() call, but can be
...
=head1 EXAMPLES
                     ...
                     int xof_enabled, unsigned char *out, int out_len)
  {
      ...
      mac = OPENSSL_BOX_EVP_MAC_fetch(NULL, "KMAC-128", NULL);
      if (mac == NULL)
          goto err;
      ctx = OPENSSL_BOX_EVP_MAC_CTX_new(mac);
      /* The mac can be freed after it is used by OPENSSL_BOX_EVP_MAC_CTX_new */
      OPENSSL_BOX_EVP_MAC_free(mac);
      if (ctx == NULL)
          goto err;

      /*
       * Setup parameters required before calling OPENSSL_BOX_EVP_MAC_init()
      ...
      *p = OSSL_PARAM_construct_end();
      ...
      if (!OPENSSL_BOX_EVP_MAC_init(ctx))
          goto err;

      /*
       * Note: the following optional parameters can be set any time
       * before OPENSSL_BOX_EVP_MAC_final().
       */
      p = params;
      ...
      *p = OSSL_PARAM_construct_end();
      ...
      if (!OPENSSL_BOX_EVP_MAC_update(ctx, in, in_len))
          goto err;
      if (!OPENSSL_BOX_EVP_MAC_final(ctx, out, &l, out_len))
          goto err;
      ret = 1;
  err:
...
L<OPENSSL_BOX_EVP_MAC_CTX_get_params(3)>, L<OPENSSL_BOX_EVP_MAC_CTX_set_params(3)>,
