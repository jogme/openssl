...
Use L<OPENSSL_BOX_EVP_PKEY_CTX_set_params(3)> after calling L<OPENSSL_BOX_EVP_PKEY_keygen_init(3)>.
...
L<OPENSSL_BOX_EVP_PKEY_get_octet_string_param(3)> or L<OPENSSL_BOX_EVP_PKEY_get_params(3)>.
They can be initialised via L<OPENSSL_BOX_EVP_PKEY_fromdata(3)>, and are returned by
L<OPENSSL_BOX_EVP_PKEY_todata(3)> given a suitable I<selection>.
...
An B<EVP_PKEY> context can be obtained by calling:
    ...
    EVP_PKEY_CTX *pctx =
        OPENSSL_BOX_EVP_PKEY_CTX_new_from_name(NULL, "ML-DSA-44", NULL);

An B<ML-DSA-44> key can be generated like this:

    pkey = OPENSSL_BOX_EVP_PKEY_Q_keygen(NULL, NULL, "ML-DSA-44");
...
The key pair components can be extracted from a key by calling:
    ...
    OPENSSL_BOX_EVP_PKEY_get_octet_string_param(pkey, OSSL_PKEY_PARAM_ML_DSA_SEED,
                                    seed, sizeof(seed), &seed_len);
    OPENSSL_BOX_EVP_PKEY_get_octet_string_param(pkey, OSSL_PKEY_PARAM_PRIV_KEY,
                                    priv, sizeof(priv), &priv_len);
    OPENSSL_BOX_EVP_PKEY_get_octet_string_param(pkey, OSSL_PKEY_PARAM_PUB_KEY,
...
L<OPENSSL_BOX_EVP_PKEY_get_raw_private_key(3)>,
L<OPENSSL_BOX_EVP_PKEY_get_raw_public_key(3)>,
