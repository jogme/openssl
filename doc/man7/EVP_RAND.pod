...
a pointer to the parent EVP_RAND_CTX as argument to the OPENSSL_BOX_EVP_RAND_CTX_new() call.
...
OPENSSL_BOX_EVP_RAND_generate(<public>, ...) and
OPENSSL_BOX_EVP_RAND_generate(<private>, ...),
respectively.
...
Before satisfying a generate request (L<OPENSSL_BOX_EVP_RAND_generate(3)>), the DRBG
...
L<OPENSSL_BOX_EVP_RAND_generate(3)>.
