...
The functions EVP_PKEY_asn1_*() and L<OPENSSL_BOX_EVP_PKEY_get0_asn1(3)> were deprecated in
...
the application should verify the result of the L<OPENSSL_BOX_EVP_EncryptInit(3)>,
L<OPENSSL_BOX_EVP_EncryptInit_ex(3)>, and L<OPENSSL_BOX_EVP_DigestInit(3)> functions. In case when
...
For example, the EVP APIs provide the functions L<OPENSSL_BOX_EVP_EncryptInit_ex(3)>,
L<OPENSSL_BOX_EVP_EncryptUpdate(3)> and L<OPENSSL_BOX_EVP_EncryptFinal(3)> to perform symmetric
...
modifies custom "METHODS" (for example L<OPENSSL_BOX_EVP_MD_meth_new(3)>,
L<OPENSSL_BOX_EVP_CIPHER_meth_new(3)>, L<OPENSSL_BOX_EVP_PKEY_meth_new(3)>, L<RSA_meth_new(3)>,
...
B<EVP_PKEY>s L<OPENSSL_BOX_EVP_PKEY_set1_RSA(3)>, L<OPENSSL_BOX_EVP_PKEY_set1_EC_KEY(3)> or similar
...
L<OPENSSL_BOX_EVP_DigestSign(3)> and L<OPENSSL_BOX_EVP_DigestVerify(3)>.
...
Using calls to convenience functions such as OPENSSL_BOX_EVP_sha256() and OPENSSL_BOX_EVP_aes_256_gcm() may
...
See L<OPENSSL_BOX_EVP_EncryptInit(3)/SIV Mode>.
...
L<OPENSSL_BOX_PKCS5_v2_PBE_keyivgen_ex(3)>, L<OPENSSL_BOX_PKCS5_v2_scrypt_keyivgen_ex(3)>,
...
L<OPENSSL_BOX_EVP_PBE_CipherInit_ex(3)>, L<OPENSSL_BOX_EVP_PBE_find_ex(3)> and L<OPENSSL_BOX_EVP_PBE_scrypt_ex(3)>.
...
L<OPENSSL_BOX_EVP_PKEY_public_check(3)> and L<OPENSSL_BOX_EVP_PKEY_param_check(3)> now work for
...
parameters L<OPENSSL_BOX_EVP_PKEY_param_check(3)> will always return 1.
...
L<OPENSSL_BOX_EVP_default_properties_is_fips_enabled(3)> and
L<OPENSSL_BOX_EVP_default_properties_enable_fips(3)>.
...
Functions such as L<OPENSSL_BOX_EVP_PKEY_get0_RSA(3)> behave slightly differently in
...
L<OPENSSL_BOX_EVP_PKEY_set1_RSA(3)>, etc.
...
L<OPENSSL_BOX_EVP_PKEY_get0_RSA(3)>, L<OPENSSL_BOX_EVP_PKEY_get0_DSA(3)>, L<OPENSSL_BOX_EVP_PKEY_get0_EC_KEY(3)> and
L<OPENSSL_BOX_EVP_PKEY_get0_DH(3)> have been made const. This may break some existing code.
...
The L<OPENSSL_BOX_EVP_PKEY_get1_RSA(3)>, L<OPENSSL_BOX_EVP_PKEY_get1_DSA(3)>, L<OPENSSL_BOX_EVP_PKEY_get1_EC_KEY(3)>
and L<OPENSSL_BOX_EVP_PKEY_get1_DH(3)> functions continue to return a non-const pointer to
enable them to be "freed". However they should also be treated as read-only.

=head4 The public key check has moved from OPENSSL_BOX_EVP_PKEY_derive() to OPENSSL_BOX_EVP_PKEY_derive_set_peer()

This may mean result in an error in L<OPENSSL_BOX_EVP_PKEY_derive_set_peer(3)> rather than
during L<OPENSSL_BOX_EVP_PKEY_derive(3)>.
To disable this check use OPENSSL_BOX_EVP_PKEY_derive_set_peer_ex(dh, peer, 0).
...
=head4 OPENSSL_BOX_EVP_CIPHER_CTX_set_flags() ordering change
...
See L<OPENSSL_BOX_EVP_EncryptInit(3)/FLAGS> for more information.
...
using OPENSSL_BOX_EVP_CIPHER_CTX_ctrl(ctx, EVP_CRTL_AEAD_SET_IVLEN, ivlen, NULL). This resulted
...
B<const EVP_MD *> such as OPENSSL_BOX_EVP_sha256() should be replaced with a call to
L<OPENSSL_BOX_EVP_MD_fetch(3)>. See L<crypto(7)/ALGORITHM FETCHING>.

If a library context is needed then all EVP_* cipher functions that return a
B<const EVP_CIPHER *> such as OPENSSL_BOX_EVP_aes_128_cbc() should be replaced vith a call to
L<OPENSSL_BOX_EVP_CIPHER_fetch(3)>. See L<crypto(7)/ALGORITHM FETCHING>.
...
L<OPENSSL_BOX_d2i_AutoPrivateKey(3)>, L<OPENSSL_BOX_d2i_PrivateKey(3)> and L<d2i_PUBKEY(3)>
...
L<OPENSSL_BOX_EVP_DigestSignInit(3)> and L<OPENSSL_BOX_EVP_DigestVerifyInit(3)>

=item *

L<OPENSSL_BOX_EVP_PBE_CipherInit(3)>, L<OPENSSL_BOX_EVP_PBE_find(3)> and L<OPENSSL_BOX_EVP_PBE_scrypt(3)>

=item *

L<OPENSSL_BOX_PKCS5_PBE_keyivgen(3)>
...
L<OPENSSL_BOX_EVP_PKEY_CTX_new_id(3)>

Use L<OPENSSL_BOX_EVP_PKEY_CTX_new_from_name(3)>

=item *

L<OPENSSL_BOX_EVP_PKEY_derive_set_peer(3)>, L<OPENSSL_BOX_EVP_PKEY_new_raw_private_key(3)>
and L<OPENSSL_BOX_EVP_PKEY_new_raw_public_key(3)>

=item *

L<OPENSSL_BOX_EVP_SignFinal(3)> and L<OPENSSL_BOX_EVP_VerifyFinal(3)>
...
L<PKCS5_pbkdf2_set(3)> and L<OPENSSL_BOX_PKCS5_v2_scrypt_keyivgen(3)>
...
L<OPENSSL_BOX_EVP_ASYM_CIPHER_fetch(3)> and L<OPENSSL_BOX_EVP_ASYM_CIPHER_do_all_provided(3)>

=item *

L<OPENSSL_BOX_EVP_CIPHER_fetch(3)> and L<OPENSSL_BOX_EVP_CIPHER_do_all_provided(3)>

=item *

L<OPENSSL_BOX_EVP_default_properties_enable_fips(3)> and
L<OPENSSL_BOX_EVP_default_properties_is_fips_enabled(3)>
...
L<OPENSSL_BOX_EVP_KEM_fetch(3)> and L<OPENSSL_BOX_EVP_KEM_do_all_provided(3)>

=item *

L<OPENSSL_BOX_EVP_KEYEXCH_fetch(3)> and L<OPENSSL_BOX_EVP_KEYEXCH_do_all_provided(3)>

=item *

L<OPENSSL_BOX_EVP_KEYMGMT_fetch(3)> and L<OPENSSL_BOX_EVP_KEYMGMT_do_all_provided(3)>

=item *

L<OPENSSL_BOX_EVP_MAC_fetch(3)> and L<OPENSSL_BOX_EVP_MAC_do_all_provided(3)>

=item *

L<OPENSSL_BOX_EVP_MD_fetch(3)> and L<OPENSSL_BOX_EVP_MD_do_all_provided(3)>

=item *

L<OPENSSL_BOX_EVP_PKEY_CTX_new_from_pkey(3)>

=item *

L<OPENSSL_BOX_EVP_PKEY_Q_keygen(3)>

=item *

L<OPENSSL_BOX_EVP_Q_mac(3)> and L<OPENSSL_BOX_EVP_Q_digest(3)>

=item *

L<EVP_RAND(3)> and L<OPENSSL_BOX_EVP_RAND_do_all_provided(3)>

=item *

L<OPENSSL_BOX_EVP_set_default_properties(3)>

=item *

L<OPENSSL_BOX_EVP_SIGNATURE_fetch(3)> and L<OPENSSL_BOX_EVP_SIGNATURE_do_all_provided(3)>
...
The existing functions for controls (such as L<OPENSSL_BOX_EVP_CIPHER_CTX_ctrl(3)>) and
manipulating flags (such as L<OPENSSL_BOX_EVP_MD_CTX_set_flags(3)>)internally use
...
For ciphers see L<OPENSSL_BOX_EVP_EncryptInit(3)/CONTROLS>, L<OPENSSL_BOX_EVP_EncryptInit(3)/FLAGS> and
L<OPENSSL_BOX_EVP_EncryptInit(3)/PARAMETERS>.

For digests see L<OPENSSL_BOX_EVP_DigestInit(3)/CONTROLS>, L<OPENSSL_BOX_EVP_DigestInit(3)/FLAGS> and
L<OPENSSL_BOX_EVP_DigestInit(3)/PARAMETERS>.
...
Any accessor that uses an ENGINE is deprecated (such as OPENSSL_BOX_EVP_PKEY_set1_engine()).
...
Applications that set or get low-level key objects (such as OPENSSL_BOX_EVP_PKEY_set1_DH()
or OPENSSL_BOX_EVP_PKEY_get0()) should instead use the OSSL_ENCODER
(See L<OSSL_ENCODER_to_bio(3)>) or OSSL_DECODER (See L<OSSL_DECODER_from_bio(3)>)
APIs, or alternatively use L<OPENSSL_BOX_EVP_PKEY_fromdata(3)> or L<OPENSSL_BOX_EVP_PKEY_todata(3)>.
...
L<OPENSSL_BOX_EVP_PKEY_get_bn_param(3)>,
L<OPENSSL_BOX_EVP_PKEY_get_int_param(3)>,
L<OPENSSL_BOX_EVP_PKEY_get_size_t_param(3)>,
L<OPENSSL_BOX_EVP_PKEY_get_utf8_string_param(3)>,
L<OPENSSL_BOX_EVP_PKEY_get_octet_string_param(3)>, or
L<OPENSSL_BOX_EVP_PKEY_get_params(3)>,
...
Applications may also use L<OPENSSL_BOX_EVP_PKEY_todata(3)> to return all fields.
...
are now deprecated. Applications should use L<OPENSSL_BOX_EVP_PKEY_fromdata(3)> to create
new keys from user provided key data. Keys should be immutable once they are
created, so if required the user may use L<OPENSSL_BOX_EVP_PKEY_todata(3)>, L<OSSL_PARAM_merge(3)>,
and L<OPENSSL_BOX_EVP_PKEY_fromdata(3)> to create a modified key.
...
high-level EVP_PKEY APIs, e.g. L<OPENSSL_BOX_EVP_PKEY_new(3)>, L<OPENSSL_BOX_EVP_PKEY_up_ref(3)> and
L<OPENSSL_BOX_EVP_PKEY_free(3)>.
See also L<OPENSSL_BOX_EVP_PKEY_CTX_new_from_name(3)> and L<OPENSSL_BOX_EVP_PKEY_CTX_new_from_pkey(3)>.
...
instead use the high level EVP APIs L<OPENSSL_BOX_EVP_EncryptInit_ex(3)>,
L<OPENSSL_BOX_EVP_EncryptUpdate(3)>, and L<OPENSSL_BOX_EVP_EncryptFinal_ex(3)> or
L<OPENSSL_BOX_EVP_DecryptInit_ex(3)>, L<OPENSSL_BOX_EVP_DecryptUpdate(3)> and L<OPENSSL_BOX_EVP_DecryptFinal_ex(3)>.
...
use the high level EVP APIs L<OPENSSL_BOX_EVP_DigestInit_ex(3)>, L<OPENSSL_BOX_EVP_DigestUpdate(3)>
and L<OPENSSL_BOX_EVP_DigestFinal_ex(3)>, or the quick one-shot L<OPENSSL_BOX_EVP_Q_digest(3)>.

Note that the functions L<SHA1(3)>, L<SHA224(3)>, L<SHA256(3)>, L<SHA384(3)>
and L<SHA512(3)> have changed to macros that use L<OPENSSL_BOX_EVP_Q_digest(3)>.
...
L<OPENSSL_BOX_EVP_MAC_CTX_new(3)>, L<OPENSSL_BOX_EVP_MAC_CTX_free(3)>, L<OPENSSL_BOX_EVP_MAC_init(3)>,
L<OPENSSL_BOX_EVP_MAC_update(3)> and L<OPENSSL_BOX_EVP_MAC_final(3)> or the single-shot MAC function
L<OPENSSL_BOX_EVP_Q_mac(3)>.
...
EVP_PKEY APIs such as L<OPENSSL_BOX_EVP_PKEY_check(3)>, L<OPENSSL_BOX_EVP_PKEY_param_check(3)>,
L<OPENSSL_BOX_EVP_PKEY_param_check_quick(3)>, L<OPENSSL_BOX_EVP_PKEY_public_check(3)>,
L<OPENSSL_BOX_EVP_PKEY_public_check_quick(3)>, L<OPENSSL_BOX_EVP_PKEY_private_check(3)>,
and L<OPENSSL_BOX_EVP_PKEY_pairwise_check(3)>.
...
time. Applications should instead use L<OPENSSL_BOX_EVP_PKEY_derive(3)>.
...
Many low-level functions have been informally discouraged from use for a long
time. Applications should instead use L<OPENSSL_BOX_EVP_PKEY_keygen_init(3)> and
L<OPENSSL_BOX_EVP_PKEY_generate(3)> as described in L<EVP_PKEY-DSA(7)>, L<EVP_PKEY-DH(7)>,
L<EVP_PKEY-RSA(7)>, L<EVP_PKEY-EC(7)> and L<EVP_PKEY-X25519(7)>.
The 'quick' one-shot function L<OPENSSL_BOX_EVP_PKEY_Q_keygen(3)> and macros for the most
...
Application should use one of L<OPENSSL_BOX_EVP_PKEY_print_public(3)>,
L<OPENSSL_BOX_EVP_PKEY_print_private(3)>, L<OPENSSL_BOX_EVP_PKEY_print_params(3)>,
L<OPENSSL_BOX_EVP_PKEY_print_public_fp(3)>, L<OPENSSL_BOX_EVP_PKEY_print_private_fp(3)> or
L<OPENSSL_BOX_EVP_PKEY_print_params_fp(3)>. Note that internally these use
L<OSSL_ENCODER_to_bio(3)> and L<OSSL_DECODER_from_bio(3)>.
...
Use L<OPENSSL_BOX_EVP_PKEY_keygen(3)> instead.
...
Use the higher level functions OPENSSL_BOX_EVP_CipherInit_ex2(), OPENSSL_BOX_EVP_CipherUpdate() and
OPENSSL_BOX_EVP_CipherFinal_ex() instead.
See the "cts_mode" parameter in
L<OPENSSL_BOX_EVP_EncryptInit(3)/Gettable and Settable EVP_CIPHER_CTX parameters>.
See L<OPENSSL_BOX_EVP_EncryptInit(3)/EXAMPLES> for a AES-256-CBC-CTS example.
...
Use L<OPENSSL_BOX_EVP_PKEY_get_bits(3)>, L<OPENSSL_BOX_EVP_PKEY_get_security_bits(3)> and
L<OPENSSL_BOX_EVP_PKEY_get_size(3)>.
...
Use OPENSSL_BOX_EVP_PKEY_is_a() to determine the type of a key.
...
There is no direct replacement. Applications may use L<OPENSSL_BOX_EVP_PKEY_copy_parameters(3)>
and L<OPENSSL_BOX_EVP_PKEY_dup(3)> instead.
...
Applications should use L<OPENSSL_BOX_EVP_PKEY_get_size(3)>.
...
Applications should use L<OPENSSL_BOX_EVP_PKEY_can_sign(3)> instead.
...
OPENSSL_BOX_EVP_CIPHER_CTX_iv(), OPENSSL_BOX_EVP_CIPHER_CTX_iv_noconst(), OPENSSL_BOX_EVP_CIPHER_CTX_original_iv()

Applications should instead use L<OPENSSL_BOX_EVP_CIPHER_CTX_get_updated_iv(3)>,
L<OPENSSL_BOX_EVP_CIPHER_CTX_get_updated_iv(3)> and L<OPENSSL_BOX_EVP_CIPHER_CTX_get_original_iv(3)>
respectively.
See L<OPENSSL_BOX_EVP_CIPHER_CTX_get_original_iv(3)> for further information.

=item *

B<EVP_CIPHER_meth_*()>, OPENSSL_BOX_EVP_MD_CTX_set_update_fn(), EVP_MD_CTX_update_fn(),
B<EVP_MD_meth_*()>
...
OPENSSL_BOX_EVP_PKEY_cmp(), OPENSSL_BOX_EVP_PKEY_cmp_parameters()

Applications should use L<OPENSSL_BOX_EVP_PKEY_eq(3)> and L<OPENSSL_BOX_EVP_PKEY_parameters_eq(3)> instead.
See L<OPENSSL_BOX_EVP_PKEY_copy_parameters(3)> for further details.

=item *

OPENSSL_BOX_EVP_PKEY_encrypt_old(), OPENSSL_BOX_EVP_PKEY_decrypt_old(),

Applications should use L<OPENSSL_BOX_EVP_PKEY_encrypt_init(3)> and L<OPENSSL_BOX_EVP_PKEY_encrypt(3)> or
L<OPENSSL_BOX_EVP_PKEY_decrypt_init(3)> and L<OPENSSL_BOX_EVP_PKEY_decrypt(3)> instead.

=item *

OPENSSL_BOX_EVP_PKEY_get0()

This function returns NULL if the key comes from a provider.

=item *

OPENSSL_BOX_EVP_PKEY_get0_DH(), OPENSSL_BOX_EVP_PKEY_get0_DSA(), OPENSSL_BOX_EVP_PKEY_get0_EC_KEY(), OPENSSL_BOX_EVP_PKEY_get0_RSA(),
OPENSSL_BOX_EVP_PKEY_get1_DH(), OPENSSL_BOX_EVP_PKEY_get1_DSA(), OPENSSL_BOX_EVP_PKEY_get1_EC_KEY and OPENSSL_BOX_EVP_PKEY_get1_RSA(),
OPENSSL_BOX_EVP_PKEY_get0_hmac(), OPENSSL_BOX_EVP_PKEY_get0_poly1305(), OPENSSL_BOX_EVP_PKEY_get0_siphash()
...
OPENSSL_BOX_EVP_PKEY_new_CMAC_key()

See L</Deprecated low-level MAC functions>.

=item *

OPENSSL_BOX_EVP_PKEY_assign(), OPENSSL_BOX_EVP_PKEY_set1_DH(), OPENSSL_BOX_EVP_PKEY_set1_DSA(),
OPENSSL_BOX_EVP_PKEY_set1_EC_KEY(), OPENSSL_BOX_EVP_PKEY_set1_RSA()
...
L<OPENSSL_BOX_EVP_PKEY_get1_encoded_public_key(3)>.
The old versions have been converted to deprecated macros that just call the
new functions.

=item *

OPENSSL_BOX_EVP_PKEY_set1_engine(), OPENSSL_BOX_EVP_PKEY_get0_engine()
...
Use L<OPENSSL_BOX_EVP_PKEY_get1_encoded_public_key(3)>.
See L</Deprecated low-level key parameter getters>
...
There is no direct replacement. Applications may use L<OPENSSL_BOX_EVP_PKEY_dup(3)>.
...
EVP_PKEY_id(), OPENSSL_BOX_EVP_PKEY_get_id()
...
L<OPENSSL_BOX_EVP_PKEY_get_id(3)> might now also return the value -1
(B<EVP_PKEY_KEYMGMT>) indicating the use of a provider to
implement the EVP_PKEY object. Therefore, the use of
L<OPENSSL_BOX_EVP_PKEY_get0_type_name(3)> is recommended for retrieving
