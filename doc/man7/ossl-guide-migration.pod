...
L<EVP_EncryptInit_ex(3)>, and L<OPENSSL_BOX_EVP_DigestInit(3)> functions. In case when
...
L<OPENSSL_BOX_EVP_PKEY_derive_set_peer(3)>, L<EVP_PKEY_new_raw_private_key(3)>
...
APIs, or alternatively use L<EVP_PKEY_fromdata(3)> or L<OPENSSL_BOX_EVP_PKEY_todata(3)>.
...
L<OPENSSL_BOX_EVP_MAC_CTX_new(3)>, L<OPENSSL_BOX_EVP_MAC_CTX_free(3)>, L<EVP_MAC_init(3)>,
L<OPENSSL_BOX_EVP_MAC_update(3)> and L<EVP_MAC_final(3)> or the single-shot MAC function
L<EVP_Q_mac(3)>.
...
Applications should use L<OPENSSL_BOX_EVP_PKEY_encrypt_init(3)> and L<EVP_PKEY_encrypt(3)> or
L<OPENSSL_BOX_EVP_PKEY_decrypt_init(3)> and L<EVP_PKEY_decrypt(3)> instead.
