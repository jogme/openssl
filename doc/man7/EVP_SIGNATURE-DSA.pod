...
L<OPENSSL_BOX_EVP_PKEY_sign(3)> is DER encoded ASN.1 in the form described in
...
L<OPENSSL_BOX_EVP_PKEY_sign_init_ex2(3)>, and implicitly fetched (through
L<EC keys|EVP_PKEY-EC(7)>) with L<OPENSSL_BOX_EVP_DigestSignInit(3)> and
L<OPENSSL_BOX_EVP_DigestVerifyInit(3)>.

It can't be used with L<OPENSSL_BOX_EVP_PKEY_sign_message_init(3)>
...
supported explicitly fetched with L<OPENSSL_BOX_EVP_PKEY_sign_init_ex2(3)> and
L<OPENSSL_BOX_EVP_PKEY_sign_message_init(3)>.

=back

=head2 Signature Parameters

The following signature parameters can be set using OPENSSL_BOX_EVP_PKEY_CTX_set_params().
This may be called after OPENSSL_BOX_EVP_PKEY_sign_init() or OPENSSL_BOX_EVP_PKEY_verify_init(),
and before calling OPENSSL_BOX_EVP_PKEY_sign() or OPENSSL_BOX_EVP_PKEY_verify(). They may also be set
using OPENSSL_BOX_EVP_PKEY_sign_init_ex() or OPENSSL_BOX_EVP_PKEY_verify_init_ex().
