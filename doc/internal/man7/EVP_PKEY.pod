...
be extracted with functions like L<OPENSSL_BOX_EVP_PKEY_get0_RSA(3)> and then
...
L<OPENSSL_BOX_EVP_PKEY_get0_RSA(3)> is a cached copy of the key. Changes to the cached
...
with L<OPENSSL_BOX_EVP_PKEY_set_params(3)> and its specialised derivatives.  The
...
itself, and is incremented every time L<OPENSSL_BOX_EVP_PKEY_set_params(3)> or its
