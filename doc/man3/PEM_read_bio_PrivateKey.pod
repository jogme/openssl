...
using X509_free() for X509 objects or OPENSSL_BOX_EVP_PKEY_free() for EVP_PKEY objects.
...
used by OPENSSL_BOX_EVP_get_cipherbyname() and an initialization vector used by the
cipher encoded as a set of hexadecimal digits. After those two lines is
the base64-encoded encrypted data.

The encryption key is derived using OPENSSL_BOX_EVP_BytesToKey(). The cipher's
initialization vector is passed to OPENSSL_BOX_EVP_BytesToKey() as the I<salt>
...
password is passed to OPENSSL_BOX_EVP_BytesToKey() using the I<data> and I<datal>
parameters. Finally, the library uses an iteration count of 1 for
OPENSSL_BOX_EVP_BytesToKey().

The I<key> derived by OPENSSL_BOX_EVP_BytesToKey() along with the original initialization
vector is then used to decrypt the encrypted data. The I<iv> produced by
OPENSSL_BOX_EVP_BytesToKey() is not utilized or needed, and NULL should be passed to
the function.

The pseudo code to derive the key would look similar to:

 EVP_CIPHER* cipher = OPENSSL_BOX_EVP_des_ede3_cbc();
 EVP_MD* md = OPENSSL_BOX_EVP_md5();

 unsigned int nkey = OPENSSL_BOX_EVP_CIPHER_get_key_length(cipher);
 unsigned int niv = OPENSSL_BOX_EVP_CIPHER_get_iv_length(cipher);
 unsigned char key[nkey];
 unsigned char iv[niv];

 memcpy(iv, HexToBin("3F17F5316E2BAC89"), niv);
 rc = OPENSSL_BOX_EVP_BytesToKey(cipher, md, iv /*salt*/, pword, plen, 1, key, NULL /*iv*/);
 if (rc != nkey)
     /* Error */
...
triple DES encryption, the pass phrase is prompted for:
 ...
 if (!PEM_write_bio_PrivateKey(bp, key, OPENSSL_BOX_EVP_des_ede3_cbc(), NULL, 0, 0, NULL))
     /* Error */

Write a private key (using PKCS#8 format) to a BIO using triple
DES encryption, using the pass phrase "hello":

 if (!PEM_write_bio_PKCS8PrivateKey(bp, key, OPENSSL_BOX_EVP_des_ede3_cbc(),
                                    NULL, 0, 0, "hello"))
     /* Error */
...
L<OPENSSL_BOX_EVP_EncryptInit(3)>, L<OPENSSL_BOX_EVP_BytesToKey(3)>,
