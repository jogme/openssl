...
OPENSSL_BOX_EVP_PKEY_get_size, OPENSSL_BOX_EVP_PKEY_get_bits, OPENSSL_BOX_EVP_PKEY_get_security_bits,
EVP_PKEY_bits, EVP_PKEY_security_bits, OPENSSL_BOX_EVP_PKEY_get_security_category,
EVP_PKEY_size
...
OPENSSL_BOX_EVP_PKEY_get_size() returns the maximum suitable size for the output
...
The primary documented use is with L<OPENSSL_BOX_EVP_SignFinal(3)> and
L<OPENSSL_BOX_EVP_SealInit(3)>, but it isn't limited there.  The returned size is
also large enough for the output buffer of L<OPENSSL_BOX_EVP_PKEY_sign(3)>,
L<OPENSSL_BOX_EVP_PKEY_encrypt(3)>, L<OPENSSL_BOX_EVP_PKEY_decrypt(3)>, L<OPENSSL_BOX_EVP_PKEY_derive(3)>.
...
OPENSSL_BOX_EVP_PKEY_get_size() is only preliminary and not exact, so the final
...
operation, such as L<OPENSSL_BOX_EVP_PKEY_sign(3)> (the I<siglen> argument will
receive that length), to avoid bugs.

OPENSSL_BOX_EVP_PKEY_get_bits() returns the cryptographic length of the cryptosystem
...
OPENSSL_BOX_EVP_PKEY_get_security_bits() returns the number of security bits of the given
...
OPENSSL_BOX_EVP_PKEY_get_security_category() returns the
...
OPENSSL_BOX_EVP_PKEY_get_size(), OPENSSL_BOX_EVP_PKEY_get_bits() and OPENSSL_BOX_EVP_PKEY_get_security_bits()
return a positive number, or 0 if this size isn't available.

OPENSSL_BOX_EVP_PKEY_get_security_category() returns the post-quantum security category
...
OPENSSL_BOX_EVP_PKEY_get_size() have a functionality where you can pass NULL for the
...
isn't strictly necessary to call OPENSSL_BOX_EVP_PKEY_get_size() to find out the
buffer size, but may be useful in cases where it's desirable to know
the upper limit in advance.

It should also be especially noted that OPENSSL_BOX_EVP_PKEY_get_size() shouldn't be
used to get the output size for OPENSSL_BOX_EVP_DigestSignFinal(), according to
L<OPENSSL_BOX_EVP_DigestSignFinal(3)/NOTES>.

=head1 SEE ALSO

L<provider-keymgmt(7)>,
L<OPENSSL_BOX_EVP_SignFinal(3)>,
L<OPENSSL_BOX_EVP_SealInit(3)>,
...
OPENSSL_BOX_EVP_PKEY_get_security_category() was added in OpenSSL 3.6.
