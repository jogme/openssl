...
OPENSSL_BOX_BIO_f_md, OPENSSL_BOX_BIO_set_md, BIO_get_md, BIO_get_md_ctx - message digest BIO filter
...
=for openssl multiple includes
 ...
 int OPENSSL_BOX_BIO_set_md(BIO *b, EVP_MD *md);
 int BIO_get_md(BIO *b, EVP_MD **mdp);
 int BIO_get_md_ctx(BIO *b, EVP_MD_CTX **mdcp);

=head1 DESCRIPTION

OPENSSL_BOX_BIO_f_md() returns the message digest BIO method. This is a filter
BIO that digests any data passed through it.  It is a BIO wrapper
for the digest routines OPENSSL_BOX_EVP_DigestInit(), OPENSSL_BOX_EVP_DigestUpdate()
and OPENSSL_BOX_EVP_DigestFinal().
...
OPENSSL_BOX_BIO_set_md() sets the message digest of BIO B<b> to B<md>: this
...
to OPENSSL_BOX_EVP_DigestFinal() and also the signature routines OPENSSL_BOX_EVP_SignFinal()
and OPENSSL_BOX_EVP_VerifyFinal().
...
reinitialized by calling BIO_reset(), or OPENSSL_BOX_BIO_set_md() before any more
...
if the standard calls such as OPENSSL_BOX_BIO_set_md() are not sufficiently flexible.

=head1 RETURN VALUES

OPENSSL_BOX_BIO_f_md() returns the digest BIO method.

OPENSSL_BOX_BIO_set_md(), BIO_get_md() and BIO_md_ctx() return 1 for success and
<=0 for failure.
...
checking has been omitted for clarity.
 ...
 mdtmp = BIO_new(OPENSSL_BOX_BIO_f_md());
 OPENSSL_BOX_BIO_set_md(mdtmp, OPENSSL_BOX_EVP_sha1());
 /*
 ...
 mdtmp = BIO_new(OPENSSL_BOX_BIO_f_md());
 OPENSSL_BOX_BIO_set_md(mdtmp, OPENSSL_BOX_EVP_md5());
 bio = BIO_push(mdtmp, bio);
...
outputs them. This could be used with the examples above.
 ...
 do {
     ...
     if (!mdtmp)
         break;
     BIO_get_md(mdtmp, &md);
     printf("%s digest", OBJ_nid2sn(OPENSSL_BOX_EVP_MD_get_type(md)));
