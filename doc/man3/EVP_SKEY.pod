...
EVP_SKEY, OPENSSL_BOX_EVP_SKEY_generate,
OPENSSL_BOX_EVP_SKEY_import, OPENSSL_BOX_EVP_SKEY_import_raw_key, OPENSSL_BOX_EVP_SKEY_up_ref,
OPENSSL_BOX_EVP_SKEY_export, OPENSSL_BOX_EVP_SKEY_get0_raw_key, OPENSSL_BOX_EVP_SKEY_get0_key_id,
OPENSSL_BOX_EVP_SKEY_get0_skeymgmt_name, OPENSSL_BOX_EVP_SKEY_get0_provider_name,
OPENSSL_BOX_EVP_SKEY_free, OPENSSL_BOX_EVP_SKEY_is_a, OPENSSL_BOX_EVP_SKEY_to_provider
...
=head1 SYNOPSIS
 ...
 int OPENSSL_BOX_EVP_SKEY_up_ref(EVP_SKEY *key);
 void OPENSSL_BOX_EVP_SKEY_free(EVP_SKEY *key);
...
The OPENSSL_BOX_EVP_SKEY_generate() functions creates a new B<EVP_SKEY> object and
initializes it according to the B<params> argument.

The OPENSSL_BOX_EVP_SKEY_import() function allocates an empty B<EVP_SKEY> structure
...
The OPENSSL_BOX_EVP_SKEY_import_raw_key() function is a helper that creates an B<EVP_SKEY> object
containing the raw byte representation of the symmetric keys.

The OPENSSL_BOX_EVP_SKEY_export() function extracts values from a key I<skey> using the
...
The OPENSSL_BOX_EVP_SKEY_get0_raw_key() returns a pointer to a raw key bytes to the passed
...
The OPENSSL_BOX_EVP_SKEY_get0_key_id() returns a NUL-terminated string providing some
...
The OPENSSL_BOX_EVP_SKEY_get0_skeymgmt_name() and OPENSSL_BOX_EVP_SKEY_get0_provider_name() return the
names of the associated EVP_SKEYMGMT object and its provider correspondingly.

OPENSSL_BOX_EVP_SKEY_up_ref() increments the reference count of I<key>.

OPENSSL_BOX_EVP_SKEY_free() decrements the reference count of I<key> and, if the reference
count is zero, frees it. If I<key> is NULL, nothing is done.

OPENSSL_BOX_EVP_SKEY_is_a() checks if the key type of I<skey> is I<name>.

OPENSSL_BOX_EVP_SKEY_to_provider() simplifies the task of importing a I<skey> into a
...
The OPENSSL_BOX_EVP_SKEY_to_provider() function will fail and return NULL if the origin
key I<skey> cannot be exported from its provider.

=head1 RETURN VALUES

OPENSSL_BOX_EVP_SKEY_generate(), OPENSSL_BOX_EVP_SKEY_import() and OPENSSL_BOX_EVP_SKEY_import_raw_key() return
either the newly allocated B<EVP_SKEY> structure or NULL if an error occurred.

OPENSSL_BOX_EVP_SKEY_get0_key_id() returns either a valid pointer or NULL.

OPENSSL_BOX_EVP_SKEY_up_ref() returns 1 for success and 0 on failure.

OPENSSL_BOX_EVP_SKEY_export() and OPENSSL_BOX_EVP_SKEY_get0_raw_key() return 1 for success and 0 on failure.

OPENSSL_BOX_EVP_SKEY_get0_skeymgmt_name() and OPENSSL_BOX_EVP_SKEY_get0_provider_name() return the
names of the associated EVP_SKEYMGMT object and its provider correspondigly.

OPENSSL_BOX_EVP_SKEY_is_a() returns 1 if I<skey> has the key type I<name>,
otherwise 0.

OPENSSL_BOX_EVP_SKEY_to_provider() returns a new B<EVP_SKEY> suitable for operations with
...
The B<EVP_SKEY> API and functions OPENSSL_BOX_EVP_SKEY_export(),
OPENSSL_BOX_EVP_SKEY_free(), OPENSSL_BOX_EVP_SKEY_get0_raw_key(), OPENSSL_BOX_EVP_SKEY_import(),
OPENSSL_BOX_EVP_SKEY_import_raw_key(), OPENSSL_BOX_EVP_SKEY_up_ref(), OPENSSL_BOX_EVP_SKEY_generate(),
OPENSSL_BOX_EVP_SKEY_get0_key_id(), OPENSSL_BOX_EVP_SKEY_get0_provider_name(),
OPENSSL_BOX_EVP_SKEY_get0_skeymgmt_name(), OPENSSL_BOX_EVP_SKEY_is_a(), OPENSSL_BOX_EVP_SKEY_to_provider()
