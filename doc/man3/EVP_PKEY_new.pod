...
EVP_PKEY,
OPENSSL_BOX_EVP_PKEY_new,
OPENSSL_BOX_EVP_PKEY_up_ref,
OPENSSL_BOX_EVP_PKEY_dup,
OPENSSL_BOX_EVP_PKEY_free,
OPENSSL_BOX_EVP_PKEY_new_raw_private_key_ex,
OPENSSL_BOX_EVP_PKEY_new_raw_private_key,
OPENSSL_BOX_EVP_PKEY_new_raw_public_key_ex,
OPENSSL_BOX_EVP_PKEY_new_raw_public_key,
OPENSSL_BOX_EVP_PKEY_new_CMAC_key,
OPENSSL_BOX_EVP_PKEY_new_mac_key,
OPENSSL_BOX_EVP_PKEY_get_raw_private_key,
OPENSSL_BOX_EVP_PKEY_get_raw_public_key
...
=head1 SYNOPSIS
 ...
 int OPENSSL_BOX_EVP_PKEY_up_ref(EVP_PKEY *key);
 EVP_PKEY *OPENSSL_BOX_EVP_PKEY_dup(EVP_PKEY *key);
 void OPENSSL_BOX_EVP_PKEY_free(EVP_PKEY *key);
 ...
 EVP_PKEY *OPENSSL_BOX_EVP_PKEY_new_raw_public_key(int type, ENGINE *e,
                                       const unsigned char *key, size_t keylen);
 EVP_PKEY *OPENSSL_BOX_EVP_PKEY_new_mac_key(int type, ENGINE *e, const unsigned char *key,
                                int keylen);
...
The OPENSSL_BOX_EVP_PKEY_new() function allocates an empty B<EVP_PKEY> structure which is
used by OpenSSL to store public and private keys. The reference count is set to
B<1>.

OPENSSL_BOX_EVP_PKEY_up_ref() increments the reference count of I<key>.

OPENSSL_BOX_EVP_PKEY_dup() duplicates the I<key>. The I<key> must not be ENGINE based or
a raw key, otherwise the duplication will fail.

OPENSSL_BOX_EVP_PKEY_free() decrements the reference count of I<key> and, if the reference
count is zero, frees it up. If I<key> is NULL, nothing is done.

OPENSSL_BOX_EVP_PKEY_new_raw_private_key_ex() allocates a new B<EVP_PKEY>. Unless an
...
OPENSSL_BOX_EVP_PKEY_new_raw_private_key() does the same as
OPENSSL_BOX_EVP_PKEY_new_raw_private_key_ex() except that the default library context and
...
OPENSSL_BOX_EVP_PKEY_new_raw_private_key_ex() and OPENSSL_BOX_EVP_PKEY_new_raw_private_key() may also
...
OPENSSL_BOX_EVP_PKEY_new_raw_public_key_ex() works in the same way as
OPENSSL_BOX_EVP_PKEY_new_raw_private_key_ex() except that I<key> points to the raw
...
OPENSSL_BOX_EVP_PKEY_new_raw_public_key() works in the same way as
OPENSSL_BOX_EVP_PKEY_new_raw_private_key_ex() except that I<key> points to the raw public
key data.
The B<EVP_PKEY> structure will be initialised without any private key
information.

OPENSSL_BOX_EVP_PKEY_new_mac_key() works in the same way as OPENSSL_BOX_EVP_PKEY_new_raw_private_key().
New applications should use OPENSSL_BOX_EVP_PKEY_new_raw_private_key() instead.

OPENSSL_BOX_EVP_PKEY_get_raw_private_key() fills the buffer provided by I<priv> with raw
...
and
B<SIPHASH>.
OPENSSL_BOX_EVP_PKEY_get_raw_private_key() also works with
C<ML-DSA-44>,
C<ML-DSA-65>,
C<ML-DSA-87>,
B<ML-KEM-512>,
...
OPENSSL_BOX_EVP_PKEY_get_raw_public_key() fills the buffer provided by I<pub> with raw
...
and
B<X448>
OPENSSL_BOX_EVP_PKEY_get_raw_public_key() also works with
C<ML-DSA-44>,
C<ML-DSA-65>,
C<ML-DSA-87>,
B<ML-KEM-512>,
...
OPENSSL_BOX_EVP_PKEY_new_CMAC_key() works in the same way as OPENSSL_BOX_EVP_PKEY_new_raw_private_key()
...
The structure returned by OPENSSL_BOX_EVP_PKEY_new() is empty. To add a private or public
key to this empty structure use the appropriate functions described in
L<OPENSSL_BOX_EVP_PKEY_set1_RSA(3)>, L<OPENSSL_BOX_EVP_PKEY_set1_DSA(3)>, L<OPENSSL_BOX_EVP_PKEY_set1_DH(3)> or
L<OPENSSL_BOX_EVP_PKEY_set1_EC_KEY(3)> for legacy key types implemented in internal
OpenSSL providers.
...
L<EVP_PKEY_set1_encoded_public_key(3)> or L<OPENSSL_BOX_EVP_PKEY_fromdata(3)>
...
OPENSSL_BOX_EVP_PKEY_new(), OPENSSL_BOX_EVP_PKEY_new_raw_private_key(), OPENSSL_BOX_EVP_PKEY_new_raw_public_key(),
OPENSSL_BOX_EVP_PKEY_new_CMAC_key() and OPENSSL_BOX_EVP_PKEY_new_mac_key() return either the newly
allocated B<EVP_PKEY> structure or NULL if an error occurred.

OPENSSL_BOX_EVP_PKEY_dup() returns the key duplicate or NULL if an error occurred.

OPENSSL_BOX_EVP_PKEY_up_ref(), OPENSSL_BOX_EVP_PKEY_get_raw_private_key() and
OPENSSL_BOX_EVP_PKEY_get_raw_public_key() return 1 for success and 0 for failure.

=head1 SEE ALSO

L<OPENSSL_BOX_EVP_PKEY_set1_RSA(3)>,
L<OPENSSL_BOX_EVP_PKEY_set1_DSA(3)>,
L<OPENSSL_BOX_EVP_PKEY_set1_DH(3)>,
L<OPENSSL_BOX_EVP_PKEY_set1_EC_KEY(3)>,
...
The
OPENSSL_BOX_EVP_PKEY_new() and OPENSSL_BOX_EVP_PKEY_free() functions exist in all versions of OpenSSL.

The OPENSSL_BOX_EVP_PKEY_up_ref() function was added in OpenSSL 1.1.0.

The
OPENSSL_BOX_EVP_PKEY_new_raw_private_key(), OPENSSL_BOX_EVP_PKEY_new_raw_public_key(),
OPENSSL_BOX_EVP_PKEY_new_CMAC_key(), OPENSSL_BOX_EVP_PKEY_new_raw_private_key() and
OPENSSL_BOX_EVP_PKEY_get_raw_public_key() functions were added in OpenSSL 1.1.1.

The OPENSSL_BOX_EVP_PKEY_dup(), OPENSSL_BOX_EVP_PKEY_new_raw_private_key_ex(), and
OPENSSL_BOX_EVP_PKEY_new_raw_public_key_ex()
functions were added in OpenSSL 3.0.

The OPENSSL_BOX_EVP_PKEY_new_CMAC_key() was deprecated in OpenSSL 3.0.
