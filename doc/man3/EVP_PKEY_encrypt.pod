...
OPENSSL_BOX_EVP_PKEY_encrypt_init_ex,
OPENSSL_BOX_EVP_PKEY_encrypt_init, OPENSSL_BOX_EVP_PKEY_encrypt - encrypt using a public key algorithm
...
The OPENSSL_BOX_EVP_PKEY_encrypt_init() function initializes a public key algorithm
context using key B<pkey> for an encryption operation.

The OPENSSL_BOX_EVP_PKEY_encrypt_init_ex() function initializes a public key algorithm
context using key B<pkey> for an encryption operation and sets the
algorithm specific B<params>.

The OPENSSL_BOX_EVP_PKEY_encrypt() function performs a public key encryption operation
...
After the call to OPENSSL_BOX_EVP_PKEY_encrypt_init() algorithm specific control
operations can be performed to set any appropriate parameters for the
operation.  These operations can be included in the OPENSSL_BOX_EVP_PKEY_encrypt_init_ex()
call.

The function OPENSSL_BOX_EVP_PKEY_encrypt() can be called more than once on the same
context if several operations are performed using the same parameters.

=head1 RETURN VALUES

OPENSSL_BOX_EVP_PKEY_encrypt_init(), OPENSSL_BOX_EVP_PKEY_encrypt_init_ex() and OPENSSL_BOX_EVP_PKEY_encrypt()
...
set 'eng = NULL;' to start with the default OpenSSL RSA implementation:
 ...
 if (OPENSSL_BOX_EVP_PKEY_encrypt(ctx, NULL, &outlen, in, inlen) <= 0)
     /* Error */
 ...
 if (OPENSSL_BOX_EVP_PKEY_encrypt(ctx, out, &outlen, in, inlen) <= 0)
     /* Error */
