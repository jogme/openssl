...
EVP_SKEYMGMT,
OPENSSL_BOX_EVP_SKEYMGMT_fetch,
OPENSSL_BOX_EVP_SKEYMGMT_up_ref,
OPENSSL_BOX_EVP_SKEYMGMT_free,
OPENSSL_BOX_EVP_SKEYMGMT_get0_provider,
OPENSSL_BOX_EVP_SKEYMGMT_is_a,
OPENSSL_BOX_EVP_SKEYMGMT_get0_description,
OPENSSL_BOX_EVP_SKEYMGMT_get0_name,
OPENSSL_BOX_EVP_SKEYMGMT_do_all_provided,
OPENSSL_BOX_EVP_SKEYMGMT_names_do_all,
OPENSSL_BOX_EVP_SKEYMGMT_get0_gen_settable_params,
OPENSSL_BOX_EVP_SKEYMGMT_get0_imp_settable_params
...
OPENSSL_BOX_EVP_SKEYMGMT_fetch() looks for an algorithm within a provider that
...
OPENSSL_BOX_EVP_SKEYMGMT_up_ref() increments the reference count for the given
B<EVP_SKEYMGMT> I<skeymgmt>.

OPENSSL_BOX_EVP_SKEYMGMT_free() decrements the reference count for the given
B<EVP_SKEYMGMT> I<skeymgmt>, and when the count reaches zero, frees it.
If the argument is NULL, nothing is done.

OPENSSL_BOX_EVP_SKEYMGMT_get0_provider() returns the provider that has this particular
implementation.

OPENSSL_BOX_EVP_SKEYMGMT_is_a() checks if I<skeymgmt> is an implementation of an
algorithm that's identified by I<name>.

OPENSSL_BOX_EVP_SKEYMGMT_get0_name() returns the algorithm name from the provided
...
OPENSSL_BOX_EVP_SKEYMGMT_names_do_all() traverses all names for the I<skeymgmt>, and
calls I<fn> with each name and I<data>.

OPENSSL_BOX_EVP_SKEYMGMT_get0_description() returns a description of the I<skeymgmt>, meant
for display and human consumption.  The description is at the discretion
of the I<skeymgmt> implementation.

OPENSSL_BOX_EVP_SKEYMGMT_do_all_provided() traverses all key I<skeymgmt> implementations by
all activated providers in the library context I<libctx>, and for each
of the implementations, calls I<fn> with the implementation method and
I<data> as arguments.

OPENSSL_BOX_EVP_SKEYMGMT_get0_gen_settable_params() and OPENSSL_BOX_EVP_SKEYMGMT_get0_imp_settable_params()
get a constant L<OSSL_PARAM(3)> array that describes the settable parameters
that can be used with OPENSSL_BOX_EVP_SKEY_generate() and OPENSSL_BOX_EVP_SKEY_import() correspondingly.

=head1 NOTES

OPENSSL_BOX_EVP_SKEYMGMT_fetch() may be called implicitly by other fetching
functions, using the same library context and properties.
Any other API that uses symmetric keys will typically do this.

=head1 RETURN VALUES

OPENSSL_BOX_EVP_SKEYMGMT_fetch() returns a pointer to the key management
implementation represented by an EVP_SKEYMGMT object, or NULL on
error.

OPENSSL_BOX_EVP_SKEYMGMT_up_ref() returns 1 on success, or 0 on error.

OPENSSL_BOX_EVP_SKEYMGMT_names_do_all() returns 1 if the callback was called for all
names. A return value of 0 means that the callback was not called for any names.

OPENSSL_BOX_EVP_SKEYMGMT_free() doesn't return any value.

OPENSSL_BOX_EVP_SKEYMGMT_get0_provider() returns a pointer to a provider object, or NULL
on error.

OPENSSL_BOX_EVP_SKEYMGMT_is_a() returns 1 if I<skeymgmt> was identifiable, otherwise 0.

OPENSSL_BOX_EVP_SKEYMGMT_get0_name() returns the algorithm name, or NULL on error.

OPENSSL_BOX_EVP_SKEYMGMT_get0_description() returns a pointer to a description, or NULL if
there isn't one.

=head1 SEE ALSO

L<EVP_SKEY(3)>, L<OPENSSL_BOX_EVP_MD_fetch(3)>, L<OSSL_LIB_CTX(3)>
