...
OPENSSL_BOX_EVP_PKEY_gettable_params, OPENSSL_BOX_EVP_PKEY_get_params,
OPENSSL_BOX_EVP_PKEY_get_int_param, OPENSSL_BOX_EVP_PKEY_get_size_t_param,
OPENSSL_BOX_EVP_PKEY_get_bn_param, OPENSSL_BOX_EVP_PKEY_get_utf8_string_param,
OPENSSL_BOX_EVP_PKEY_get_octet_string_param
...
=head1 SYNOPSIS
 ...
 const OSSL_PARAM *OPENSSL_BOX_EVP_PKEY_gettable_params(EVP_PKEY *pkey);
...
OPENSSL_BOX_EVP_PKEY_get_params() retrieves parameters from the key I<pkey>, according to
the contents of I<params>.

OPENSSL_BOX_EVP_PKEY_gettable_params() returns a constant list of I<params> indicating
...
obtained using any of the functions OPENSSL_BOX_EVP_PKEY_get_int_param(),
OPENSSL_BOX_EVP_PKEY_get_size_t_param() or OPENSSL_BOX_EVP_PKEY_get_bn_param(). Attempting to
obtain an integer value that does not fit into a native C B<int> type will cause
OPENSSL_BOX_EVP_PKEY_get_int_param() to fail. Similarly attempting to obtain an integer
value that is negative or does not fit into a native C B<size_t> type using
OPENSSL_BOX_EVP_PKEY_get_size_t_param() will also fail.

OPENSSL_BOX_EVP_PKEY_get_int_param() retrieves a key I<pkey> integer value I<*out>
associated with a name of I<key_name> if it fits into C<int> type. For
parameters that do not fit into C<int> use OPENSSL_BOX_EVP_PKEY_get_bn_param().

OPENSSL_BOX_EVP_PKEY_get_size_t_param() retrieves a key I<pkey> size_t value I<*out>
associated with a name of I<key_name> if it fits into C<size_t> type. For
parameters that do not fit into C<size_t> use OPENSSL_BOX_EVP_PKEY_get_bn_param().

OPENSSL_BOX_EVP_PKEY_get_bn_param() retrieves a key I<pkey> BIGNUM value I<**bn>
associated with a name of I<key_name>. If I<*bn> is NULL then the BIGNUM
is allocated by the method.

OPENSSL_BOX_EVP_PKEY_get_utf8_string_param() get a key I<pkey> UTF8 string value into a
...
OPENSSL_BOX_EVP_PKEY_get_octet_string_param() get a key I<pkey>'s octet string value into a
...
OPENSSL_BOX_EVP_PKEY_gettable_params() returns NULL on error or if it is not supported.
...
An error may be returned by methods OPENSSL_BOX_EVP_PKEY_get_utf8_string_param() and
OPENSSL_BOX_EVP_PKEY_get_octet_string_param() if I<max_buf_sz> is not big enough to hold the
...
=head1 EXAMPLES
 ...
 if (!OPENSSL_BOX_EVP_PKEY_get_octet_string_param(key, OSSL_PKEY_PARAM_PUB_KEY,
                                      pub, sizeof(pub), &len)) {
     /* Error */
 }
 if (!OPENSSL_BOX_EVP_PKEY_get_bn_param(key, OSSL_PKEY_PARAM_PRIV_KEY, &bn_priv)) {
     /* Error */
 }
...
L<OPENSSL_BOX_EVP_PKEY_CTX_new(3)>, L<provider-keymgmt(7)>, L<OSSL_PARAM(3)>
