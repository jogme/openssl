...
OPENSSL_BOX_EVP_PKEY_decapsulate_init, OPENSSL_BOX_EVP_PKEY_auth_decapsulate_init, OPENSSL_BOX_EVP_PKEY_decapsulate
...
The OPENSSL_BOX_EVP_PKEY_decapsulate_init() function initializes a private key algorithm
...
Note that I<ctx> usually is produced using L<OPENSSL_BOX_EVP_PKEY_CTX_new_from_pkey(3)>,
specifying the private key to use.

The OPENSSL_BOX_EVP_PKEY_auth_decapsulate_init() function is similar to
OPENSSL_BOX_EVP_PKEY_decapsulate_init() but also passes an I<authpub> authentication public
key that is used during decapsulation.

The OPENSSL_BOX_EVP_PKEY_decapsulate() function performs a private key decapsulation
...
After the call to OPENSSL_BOX_EVP_PKEY_decapsulate_init() algorithm-specific parameters
for the operation may be set or modified using L<OPENSSL_BOX_EVP_PKEY_CTX_set_params(3)>.

=head1 RETURN VALUES

OPENSSL_BOX_EVP_PKEY_decapsulate_init(), OPENSSL_BOX_EVP_PKEY_auth_decapsulate_init() and
OPENSSL_BOX_EVP_PKEY_decapsulate() return 1 for success and 0 or a negative value for
...
Decapsulate data using RSA:
 ...
 ctx = OPENSSL_BOX_EVP_PKEY_CTX_new_from_pkey(libctx, rsa_priv_key, NULL);
 if (ctx == NULL)
     /* Error */
 ...
 if (OPENSSL_BOX_EVP_PKEY_decapsulate(ctx, NULL, &secretlen, in, inlen) <= 0)
     /* Error */
 ...
 if (OPENSSL_BOX_EVP_PKEY_decapsulate(ctx, secret, &secretlen, in, inlen) <= 0)
     /* Error */
...
The functions OPENSSL_BOX_EVP_PKEY_decapsulate_init() and OPENSSL_BOX_EVP_PKEY_decapsulate() were added
in OpenSSL 3.0.

The function OPENSSL_BOX_EVP_PKEY_auth_decapsulate_init() was added in OpenSSL 3.2.
