...
OPENSSL_BOX_EVP_DigestSignInit_ex, OPENSSL_BOX_EVP_DigestSignInit, OPENSSL_BOX_EVP_DigestSignUpdate,
OPENSSL_BOX_EVP_DigestSignFinal, OPENSSL_BOX_EVP_DigestSign - EVP signing functions
...
=head1 SYNOPSIS
 ...
 int OPENSSL_BOX_EVP_DigestSignUpdate(EVP_MD_CTX *ctx, const void *d, size_t cnt);
 int OPENSSL_BOX_EVP_DigestSignFinal(EVP_MD_CTX *ctx, unsigned char *sig, size_t *siglen);

 int OPENSSL_BOX_EVP_DigestSign(EVP_MD_CTX *ctx, unsigned char *sig,
...
OPENSSL_BOX_EVP_DigestSignInit_ex() sets up signing context I<ctx> to use a digest
...
I<ctx> must be created with OPENSSL_BOX_EVP_MD_CTX_new() before calling this function. If
...
EVP_PKEY_CTX value before being passed to OPENSSL_BOX_EVP_DigestSignInit_ex()
(which means the EVP_PKEY_CTX is created inside OPENSSL_BOX_EVP_DigestSignInit_ex()
and it will be freed automatically when the EVP_MD_CTX is freed). If the
EVP_PKEY_CTX to be used is created by OPENSSL_BOX_EVP_DigestSignInit_ex then it
...
No B<EVP_PKEY_CTX> will be created by OPENSSL_BOX_EVP_DigestSignInit_ex() if the
passed I<ctx> has already been assigned one via L<OPENSSL_BOX_EVP_MD_CTX_set_pkey_ctx(3)>.
See also L<SM2(7)>.
...
OPENSSL_BOX_EVP_DigestSignInit() works in the same way as OPENSSL_BOX_EVP_DigestSignInit_ex()
...
OPENSSL_BOX_EVP_DigestSignUpdate() hashes I<cnt> bytes of data at I<d> into the
signature context I<ctx>. This function can be called several times on the
same I<ctx> to include additional data. I<ctx> B<MUST NOT> be NULL.

Unless I<sig> is NULL OPENSSL_BOX_EVP_DigestSignFinal() signs the data in I<ctx>
...
OPENSSL_BOX_EVP_DigestSign() is similar to a single call to OPENSSL_BOX_EVP_DigestSignUpdate() and
OPENSSL_BOX_EVP_DigestSignFinal().
Unless I<sig> is NULL, OPENSSL_BOX_EVP_DigestSign() signs the data I<tbs> of length I<tbslen>
...
OPENSSL_BOX_EVP_DigestSignInit(), OPENSSL_BOX_EVP_DigestSignUpdate(), OPENSSL_BOX_EVP_DigestSignFinal() and
OPENSSL_BOX_EVP_DigestSign() return 1 for success and 0 for failure.
...
OPENSSL_BOX_EVP_DigestSign() is a one shot operation which signs a single block of data
in one function. For algorithms that support streaming it is equivalent to
calling OPENSSL_BOX_EVP_DigestSignUpdate() and OPENSSL_BOX_EVP_DigestSignFinal(). For algorithms which
...
The call to OPENSSL_BOX_EVP_DigestSignFinal() internally finalizes a copy of the digest
context. This means that calls to OPENSSL_BOX_EVP_DigestSignUpdate() and
OPENSSL_BOX_EVP_DigestSignFinal() can be called later to digest and sign additional data.
Applications may disable this behavior by setting the EVP_MD_CTX_FLAG_FINALISE
context flag via L<OPENSSL_BOX_EVP_MD_CTX_set_flags(3)>.

Note that not all providers support continuation, in case the selected
provider does not allow to duplicate contexts OPENSSL_BOX_EVP_DigestSignFinal() will
finalize the digest context and attempting to process additional data via
OPENSSL_BOX_EVP_DigestSignUpdate() will result in an error.

OPENSSL_BOX_EVP_DigestSignInit() and OPENSSL_BOX_EVP_DigestSignInit_ex() functions can be called
...
OPENSSL_BOX_EVP_DigestSign() can not be called again, once a signature is generated (by
passing I<sig> as non NULL), unless the B<EVP_MD_CTX> is reinitialised by
calling OPENSSL_BOX_EVP_DigestSignInit_ex().

Ignoring failure returns of OPENSSL_BOX_EVP_DigestSignInit() and OPENSSL_BOX_EVP_DigestSignInit_ex()
functions can lead to subsequent undefined behavior when calling
OPENSSL_BOX_EVP_DigestSignUpdate(), OPENSSL_BOX_EVP_DigestSignFinal(), or OPENSSL_BOX_EVP_DigestSign().

The use of OPENSSL_BOX_EVP_PKEY_get_size() with these functions is discouraged because some
signature operations may have a signature length which depends on the
parameters set. As a result OPENSSL_BOX_EVP_PKEY_get_size() would have to return a value
which indicates the maximum possible signature for any set of parameters.

=head1 SEE ALSO

L<OPENSSL_BOX_EVP_DigestVerifyInit(3)>,
L<OPENSSL_BOX_EVP_DigestInit(3)>,
...
OPENSSL_BOX_EVP_DigestSignInit(), OPENSSL_BOX_EVP_DigestSignUpdate() and OPENSSL_BOX_EVP_DigestSignFinal()
were added in OpenSSL 1.0.0.

OPENSSL_BOX_EVP_DigestSignInit_ex() was added in OpenSSL 3.0.

OPENSSL_BOX_EVP_DigestSignUpdate() was converted from a macro to a function in OpenSSL 3.0.
