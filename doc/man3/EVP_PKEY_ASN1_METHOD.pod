...
OPENSSL_BOX_EVP_PKEY_asn1_new,
OPENSSL_BOX_EVP_PKEY_asn1_copy,
OPENSSL_BOX_EVP_PKEY_asn1_free,
OPENSSL_BOX_EVP_PKEY_asn1_add0,
OPENSSL_BOX_EVP_PKEY_asn1_add_alias,
OPENSSL_BOX_EVP_PKEY_asn1_set_public,
OPENSSL_BOX_EVP_PKEY_asn1_set_private,
OPENSSL_BOX_EVP_PKEY_asn1_set_param,
OPENSSL_BOX_EVP_PKEY_asn1_set_free,
OPENSSL_BOX_EVP_PKEY_asn1_set_ctrl,
OPENSSL_BOX_EVP_PKEY_asn1_set_item,
OPENSSL_BOX_EVP_PKEY_asn1_set_siginf,
OPENSSL_BOX_EVP_PKEY_asn1_set_check,
OPENSSL_BOX_EVP_PKEY_asn1_set_public_check,
OPENSSL_BOX_EVP_PKEY_asn1_set_param_check,
OPENSSL_BOX_EVP_PKEY_asn1_set_security_bits,
OPENSSL_BOX_EVP_PKEY_asn1_set_set_priv_key,
OPENSSL_BOX_EVP_PKEY_asn1_set_set_pub_key,
OPENSSL_BOX_EVP_PKEY_asn1_set_get_priv_key,
OPENSSL_BOX_EVP_PKEY_asn1_set_get_pub_key,
OPENSSL_BOX_EVP_PKEY_get0_asn1
...
L<OPENSSL_BOX_EVP_PKEY_asn1_add0(3)>.
...
It's called by L<OPENSSL_BOX_EVP_PKEY_eq(3)>.
...
It's called by L<OPENSSL_BOX_EVP_PKEY_print_public(3)>.
...
readable text to B<out>, indented B<indent> spaces.
It MUST return 0 on error, 1 on success.
It's called by L<OPENSSL_BOX_EVP_PKEY_print_private(3)>.
...
It's called by L<OPENSSL_BOX_EVP_PKEY_get_size(3)>.

The pkey_bits() method returns the key size in bits.
It's called by L<OPENSSL_BOX_EVP_PKEY_get_bits(3)>.
...
otherwise 1.
It's called by L<OPENSSL_BOX_EVP_PKEY_missing_parameters(3)>.

The param_copy() method copies key parameters from B<from> to B<to>.
It MUST return 0 on error, 1 on success.
It's called by L<OPENSSL_BOX_EVP_PKEY_copy_parameters(3)>.
...
It's called by L<OPENSSL_BOX_EVP_PKEY_parameters_eq(3)>.

The param_print() method prints the private key parameters in humanly
readable text to B<out>, indented B<indent> spaces.
It MUST return 0 on error, 1 on success.
It's called by L<OPENSSL_BOX_EVP_PKEY_print_params(3)>.
...
It's called by L<OPENSSL_BOX_EVP_PKEY_free(3)>, L<OPENSSL_BOX_EVP_PKEY_set_type(3)>,
L<OPENSSL_BOX_EVP_PKEY_set_type_str(3)>, and L<OPENSSL_BOX_EVP_PKEY_assign(3)>.

 int (*pkey_ctrl) (EVP_PKEY *pkey, int op, long arg1, void *arg2);

The pkey_ctrl() method adds extra algorithm specific control.
It's called by L<OPENSSL_BOX_EVP_PKEY_get_default_digest_nid(3)>,
L<EVP_PKEY_set1_encoded_public_key(3)>,
L<OPENSSL_BOX_EVP_PKEY_get1_encoded_public_key(3)>, L<PKCS7_SIGNER_INFO_set(3)>,
...
OPENSSL_BOX_i2d_PrivateKey().
They're called by L<OPENSSL_BOX_d2i_PrivateKey(3)> and L<OPENSSL_BOX_i2d_PrivateKey(3)>.
...
They are called by L<OPENSSL_BOX_EVP_PKEY_check(3)>, L<OPENSSL_BOX_EVP_PKEY_public_check(3)> and
...
They are called by L<OPENSSL_BOX_EVP_PKEY_new_raw_private_key(3)>, and
L<OPENSSL_BOX_EVP_PKEY_new_raw_public_key(3)> respectively.
...
OPENSSL_BOX_EVP_PKEY_asn1_new() creates and returns a new B<EVP_PKEY_ASN1_METHOD>
...
OPENSSL_BOX_EVP_PKEY_asn1_copy() copies an B<EVP_PKEY_ASN1_METHOD> object from
B<src> to B<dst>.
This function is not thread safe, it's recommended to only use this
when initializing the application.

OPENSSL_BOX_EVP_PKEY_asn1_free() frees an existing B<EVP_PKEY_ASN1_METHOD> pointed
by B<ameth>. If the argument is NULL, nothing is done.

OPENSSL_BOX_EVP_PKEY_asn1_add0() adds B<ameth> to the user defined stack of
methods unless another B<EVP_PKEY_ASN1_METHOD> with the same NID is
already there.
This function is not thread safe, it's recommended to only use this
when initializing the application.

OPENSSL_BOX_EVP_PKEY_asn1_add_alias() creates an alias with the NID B<to> for the
B<EVP_PKEY_ASN1_METHOD> with NID B<from> unless another
B<EVP_PKEY_ASN1_METHOD> with the same NID is already added.
This function is not thread safe, it's recommended to only use this
when initializing the application.

OPENSSL_BOX_EVP_PKEY_asn1_set_public(), OPENSSL_BOX_EVP_PKEY_asn1_set_private(),
OPENSSL_BOX_EVP_PKEY_asn1_set_param(), OPENSSL_BOX_EVP_PKEY_asn1_set_free(),
OPENSSL_BOX_EVP_PKEY_asn1_set_ctrl(), OPENSSL_BOX_EVP_PKEY_asn1_set_item(),
OPENSSL_BOX_EVP_PKEY_asn1_set_siginf(), OPENSSL_BOX_EVP_PKEY_asn1_set_check(),
OPENSSL_BOX_EVP_PKEY_asn1_set_public_check(), OPENSSL_BOX_EVP_PKEY_asn1_set_param_check(),
OPENSSL_BOX_EVP_PKEY_asn1_set_security_bits(), OPENSSL_BOX_EVP_PKEY_asn1_set_set_priv_key(),
OPENSSL_BOX_EVP_PKEY_asn1_set_set_pub_key(), OPENSSL_BOX_EVP_PKEY_asn1_set_get_priv_key() and
OPENSSL_BOX_EVP_PKEY_asn1_set_get_pub_key() set the diverse methods of the given
B<EVP_PKEY_ASN1_METHOD> object.

OPENSSL_BOX_EVP_PKEY_get0_asn1() finds the B<EVP_PKEY_ASN1_METHOD> associated
with the key B<pkey>.

=head1 RETURN VALUES

OPENSSL_BOX_EVP_PKEY_asn1_new() returns NULL on error, or a pointer to an
B<EVP_PKEY_ASN1_METHOD> object otherwise.

OPENSSL_BOX_EVP_PKEY_asn1_add0() and OPENSSL_BOX_EVP_PKEY_asn1_add_alias() return 0 on error,
or 1 on success.

OPENSSL_BOX_EVP_PKEY_get0_asn1() returns NULL on error, or a pointer to a constant
B<EVP_PKEY_ASN1_METHOD> object otherwise.

=head1 HISTORY

The signature of the I<pub_decode> functional argument of
OPENSSL_BOX_EVP_PKEY_asn1_set_public() has changed in OpenSSL 3.0 so its I<pub>
