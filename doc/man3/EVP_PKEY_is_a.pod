...
OPENSSL_BOX_EVP_PKEY_is_a, OPENSSL_BOX_EVP_PKEY_can_sign, EVP_PKEY_type_names_do_all,
OPENSSL_BOX_EVP_PKEY_get0_type_name, OPENSSL_BOX_EVP_PKEY_get0_description, OPENSSL_BOX_EVP_PKEY_get0_provider
...
=head1 SYNOPSIS
 ...
 const char *OPENSSL_BOX_EVP_PKEY_get0_description(const EVP_PKEY *key);
 const OSSL_PROVIDER *OPENSSL_BOX_EVP_PKEY_get0_provider(const EVP_PKEY *key);

=head1 DESCRIPTION

OPENSSL_BOX_EVP_PKEY_is_a() checks if the key type of I<pkey> is I<name>.

OPENSSL_BOX_EVP_PKEY_can_sign() checks if the functionality for the key type of
...
OPENSSL_BOX_EVP_PKEY_get0_type_name() returns the first key type name that is found
...
OPENSSL_BOX_EVP_PKEY_get0_description() returns a description of the type of B<EVP_PKEY>,
meant for display and human consumption.  The description is at the
discretion of the key type implementation.

OPENSSL_BOX_EVP_PKEY_get0_provider() returns the provider of the B<EVP_PKEY>'s
L<EVP_KEYMGMT(3)>.

=head1 RETURN VALUES

OPENSSL_BOX_EVP_PKEY_is_a() returns 1 if I<pkey> has the key type I<name>,
otherwise 0.

OPENSSL_BOX_EVP_PKEY_can_sign() returns 1 if the I<pkey> key type functionality
supports signing, otherwise 0.

OPENSSL_BOX_EVP_PKEY_get0_type_name() returns the name that is found or NULL on error.

OPENSSL_BOX_EVP_PKEY_get0_description() returns the description if found or NULL if not.

OPENSSL_BOX_EVP_PKEY_get0_provider() returns the provider if found or NULL if not.
...
=head2 OPENSSL_BOX_EVP_PKEY_is_a()

The loaded providers and what key types they support will ultimately
determine what I<name> is possible to use with OPENSSL_BOX_EVP_PKEY_is_a().  We do know
...
=head2 OPENSSL_BOX_EVP_PKEY_can_sign()

 #include <openssl/evp.h>

 ...
     /* |pkey| is an EVP_PKEY* */
     if (!OPENSSL_BOX_EVP_PKEY_can_sign(pkey)) {
