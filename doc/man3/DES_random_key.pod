...
instead use L<OPENSSL_BOX_EVP_EncryptInit_ex(3)>, L<OPENSSL_BOX_EVP_EncryptUpdate(3)> and
L<OPENSSL_BOX_EVP_EncryptFinal_ex(3)> or the equivalently named decrypt functions.
...
L<OPENSSL_BOX_EVP_DigestInit(3)> etc. instead.
...
L<OPENSSL_BOX_EVP_EncryptInit(3)> etc. instead of calling these
functions directly.
