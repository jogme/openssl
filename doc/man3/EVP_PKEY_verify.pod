...
OPENSSL_BOX_EVP_PKEY_verify_init, OPENSSL_BOX_EVP_PKEY_verify_init_ex, OPENSSL_BOX_EVP_PKEY_verify_init_ex2,
OPENSSL_BOX_EVP_PKEY_verify, OPENSSL_BOX_EVP_PKEY_verify_message_init, OPENSSL_BOX_EVP_PKEY_verify_message_update,
OPENSSL_BOX_EVP_PKEY_verify_message_final, OPENSSL_BOX_EVP_PKEY_CTX_set_signature - signature
...
=head1 SYNOPSIS
 ...
 int OPENSSL_BOX_EVP_PKEY_verify_init_ex2(EVP_PKEY_CTX *ctx, EVP_SIGNATURE *algo,
                              const OSSL_PARAM params[]);
 int OPENSSL_BOX_EVP_PKEY_verify_message_init(EVP_PKEY_CTX *ctx, EVP_SIGNATURE *algo,
                                  const OSSL_PARAM params[]);
...
OPENSSL_BOX_EVP_PKEY_verify_init() initializes a public key algorithm context I<ctx> for
verification using the algorithm given when the context was created
using L<OPENSSL_BOX_EVP_PKEY_CTX_new(3)> or variants thereof.  The algorithm is used to
fetch a B<EVP_SIGNATURE> method implicitly, see L<provider(7)/Implicit fetch>
for more information about implicit fetches.

OPENSSL_BOX_EVP_PKEY_verify_init_ex() is the same as OPENSSL_BOX_EVP_PKEY_verify_init() but additionally
sets the passed parameters I<params> on the context before returning.

OPENSSL_BOX_EVP_PKEY_verify_init_ex2() is the same as OPENSSL_BOX_EVP_PKEY_verify_init_ex(), but works
...
treatment of the input to OPENSSL_BOX_EVP_PKEY_verify() may be pre-determined, and in that
case, those details may normally not be changed.
See L</NOTES> below for a deeper explanation.

OPENSSL_BOX_EVP_PKEY_verify_message_init() initializes a public key algorithm context
I<ctx> for verifying an unlimited size message using the algorithm given by
I<algo> and the key given through L<OPENSSL_BOX_EVP_PKEY_CTX_new(3)> or
L<OPENSSL_BOX_EVP_PKEY_CTX_new_from_pkey(3)>.
Passing the message is supported both in a one-shot fashion using
OPENSSL_BOX_EVP_PKEY_verify(), and through the combination of
OPENSSL_BOX_EVP_PKEY_verify_message_update() and OPENSSL_BOX_EVP_PKEY_verify_message_final().
This function enables using algorithms that can process input of arbitrary
length, such as ED25519, RSA-SHA256 and similar.

OPENSSL_BOX_EVP_PKEY_CTX_set_signature() specifies the I<siglen> bytes long signature
I<sig> to be verified against by OPENSSL_BOX_EVP_PKEY_verify_message_final().
It I<must> be used together with OPENSSL_BOX_EVP_PKEY_verify_message_update() and
OPENSSL_BOX_EVP_PKEY_verify_message_final().
See L</NOTES> below for a deeper explanation.

OPENSSL_BOX_EVP_PKEY_verify_message_update() adds I<inlen> bytes from I<in> to the data
...
OPENSSL_BOX_EVP_PKEY_verify_message_final() verifies the processed data, given only I<ctx>.
The signature to verify against must have been given with
OPENSSL_BOX_EVP_PKEY_CTX_set_signature().

OPENSSL_BOX_EVP_PKEY_verify() is a one-shot function that performs the same thing as
OPENSSL_BOX_EVP_PKEY_CTX_set_signature() call with I<sig> and I<siglen> as parameters,
followed by a single OPENSSL_BOX_EVP_PKEY_verify_message_update() call with I<tbs> and
I<tbslen>, followed by OPENSSL_BOX_EVP_PKEY_verify_message_final() call.

=head1 NOTES

=begin comment

These notes are largely replicated in OPENSSL_BOX_EVP_PKEY_sign.pod, please keep them
in sync.
...
with OPENSSL_BOX_EVP_PKEY_verify_init_ex2() or with OPENSSL_BOX_EVP_PKEY_verify_message_init().
...
with, as well as the padding mode (see L<OPENSSL_BOX_EVP_PKEY_CTX_set_signature_md(3)> and
...
particular, neither L<OPENSSL_BOX_EVP_PKEY_CTX_set_signature_md(3)> nor the B<OSSL_PARAM>
...
desired, please use L<OPENSSL_BOX_EVP_DigestVerifyInit(3)> and associated functions, or
L<EVP_VerifyInit(3)> and associated functions.

=head2 Performing multiple verifications

When initialized using OPENSSL_BOX_EVP_PKEY_verify_init_ex() or  OPENSSL_BOX_EVP_PKEY_verify_init_ex2(),
OPENSSL_BOX_EVP_PKEY_verify() can be called more than once on the same context to have
several one-shot operations performed using the same parameters.

When initialized using OPENSSL_BOX_EVP_PKEY_verify_message_init(), it's not possible to
call OPENSSL_BOX_EVP_PKEY_verify() multiple times.

=head2 On OPENSSL_BOX_EVP_PKEY_CTX_set_signature()
...
using OPENSSL_BOX_EVP_PKEY_verify_message_update() and OPENSSL_BOX_EVP_PKEY_verify_message_final() to
perform the verification.

=head1 RETURN VALUES

All functions return 1 for success and 0 or a negative value for failure.
However, unlike other functions, the return value 0 from OPENSSL_BOX_EVP_PKEY_verify(),
OPENSSL_BOX_EVP_PKEY_verify_recover() and OPENSSL_BOX_EVP_PKEY_verify_message_final() only indicates
...
These examples are largely replicated in OPENSSL_BOX_EVP_PKEY_sign.pod, please keep them
in sync.
...
Verify signature using PKCS#1 padding and a SHA256 digest as input:
  ...
  */
 ctx = OPENSSL_BOX_EVP_PKEY_CTX_new(verify_key, NULL /* no engine */);
 if (ctx == NULL)
     /* Error occurred */
 ...
 if (EVP_PKEY_CTX_set_rsa_padding(ctx, RSA_PKCS1_PADDING) <= 0)
     /* Error */
 if (OPENSSL_BOX_EVP_PKEY_CTX_set_signature_md(ctx, OPENSSL_BOX_EVP_sha256()) <= 0)
     /* Error */

 /* Perform operation */
 ret = OPENSSL_BOX_EVP_PKEY_verify(ctx, sig, siglen, md, mdlen);
...
input digest is assumed to have been computed using SHA256.
  ...
  * and that verify_key is an RSA public key
  */
 ctx = OPENSSL_BOX_EVP_PKEY_CTX_new(signing_key, NULL /* no engine */);
 alg = OPENSSL_BOX_EVP_SIGNATURE_fetch(NULL, "RSA-SHA256", NULL);

 if (ctx == NULL)
     /* Error occurred */
 if (OPENSSL_BOX_EVP_PKEY_verify_init_ex2(ctx, alg, NULL) <= 0)
     /* Error */

 /* Determine buffer length */
 if (OPENSSL_BOX_EVP_PKEY_verify(ctx, sig, siglen, md, mdlen) <= 0)
...
B<RSA_PKCS1_PADDING>.
 ...
 if (ctx == NULL || alg == NULL)
     /* Error occurred */
 if (OPENSSL_BOX_EVP_PKEY_verify_message_init(ctx, alg, NULL) <= 0)
     /* Error */

 /* Perform operation */
 ret = OPENSSL_BOX_EVP_PKEY_verify(ctx, sig, siglen, in, inlen);
...
functionality.
 ...
 OPENSSL_BOX_EVP_PKEY_CTX_set_signature(ctx, sig, siglen);

 /* Perform operation */
 while (inlen > 0) {
     if (OPENSSL_BOX_EVP_PKEY_verify_message_update(ctx, in, inlen)) <= 0)
         /* Error */
     if (inlen > 256) {
         inlen -= 256;
         in += 256;
 ...
 ret = OPENSSL_BOX_EVP_PKEY_verify_message_final(ctx);
...
L<OPENSSL_BOX_EVP_PKEY_CTX_new(3)>,
L<OPENSSL_BOX_EVP_PKEY_encrypt(3)>,
L<OPENSSL_BOX_EVP_PKEY_decrypt(3)>,
L<OPENSSL_BOX_EVP_PKEY_sign(3)>,
L<OPENSSL_BOX_EVP_PKEY_verify_recover(3)>,
L<OPENSSL_BOX_EVP_PKEY_derive(3)>

=head1 HISTORY

The OPENSSL_BOX_EVP_PKEY_verify_init() and OPENSSL_BOX_EVP_PKEY_verify() functions were added in
OpenSSL 1.0.0.

The OPENSSL_BOX_EVP_PKEY_verify_init_ex() function was added in OpenSSL 3.0.

The OPENSSL_BOX_EVP_PKEY_verify_init_ex2(), OPENSSL_BOX_EVP_PKEY_verify_message_init(),
OPENSSL_BOX_EVP_PKEY_verify_message_update(), OPENSSL_BOX_EVP_PKEY_verify_message_final() and
OPENSSL_BOX_EVP_PKEY_CTX_set_signature() functions where added in OpenSSL 3.4.
