...
initialised with L<OPENSSL_BOX_EVP_CIPHER_CTX_reset(3)> and L<OPENSSL_BOX_EVP_MAC_CTX_new(3)>
respectively.
...
set using L<OPENSSL_BOX_EVP_EncryptInit_ex(3)>. The hmac context and digest can be set using
L<OPENSSL_BOX_EVP_MAC_CTX_set_params(3)> with the B<OSSL_MAC_PARAM_KEY> and
...
I<iv>. using a function like L<OPENSSL_BOX_EVP_DecryptInit_ex(3)>. The key material and
digest for I<hctx> need to be set using L<OPENSSL_BOX_EVP_MAC_CTX_set_params(3)> with the
...
initialised with L<OPENSSL_BOX_EVP_MAC_CTX_new(3)> and the digest set with
L<OPENSSL_BOX_EVP_MAC_CTX_set_params(3)>.
...
Reference Implementation:
                                     ...
                                     EVP_MAC_CTX *hctx, int enc)
 {
     ...
     if (enc) { /* create new session */
         ...
         if (OPENSSL_BOX_EVP_EncryptInit_ex(&ctx, OPENSSL_BOX_EVP_aes_256_cbc(), NULL, key->aes_key,
                                iv) == 0)
         ...
         if (OPENSSL_BOX_EVP_MAC_CTX_set_params(hctx, params) == 0)
         ...
         if (OPENSSL_BOX_EVP_DecryptInit_ex(&ctx, OPENSSL_BOX_EVP_aes_256_cbc(), NULL, key->aes_key,
                                iv) == 0)
            return -1; /* error in cipher initialisation */
