...
OPENSSL_BOX_EVP_PKEY_CTX_ctrl,
OPENSSL_BOX_EVP_PKEY_CTX_ctrl_str,
OPENSSL_BOX_EVP_PKEY_CTX_ctrl_uint64,
OPENSSL_BOX_EVP_PKEY_CTX_md,
OPENSSL_BOX_EVP_PKEY_CTX_set_signature_md,
OPENSSL_BOX_EVP_PKEY_CTX_get_signature_md,
EVP_PKEY_CTX_set_mac_key,
OPENSSL_BOX_EVP_PKEY_CTX_set_group_name,
OPENSSL_BOX_EVP_PKEY_CTX_get_group_name,
...
OPENSSL_BOX_EVP_PKEY_CTX_set1_id, OPENSSL_BOX_EVP_PKEY_CTX_get1_id, OPENSSL_BOX_EVP_PKEY_CTX_get1_id_len,
OPENSSL_BOX_EVP_PKEY_CTX_set_kem_op
...
=head1 SYNOPSIS
 ...
 int OPENSSL_BOX_EVP_PKEY_CTX_get_signature_md(EVP_PKEY_CTX *ctx, const EVP_MD **pmd);
 ...
 int OPENSSL_BOX_EVP_PKEY_CTX_set1_id(EVP_PKEY_CTX *ctx, void *id, size_t id_len);
...
OPENSSL_BOX_EVP_PKEY_CTX_ctrl() sends a control operation to the context I<ctx>. The key
...
Applications will not normally call OPENSSL_BOX_EVP_PKEY_CTX_ctrl() directly but will
instead call one of the algorithm specific functions below.

OPENSSL_BOX_EVP_PKEY_CTX_ctrl_uint64() is a wrapper that directly passes a
uint64 value as I<p2> to OPENSSL_BOX_EVP_PKEY_CTX_ctrl().

OPENSSL_BOX_EVP_PKEY_CTX_ctrl_str() allows an application to send an algorithm
...
OPENSSL_BOX_EVP_PKEY_CTX_md() sends a message digest control operation to the context
I<ctx>. The message digest is specified by its name I<md>.

OPENSSL_BOX_EVP_PKEY_CTX_set_signature_md() sets the message digest type used
in a signature. It can be used in the RSA, DSA and ECDSA algorithms.

OPENSSL_BOX_EVP_PKEY_CTX_get_signature_md()gets the message digest type used
in a signature. It can be used in the RSA, DSA and ECDSA algorithms.
...
Normally applications would call L<OPENSSL_BOX_EVP_PKEY_new_raw_private_key(3)> or similar
functions instead.

EVP_PKEY_CTX_set_mac_key() can be used with any of the algorithms supported by
the L<OPENSSL_BOX_EVP_PKEY_new_raw_private_key(3)> function.

OPENSSL_BOX_EVP_PKEY_CTX_set_group_name() sets the group name to I<name> for parameter and
...
OPENSSL_BOX_EVP_PKEY_CTX_get_group_name() finds the group name that's currently
...
Two RSA padding modes behave differently if OPENSSL_BOX_EVP_PKEY_CTX_set_signature_md()
...
L<OPENSSL_BOX_EVP_PKEY_CTX_set_params(3)> function. These are
...
OPENSSL_BOX_EVP_PKEY_derive() is the output of the KDF instead of the DH shared secret.
...
Use OPENSSL_BOX_EVP_PKEY_CTX_set_group_name() (described above) to set the curve name to
I<name> for parameter and key generation.

EVP_PKEY_CTX_set_ec_paramgen_curve_nid() does the same as
OPENSSL_BOX_EVP_PKEY_CTX_set_group_name(), but is specific to EC and uses a I<nid> rather
than a name string.

For EC parameter generation, one of OPENSSL_BOX_EVP_PKEY_CTX_set_group_name()
...
OPENSSL_BOX_EVP_PKEY_CTX_get_group_name() (described above) can be used to obtain the curve
...
OPENSSL_BOX_EVP_PKEY_CTX_set1_id(), OPENSSL_BOX_EVP_PKEY_CTX_get1_id() and OPENSSL_BOX_EVP_PKEY_CTX_get1_id_len()
are used to manipulate the special identifier field for specific signature
algorithms such as SM2. The OPENSSL_BOX_EVP_PKEY_CTX_set1_id() sets an ID pointed by I<id> with
...
OPENSSL_BOX_EVP_PKEY_CTX_get1_id_len() returns the length of the ID set via a previous call
to OPENSSL_BOX_EVP_PKEY_CTX_set1_id(). The length is usually used to allocate adequate
memory for further calls to OPENSSL_BOX_EVP_PKEY_CTX_get1_id(). OPENSSL_BOX_EVP_PKEY_CTX_get1_id()
returns the previously set ID value to caller in I<id>. The caller should
allocate adequate memory space for the I<id> before calling OPENSSL_BOX_EVP_PKEY_CTX_get1_id().

OPENSSL_BOX_EVP_PKEY_CTX_set_kem_op() sets the KEM operation to run. This can be set after
OPENSSL_BOX_EVP_PKEY_encapsulate_init() or OPENSSL_BOX_EVP_PKEY_decapsulate_init() to select the kem
...
OPENSSL_BOX_EVP_PKEY_encapsulate() or OPENSSL_BOX_EVP_PKEY_decapsulate().
...
L<OPENSSL_BOX_EVP_PKEY_CTX_set_params(3)>,
...
L<OPENSSL_BOX_EVP_PKEY_verify(3)>,
L<OPENSSL_BOX_EVP_PKEY_verify_recover(3)>,
L<OPENSSL_BOX_EVP_PKEY_derive(3)>,
L<OPENSSL_BOX_EVP_PKEY_keygen(3)>
L<OPENSSL_BOX_EVP_PKEY_encapsulate(3)>
L<OPENSSL_BOX_EVP_PKEY_decapsulate(3)>
...
OPENSSL_BOX_EVP_PKEY_CTX_set_group_name() and OPENSSL_BOX_EVP_PKEY_CTX_get_group_name()
were added in OpenSSL 3.0.

The OPENSSL_BOX_EVP_PKEY_CTX_set1_id(), OPENSSL_BOX_EVP_PKEY_CTX_get1_id() and
OPENSSL_BOX_EVP_PKEY_CTX_get1_id_len() macros were added in 1.1.1, other functions were
