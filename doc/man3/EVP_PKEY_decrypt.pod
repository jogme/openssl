...
OPENSSL_BOX_EVP_PKEY_decrypt_init, OPENSSL_BOX_EVP_PKEY_decrypt_init_ex,
OPENSSL_BOX_EVP_PKEY_decrypt - decrypt using a public key algorithm
...
The OPENSSL_BOX_EVP_PKEY_decrypt_init() function initializes a public key algorithm
context using key I<pkey> for a decryption operation.

The OPENSSL_BOX_EVP_PKEY_decrypt_init_ex() function initializes a public key algorithm
context using key I<pkey> for a decryption operation and sets the
algorithm specific I<params>.

The OPENSSL_BOX_EVP_PKEY_decrypt() function performs a public key decryption operation
...
After the call to OPENSSL_BOX_EVP_PKEY_decrypt_init() algorithm specific control
operations can be performed to set any appropriate parameters for the
operation.  These operations can be included in the OPENSSL_BOX_EVP_PKEY_decrypt_init_ex()
call.

The function OPENSSL_BOX_EVP_PKEY_decrypt() can be called more than once on the same
context if several operations are performed using the same parameters.

=head1 RETURN VALUES

OPENSSL_BOX_EVP_PKEY_decrypt_init(), OPENSSL_BOX_EVP_PKEY_decrypt_init_ex() and OPENSSL_BOX_EVP_PKEY_decrypt()
...
both the return value from the OPENSSL_BOX_EVP_PKEY_decrypt() and the B<outlen> provided
...
Since version 3.2.0, the OPENSSL_BOX_EVP_PKEY_decrypt() method when used with PKCS#1
...
Decrypt data using OAEP (for RSA keys):
  ...
  */
 ctx = OPENSSL_BOX_EVP_PKEY_CTX_new(key, eng);
 if (!ctx)
     /* Error occurred */
 ...
 if (OPENSSL_BOX_EVP_PKEY_decrypt(ctx, NULL, &outlen, in, inlen) <= 0)
     /* Error */
 ...
 if (OPENSSL_BOX_EVP_PKEY_decrypt(ctx, out, &outlen, in, inlen) <= 0)
     /* Error */
