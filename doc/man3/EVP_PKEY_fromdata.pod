...
OPENSSL_BOX_EVP_PKEY_fromdata_init, OPENSSL_BOX_EVP_PKEY_fromdata, OPENSSL_BOX_EVP_PKEY_fromdata_settable
...
be created with L<OPENSSL_BOX_EVP_PKEY_CTX_new_from_name(3)> or
...
OPENSSL_BOX_EVP_PKEY_fromdata_init() initializes a public key algorithm context
for creating a key or key parameters from user data.

OPENSSL_BOX_EVP_PKEY_fromdata() creates the structure to store a key or key parameters,
given data from I<params>, I<selection> and a context that's been initialized
with OPENSSL_BOX_EVP_PKEY_fromdata_init().  The result is written to I<*ppkey>.
...
OPENSSL_BOX_EVP_PKEY_fromdata_settable() gets a constant L<OSSL_PARAM(3)> array that describes
the settable parameters that can be used with OPENSSL_BOX_EVP_PKEY_fromdata().
I<selection> is described in L</Selections>.
...
This is the mirror function to L<OPENSSL_BOX_EVP_PKEY_todata(3)>.
...
OPENSSL_BOX_EVP_PKEY_fromdata_init() and OPENSSL_BOX_EVP_PKEY_fromdata() return 1 for success and 0 or
...
is the OPENSSL_BOX_EVP_PKEY_fromdata() set of functions.  In real applications,
...
=head2 Creating an RSA keypair using raw key data
 ...
 int main()
 {
     EVP_PKEY_CTX *ctx = OPENSSL_BOX_EVP_PKEY_CTX_new_from_name(NULL, "RSA", NULL);
     EVP_PKEY *pkey = NULL;
...
=head2 Creating an ECC keypair using raw key data
 ...
 int main()
 {
     ...
     ctx = OPENSSL_BOX_EVP_PKEY_CTX_new_from_name(NULL, "EC", NULL);
     if (ctx == NULL
         || params == NULL
         || OPENSSL_BOX_EVP_PKEY_fromdata_init(ctx) <= 0
         || OPENSSL_BOX_EVP_PKEY_fromdata(ctx, &pkey, EVP_PKEY_KEYPAIR, params) <= 0) {
...
=head2 Finding out params for an unknown key type
 ...
 {
     EVP_PKEY_CTX *ctx = OPENSSL_BOX_EVP_PKEY_CTX_new_from_name(NULL, argv[1], NULL);
     const OSSL_PARAM *settable_params = NULL;

     if (ctx == NULL)
        exit(1);
    settable_params = OPENSSL_BOX_EVP_PKEY_fromdata_settable(ctx, EVP_PKEY_KEYPAIR);
...
OPENSSL_BOX_EVP_PKEY_fromdata_settable() may also be used programmatically, for
...
L<OPENSSL_BOX_EVP_PKEY_gettable_params(3)>,
L<OSSL_PARAM(3)>,
L<provider(7)>,
L<EVP_PKEY-RSA(7)>,
L<EVP_PKEY-EC(7)>,
