...
OPENSSL_BOX_EVP_PKEY_encapsulate_init, OPENSSL_BOX_EVP_PKEY_auth_encapsulate_init, OPENSSL_BOX_EVP_PKEY_encapsulate
...
The OPENSSL_BOX_EVP_PKEY_encapsulate_init() function initializes a public key algorithm
context I<ctx> for an encapsulation operation and then sets the I<params>
on the context in the same way as calling L<OPENSSL_BOX_EVP_PKEY_CTX_set_params(3)>.
Note that I<ctx> is usually is produced using L<OPENSSL_BOX_EVP_PKEY_CTX_new_from_pkey(3)>,
specifying the public key to use.

The OPENSSL_BOX_EVP_PKEY_auth_encapsulate_init() function is similar to
OPENSSL_BOX_EVP_PKEY_encapsulate_init() but also passes an I<authpriv> authentication private
key that is used during encapsulation.

The OPENSSL_BOX_EVP_PKEY_encapsulate() function performs a public key encapsulation
...
L<OPENSSL_BOX_EVP_PKEY_decapsulate(3)> to securely recover the same shared secret.
...
After the call to OPENSSL_BOX_EVP_PKEY_encapsulate_init(), algorithm-specific parameters
for the operation may be set or modified using L<OPENSSL_BOX_EVP_PKEY_CTX_set_params(3)>.

=head1 RETURN VALUES

OPENSSL_BOX_EVP_PKEY_encapsulate_init(), OPENSSL_BOX_EVP_PKEY_auth_encapsulate_init() and
OPENSSL_BOX_EVP_PKEY_encapsulate() return 1 for success and 0 or a negative value for
...
Encapsulate an RSASVE key (for RSA keys).
 ...
 ctx = OPENSSL_BOX_EVP_PKEY_CTX_new_from_pkey(libctx, rsa_pub_key, NULL);
 if (ctx == NULL)
     /* Error */
 if (OPENSSL_BOX_EVP_PKEY_encapsulate_init(ctx, NULL) <= 0)
     /* Error */

 /* Set the mode - only 'RSASVE' is currently supported */
  if (OPENSSL_BOX_EVP_PKEY_CTX_set_kem_op(ctx, "RSASVE") <= 0)
     /* Error */
 /* Determine buffer length */
 if (OPENSSL_BOX_EVP_PKEY_encapsulate(ctx, NULL, &outlen, NULL, &secretlen) <= 0)
     /* Error */
  ...
  */
 if (OPENSSL_BOX_EVP_PKEY_encapsulate(ctx, out, &outlen, secret, &secretlen) <= 0)
     /* Error */
...
The functions OPENSSL_BOX_EVP_PKEY_encapsulate_init() and OPENSSL_BOX_EVP_PKEY_encapsulate() were
added in OpenSSL 3.0.
The function OPENSSL_BOX_EVP_PKEY_auth_encapsulate_init() was added in OpenSSL 3.2.
