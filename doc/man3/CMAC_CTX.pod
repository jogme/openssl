...
=item L<OPENSSL_BOX_EVP_MAC_CTX_new(3)> to create a new MAC context.

=item L<OPENSSL_BOX_EVP_MAC_CTX_free(3)> to free the MAC context.

=item L<OPENSSL_BOX_EVP_MAC_init(3)> to initialize the MAC context.

=item L<OPENSSL_BOX_EVP_MAC_update(3)> to update the MAC with data.

=item L<OPENSSL_BOX_EVP_MAC_final(3)> to finalize the MAC and retrieve the output.

=back

Alternatively, for a single-step MAC computation, use the L<OPENSSL_BOX_EVP_Q_mac(3)>
function.
...
see L<OPENSSL_BOX_EVP_MAC_CTX_new(3)>, L<OPENSSL_BOX_EVP_MAC_CTX_free(3)>, L<OPENSSL_BOX_EVP_MAC_init(3)>,
L<OPENSSL_BOX_EVP_MAC_update(3)>, and L<OPENSSL_BOX_EVP_MAC_final(3)>.
