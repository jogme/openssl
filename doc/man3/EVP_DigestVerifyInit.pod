...
OPENSSL_BOX_EVP_DigestVerifyInit_ex, OPENSSL_BOX_EVP_DigestVerifyInit, OPENSSL_BOX_EVP_DigestVerifyUpdate,
OPENSSL_BOX_EVP_DigestVerifyFinal, OPENSSL_BOX_EVP_DigestVerify - EVP signature verification functions
...
=head1 SYNOPSIS
 ...
 int OPENSSL_BOX_EVP_DigestVerifyUpdate(EVP_MD_CTX *ctx, const void *d, size_t cnt);
 int OPENSSL_BOX_EVP_DigestVerifyFinal(EVP_MD_CTX *ctx, const unsigned char *sig,
                           size_t siglen);
 int OPENSSL_BOX_EVP_DigestVerify(EVP_MD_CTX *ctx, const unsigned char *sig,
...
OPENSSL_BOX_EVP_DigestVerifyInit_ex() sets up verification context B<ctx> to use a
...
B<ctx> must be created with OPENSSL_BOX_EVP_MD_CTX_new() before calling this function. If
...
an EVP_PKEY_CTX value before being passed to OPENSSL_BOX_EVP_DigestVerifyInit_ex()
(which means the EVP_PKEY_CTX is created inside
OPENSSL_BOX_EVP_DigestVerifyInit_ex() and it will be freed automatically when the
EVP_MD_CTX is freed). If the EVP_PKEY_CTX to be used is created by
OPENSSL_BOX_EVP_DigestVerifyInit_ex then it will use the B<OSSL_LIB_CTX> specified
in I<libctx> and the property query string specified in I<props>.

No B<EVP_PKEY_CTX> will be created by OPENSSL_BOX_EVP_DigestVerifyInit_ex() if the
passed B<ctx> has already been assigned one via L<OPENSSL_BOX_EVP_MD_CTX_set_pkey_ctx(3)>.
See also L<SM2(7)>.
...
OPENSSL_BOX_EVP_DigestVerifyInit() works in the same way as
OPENSSL_BOX_EVP_DigestVerifyInit_ex() except that the B<mdname> parameter will be
...
OPENSSL_BOX_EVP_DigestVerifyUpdate() hashes B<cnt> bytes of data at B<d> into the
verification context B<ctx>. This function can be called several times on the
same B<ctx> to include additional data.

OPENSSL_BOX_EVP_DigestVerifyFinal() verifies the data in B<ctx> against the signature in
B<sig> of length B<siglen>.

OPENSSL_BOX_EVP_DigestVerify() verifies B<tbslen> bytes at B<tbs> against the signature
in B<sig> of length B<siglen>.

=head1 RETURN VALUES

OPENSSL_BOX_EVP_DigestVerifyInit() and OPENSSL_BOX_EVP_DigestVerifyUpdate() return 1 for success and 0
for failure.

OPENSSL_BOX_EVP_DigestVerifyFinal() and OPENSSL_BOX_EVP_DigestVerify() return 1 for success; any other
...
OPENSSL_BOX_EVP_DigestVerify() is a one shot operation which verifies a single block of
data in one function. For algorithms that support streaming it is equivalent
to calling OPENSSL_BOX_EVP_DigestVerifyUpdate() and OPENSSL_BOX_EVP_DigestVerifyFinal(). For
...
The call to OPENSSL_BOX_EVP_DigestVerifyFinal() internally finalizes a copy of the digest
context. This means that EVP_VerifyUpdate() and OPENSSL_BOX_EVP_VerifyFinal() can
...
L<OPENSSL_BOX_EVP_MD_CTX_set_flags(3)>.

Note that not all providers support continuation, in case the selected
provider does not allow to duplicate contexts OPENSSL_BOX_EVP_DigestVerifyFinal() will
finalize the digest context and attempting to process additional data via
OPENSSL_BOX_EVP_DigestVerifyUpdate() will result in an error.

OPENSSL_BOX_EVP_DigestVerifyInit() and OPENSSL_BOX_EVP_DigestVerifyInit_ex() functions can be called
...
OPENSSL_BOX_EVP_DigestVerify() can only be called once, and cannot be used again without
reinitialising the B<EVP_MD_CTX> by calling OPENSSL_BOX_EVP_DigestVerifyInit_ex().

Ignoring failure returns of OPENSSL_BOX_EVP_DigestVerifyInit() and OPENSSL_BOX_EVP_DigestVerifyInit_ex()
functions can lead to subsequent undefined behavior when calling
OPENSSL_BOX_EVP_DigestVerifyUpdate(), OPENSSL_BOX_EVP_DigestVerifyFinal(), or OPENSSL_BOX_EVP_DigestVerify().

=head1 SEE ALSO

L<OPENSSL_BOX_EVP_DigestSignInit(3)>,
...
OPENSSL_BOX_EVP_DigestVerifyInit(), OPENSSL_BOX_EVP_DigestVerifyUpdate() and OPENSSL_BOX_EVP_DigestVerifyFinal()
were added in OpenSSL 1.0.0.

OPENSSL_BOX_EVP_DigestVerifyInit_ex() was added in OpenSSL 3.0.

OPENSSL_BOX_EVP_DigestVerifyUpdate() was converted from a macro to a function in OpenSSL
3.0.
