...
using L<OPENSSL_BOX_EVP_PKEY_derive(3)>.

EVP_PKEY_set_tls1_prf_md() sets the message digest associated with the
TLS PRF. OPENSSL_BOX_EVP_md5_sha1() is treated as a special case which uses the PRF
...
OPENSSL_BOX_EVP_PKEY_derive() function. Since the output length is variable, setting
...
and seed value "seed":
 ...
 pctx = OPENSSL_BOX_EVP_PKEY_CTX_new_id(EVP_PKEY_TLS1_PRF, NULL);
