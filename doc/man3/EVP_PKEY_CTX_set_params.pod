...
OPENSSL_BOX_EVP_PKEY_CTX_set_params,
OPENSSL_BOX_EVP_PKEY_CTX_settable_params,
OPENSSL_BOX_EVP_PKEY_CTX_get_params,
OPENSSL_BOX_EVP_PKEY_CTX_gettable_params
...
The OPENSSL_BOX_EVP_PKEY_CTX_get_params() and OPENSSL_BOX_EVP_PKEY_CTX_set_params() functions allow
...
These methods replace the OPENSSL_BOX_EVP_PKEY_CTX_ctrl() mechanism. (OPENSSL_BOX_EVP_PKEY_CTX_ctrl now
calls these methods internally to interact with providers).

OPENSSL_BOX_EVP_PKEY_CTX_gettable_params() and OPENSSL_BOX_EVP_PKEY_CTX_settable_params() get a
...
that can be used with OPENSSL_BOX_EVP_PKEY_CTX_get_params() and OPENSSL_BOX_EVP_PKEY_CTX_set_params()
respectively.
These functions must only be called after the EVP_PKEY_CTX has been initialised
for use in an operation.
...
OPENSSL_BOX_EVP_PKEY_CTX_set_params() returns 1 for success or 0 otherwise.
OPENSSL_BOX_EVP_PKEY_CTX_settable_params() returns an OSSL_PARAM array on success or NULL on
error.
