...
OPENSSL_BOX_EVP_BytesToKey - password based encryption routine
...
OPENSSL_BOX_EVP_BytesToKey() derives a key and IV from various parameters. B<type> is
...
defined in PKCS#5v2.1 and provided by OPENSSL_BOX_PKCS5_PBKDF2_HMAC.
...
If B<data> is NULL, then OPENSSL_BOX_EVP_BytesToKey() returns the number of bytes
needed to store the derived key.
Otherwise, OPENSSL_BOX_EVP_BytesToKey() returns the size of the derived key in bytes,
or 0 on error.

=head1 SEE ALSO

L<evp(7)>, L<RAND_bytes(3)>,
L<OPENSSL_BOX_PKCS5_PBKDF2_HMAC(3)>,
