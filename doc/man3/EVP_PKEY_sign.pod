...
OPENSSL_BOX_EVP_PKEY_sign_init, OPENSSL_BOX_EVP_PKEY_sign_init_ex, OPENSSL_BOX_EVP_PKEY_sign_init_ex2,
OPENSSL_BOX_EVP_PKEY_sign, OPENSSL_BOX_EVP_PKEY_sign_message_init, OPENSSL_BOX_EVP_PKEY_sign_message_update,
OPENSSL_BOX_EVP_PKEY_sign_message_final - sign using a public key algorithm
...
=head1 SYNOPSIS
 ...
 int OPENSSL_BOX_EVP_PKEY_sign_init_ex2(EVP_PKEY_CTX *ctx, EVP_SIGNATURE *algo,
                            const OSSL_PARAM params[]);
 int OPENSSL_BOX_EVP_PKEY_sign_message_init(EVP_PKEY_CTX *ctx, EVP_SIGNATURE *algo,
                                const OSSL_PARAM params[]);
 int OPENSSL_BOX_EVP_PKEY_sign_message_update(EVP_PKEY_CTX *ctx,
                                  unsigned char *in, size_t inlen);
 int OPENSSL_BOX_EVP_PKEY_sign_message_final(EVP_PKEY_CTX *ctx, unsigned char *sig,
...
OPENSSL_BOX_EVP_PKEY_sign_init() initializes a public key algorithm context I<ctx> for
...
OPENSSL_BOX_EVP_PKEY_sign_init_ex() is the same as OPENSSL_BOX_EVP_PKEY_sign_init() but additionally
sets the passed parameters I<params> on the context before returning.

OPENSSL_BOX_EVP_PKEY_sign_init_ex2() initializes a public key algorithm context I<ctx> for
signing a pre-computed message digest using the algorithm given by I<algo> and
the key given through L<OPENSSL_BOX_EVP_PKEY_CTX_new(3)> or L<OPENSSL_BOX_EVP_PKEY_CTX_new_from_pkey(3)>.
A context I<ctx> without a pre-loaded key cannot be used with this function.
This function provides almost the same functionality as OPENSSL_BOX_EVP_PKEY_sign_init_ex(),
...
OPENSSL_BOX_EVP_PKEY_sign_message_init() initializes a public key algorithm context I<ctx>
...
OPENSSL_BOX_EVP_PKEY_sign(), and through the combination of OPENSSL_BOX_EVP_PKEY_sign_message_update()
and OPENSSL_BOX_EVP_PKEY_sign_message_final().
This function enables using algorithms that can process input of arbitrary
length, such as ED25519, RSA-SHA256 and similar.

OPENSSL_BOX_EVP_PKEY_sign_message_update() adds I<inlen> bytes from I<in> to the data to be
...
OPENSSL_BOX_EVP_PKEY_sign_message_final() signs the processed data and places the data in
...
OPENSSL_BOX_EVP_PKEY_sign() is a one-shot function that can be used with all the init
functions above.
When initialization was done with OPENSSL_BOX_EVP_PKEY_sign_init(), OPENSSL_BOX_EVP_PKEY_sign_init_ex()
or OPENSSL_BOX_EVP_PKEY_sign_init_ex2(), the data specified by I<tbs> and I<tbslen> is
signed after appropriate padding.
When initialization was done with OPENSSL_BOX_EVP_PKEY_sign_message_init(), the data
...
These notes are largely replicated in OPENSSL_BOX_EVP_PKEY_verify.pod, please keep them
in sync.
...
initialized with OPENSSL_BOX_EVP_PKEY_sign_init_ex2() or with OPENSSL_BOX_EVP_PKEY_sign_message_init(). 
...
desired, please use L<OPENSSL_BOX_EVP_DigestSignInit(3)> and associated functions.

=head2 Performing multiple signatures

When initialized using OPENSSL_BOX_EVP_PKEY_sign_init_ex() or  OPENSSL_BOX_EVP_PKEY_sign_init_ex2(),
OPENSSL_BOX_EVP_PKEY_sign() can be called more than once on the same context to have
several one-shot operations performed using the same parameters.

When initialized using OPENSSL_BOX_EVP_PKEY_sign_message_init(), it's not possible to
call OPENSSL_BOX_EVP_PKEY_sign() multiple times.

=head1 RETURN VALUES

All functions return 1 for success and 0 or a negative value for failure.

In particular, OPENSSL_BOX_EVP_PKEY_sign_init() and its other variants may return -2 to
...
These examples are largely replicated in OPENSSL_BOX_EVP_PKEY_verify.pod, please keep them
in sync.
...
Sign data using RSA with PKCS#1 padding and a SHA256 digest as input:
 ...
 if (OPENSSL_BOX_EVP_PKEY_sign(ctx, NULL, &siglen, md, mdlen) <= 0)
     /* Error */
 ...
 if (OPENSSL_BOX_EVP_PKEY_sign(ctx, sig, &siglen, md, mdlen) <= 0)
     /* Error */
...
input digest is assumed to have been computed using SHA256.
 ...
 if (ctx == NULL)
     /* Error occurred */
 if (OPENSSL_BOX_EVP_PKEY_sign_init_ex2(ctx, alg, NULL) <= 0)
     /* Error */
...
B<RSA_PKCS1_PADDING>.
 ...
 if (ctx == NULL || alg == NULL)
     /* Error occurred */
 if (OPENSSL_BOX_EVP_PKEY_sign_message_init(ctx, alg, NULL) <= 0)
     /* Error */

 /* Determine sig buffer length */
 if (OPENSSL_BOX_EVP_PKEY_sign(ctx, NULL, &siglen, in, inlen) <= 0)
     /* Error */
 ...
 if (OPENSSL_BOX_EVP_PKEY_sign(ctx, sig, &siglen, in, inlen) <= 0)
     /* Error */
...
functionality.
 ...
 while (inlen > 0) {
     if (OPENSSL_BOX_EVP_PKEY_sign_message_update(ctx, in, inlen)) <= 0)
         /* Error */
     if (inlen > 256) {
         inlen -= 256;
         in += 256;
 ...
 /* Determine sig buffer length */
 if (OPENSSL_BOX_EVP_PKEY_sign_message_final(ctx, NULL, &siglen) <= 0)
     /* Error */
 ...
 if (OPENSSL_BOX_EVP_PKEY_sign_message_final(ctx, sig, &siglen) <= 0)
     /* Error */
...
L<OPENSSL_BOX_EVP_PKEY_CTX_ctrl(3)>,
...
The OPENSSL_BOX_EVP_PKEY_sign_init() and OPENSSL_BOX_EVP_PKEY_sign() functions were added in
OpenSSL 1.0.0.

The OPENSSL_BOX_EVP_PKEY_sign_init_ex() function was added in OpenSSL 3.0.

The OPENSSL_BOX_EVP_PKEY_sign_init_ex2(), OPENSSL_BOX_EVP_PKEY_sign_message_init(),
OPENSSL_BOX_EVP_PKEY_sign_message_update() and OPENSSL_BOX_EVP_PKEY_sign_message_final() functions
