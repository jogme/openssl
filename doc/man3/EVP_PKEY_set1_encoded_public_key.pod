...
EVP_PKEY_set1_encoded_public_key, OPENSSL_BOX_EVP_PKEY_get1_encoded_public_key,
...
L<OPENSSL_BOX_EVP_PKEY_copy_parameters(3)>.
...
OPENSSL_BOX_EVP_PKEY_get1_encoded_public_key() does the equivalent operation except that
...
OPENSSL_BOX_EVP_PKEY_get1_encoded_public_key() with all the same arguments. New applications
should use OPENSSL_BOX_EVP_PKEY_get1_encoded_public_key() instead.
...
OPENSSL_BOX_EVP_PKEY_get1_encoded_public_key() returns the length of the encoded key or 0 for failure.

=head1 EXAMPLES

See L<OPENSSL_BOX_EVP_PKEY_derive_init(3)> and L<OPENSSL_BOX_EVP_PKEY_derive(3)> for information about
...
=head2 Set up a peer's EVP_PKEY ready for a key exchange operation
 ...
 {
     EVP_PKEY *peerkey = OPENSSL_BOX_EVP_PKEY_new();

     if (peerkey == NULL || OPENSSL_BOX_EVP_PKEY_copy_parameters(peerkey, ourkey) <= 0)
         return 0;
...
=head2 Get an encoded public key to send to a peer
 ...
 int get_encoded_pub_key(EVP_PKEY *ourkey)
 {
    ...
    pubkey_len = OPENSSL_BOX_EVP_PKEY_get1_encoded_public_key(ourkey, &pubkey);
    if (pubkey_len == 0)
        return 0;
...
L<OPENSSL_BOX_EVP_PKEY_copy_parameters(3)>,
L<OPENSSL_BOX_EVP_PKEY_derive_init(3)>,
...
EVP_PKEY_set1_encoded_public_key() and OPENSSL_BOX_EVP_PKEY_get1_encoded_public_key() were
