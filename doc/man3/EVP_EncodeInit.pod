...
OPENSSL_BOX_EVP_ENCODE_CTX_new, OPENSSL_BOX_EVP_ENCODE_CTX_free, OPENSSL_BOX_EVP_ENCODE_CTX_copy,
OPENSSL_BOX_EVP_ENCODE_CTX_num, OPENSSL_BOX_EVP_EncodeInit, OPENSSL_BOX_EVP_EncodeUpdate, OPENSSL_BOX_EVP_EncodeFinal,
OPENSSL_BOX_EVP_EncodeBlock, OPENSSL_BOX_EVP_DecodeInit, OPENSSL_BOX_EVP_DecodeUpdate, OPENSSL_BOX_EVP_DecodeFinal,
OPENSSL_BOX_EVP_DecodeBlock - EVP base64 encode/decode routines
...
=head1 SYNOPSIS
 ...
 int OPENSSL_BOX_EVP_ENCODE_CTX_copy(EVP_ENCODE_CTX *dctx, EVP_ENCODE_CTX *sctx);
...
OPENSSL_BOX_EVP_ENCODE_CTX_new() allocates, initializes and returns a context to be used for
the encode/decode functions.

OPENSSL_BOX_EVP_ENCODE_CTX_free() cleans up an encode/decode context B<ctx> and frees up the
...
OPENSSL_BOX_EVP_EncodeInit() initialises B<ctx> for the start of a new encoding operation.

OPENSSL_BOX_EVP_EncodeUpdate() encode B<inl> bytes of data found in the buffer pointed to by
...
subsequent call to OPENSSL_BOX_EVP_EncodeUpdate() or OPENSSL_BOX_EVP_EncodeFinal(). To calculate the
...
additional byte for a NUL terminator. OPENSSL_BOX_EVP_EncodeUpdate() may be called
repeatedly to process large amounts of input data. In the event of an error
OPENSSL_BOX_EVP_EncodeUpdate() will set B<*outl> to 0 and return 0. On success 1 will be
returned.

OPENSSL_BOX_EVP_EncodeFinal() must be called at the end of an encoding operation. It will
...
OPENSSL_BOX_EVP_ENCODE_CTX_copy() can be used to copy a context B<sctx> to a context
B<dctx>. B<dctx> must be initialized before calling this function.

OPENSSL_BOX_EVP_ENCODE_CTX_num() will return the number of as yet unprocessed bytes still to
be encoded or decoded that are pending in the B<ctx> object.

OPENSSL_BOX_EVP_EncodeBlock() encodes a full block of input data in B<f> and of length
...
OPENSSL_BOX_EVP_DecodeInit() initialises B<ctx> for the start of a new decoding operation.

OPENSSL_BOX_EVP_DecodeUpdate() decodes B<inl> characters of data found in the buffer
pointed to by B<in>.
...
in future calls to OPENSSL_BOX_EVP_DecodeUpdate() or OPENSSL_BOX_EVP_DecodeFinal().
...
caller to avoid further calls to OPENSSL_BOX_EVP_DecodeUpdate() after a 0 or negative
(error) return.
...
OPENSSL_BOX_EVP_DecodeUpdate() returns -1 to indicate an error.
...
OPENSSL_BOX_EVP_DecodeFinal() should be called at the end of a decoding operation,
...
OPENSSL_BOX_EVP_DecodeBlock() will decode the block of B<n> characters of base64 data
...
OPENSSL_BOX_EVP_DecodeBlock() will return the length of the data decoded or -1 on error.

=head1 RETURN VALUES

OPENSSL_BOX_EVP_ENCODE_CTX_new() returns a pointer to the newly allocated EVP_ENCODE_CTX
object or NULL on error.

OPENSSL_BOX_EVP_ENCODE_CTX_num() returns the number of bytes pending encoding or decoding in
B<ctx>.

OPENSSL_BOX_EVP_EncodeUpdate() returns 0 on error or 1 on success.

OPENSSL_BOX_EVP_EncodeBlock() returns the number of bytes encoded excluding the NUL
terminator.

OPENSSL_BOX_EVP_DecodeUpdate() returns -1 on error and 0 or 1 on success. If 0 is returned
then no more non-padding base64 characters are expected.

OPENSSL_BOX_EVP_DecodeFinal() returns -1 on error or 1 on success.

OPENSSL_BOX_EVP_DecodeBlock() returns the length of the data decoded or -1 on error.
...
The OPENSSL_BOX_EVP_DecodeUpdate() function was fixed in OpenSSL 3.5,
