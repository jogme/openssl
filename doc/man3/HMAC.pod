...
I<evp_md> is a message digest such as OPENSSL_BOX_EVP_sha1(), OPENSSL_BOX_EVP_ripemd160() etc.
HMAC does not support variable output length digests such as OPENSSL_BOX_EVP_shake128() and
OPENSSL_BOX_EVP_shake256().

HMAC() uses the default B<OSSL_LIB_CTX>.
Use L<OPENSSL_BOX_EVP_Q_mac(3)> instead if a library context is required.

All of the functions described below are deprecated.
Applications should instead use L<OPENSSL_BOX_EVP_MAC_CTX_new(3)>, L<OPENSSL_BOX_EVP_MAC_CTX_free(3)>,
L<OPENSSL_BOX_EVP_MAC_init(3)>, L<OPENSSL_BOX_EVP_MAC_update(3)> and L<OPENSSL_BOX_EVP_MAC_final(3)>
or the 'quick' single-shot MAC function L<OPENSSL_BOX_EVP_Q_mac(3)>.
...
These flags have the same meaning as for L<OPENSSL_BOX_EVP_MD_CTX_set_flags(3)>.
...
L<SHA1(3)>, OPENSSL_BOX_EVP_Q_mac(3), L<evp(7)>
