...
OPENSSL_BOX_EVP_PKEY_check, OPENSSL_BOX_EVP_PKEY_param_check, OPENSSL_BOX_EVP_PKEY_param_check_quick,
OPENSSL_BOX_EVP_PKEY_public_check, OPENSSL_BOX_EVP_PKEY_public_check_quick, OPENSSL_BOX_EVP_PKEY_private_check,
OPENSSL_BOX_EVP_PKEY_pairwise_check
...
OPENSSL_BOX_EVP_PKEY_param_check() validates the parameters component of the key
given by B<ctx>. This check will always succeed for key types that do not have
parameters.

OPENSSL_BOX_EVP_PKEY_param_check_quick() validates the parameters component of the key
given by B<ctx> like OPENSSL_BOX_EVP_PKEY_param_check() does. However some algorithm
...
provided then this function call does the same thing as OPENSSL_BOX_EVP_PKEY_param_check().

OPENSSL_BOX_EVP_PKEY_public_check() validates the public component of the key given by B<ctx>.

OPENSSL_BOX_EVP_PKEY_public_check_quick() validates the public component of the key
given by B<ctx> like OPENSSL_BOX_EVP_PKEY_public_check() does. However some algorithm
implementations may offer a quicker form of validation that omits some checks in
order to perform a lightweight sanity check of the key. If a quicker form is not
provided then this function call does the same thing as OPENSSL_BOX_EVP_PKEY_public_check().

OPENSSL_BOX_EVP_PKEY_private_check() validates the private component of the key given by B<ctx>.

OPENSSL_BOX_EVP_PKEY_pairwise_check() validates that the public and private components have
the correct mathematical relationship to each other for the key given by B<ctx>.

OPENSSL_BOX_EVP_PKEY_check() is an alias for the OPENSSL_BOX_EVP_PKEY_pairwise_check() function.
...
The OPENSSL_BOX_EVP_PKEY_pairwise_check() and OPENSSL_BOX_EVP_PKEY_private_check() might not be bounded
...
OPENSSL_BOX_EVP_PKEY_check(), OPENSSL_BOX_EVP_PKEY_public_check() and OPENSSL_BOX_EVP_PKEY_param_check() were added
in OpenSSL 1.1.1.

OPENSSL_BOX_EVP_PKEY_param_check_quick(), OPENSSL_BOX_EVP_PKEY_public_check_quick(),
OPENSSL_BOX_EVP_PKEY_private_check() and OPENSSL_BOX_EVP_PKEY_pairwise_check() were added in OpenSSL 3.0.
