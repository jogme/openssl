...
This is the default mode. Calling L<OPENSSL_BOX_EVP_PKEY_derive(3)> on an EVP_PKEY_CTX set
...
In this mode calling L<OPENSSL_BOX_EVP_PKEY_derive(3)> will just perform the extract
...
In this mode calling L<OPENSSL_BOX_EVP_PKEY_derive(3)> will just perform the expand
...
L<OPENSSL_BOX_EVP_PKEY_CTX_ctrl_str(3)>.
...
A context for HKDF can be obtained by calling:
 ...
 EVP_PKEY_CTX *pctx = OPENSSL_BOX_EVP_PKEY_CTX_new_id(EVP_PKEY_HKDF, NULL);
...
parameter to the L<OPENSSL_BOX_EVP_PKEY_derive(3)> function.
...
desired length, and pass that buffer to L<OPENSSL_BOX_EVP_PKEY_derive(3)> along with (a
...
salt value "salt" and info value "label":
 ...
 pctx = OPENSSL_BOX_EVP_PKEY_CTX_new_id(EVP_PKEY_HKDF, NULL);
