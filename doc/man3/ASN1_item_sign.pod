...
See OPENSSL_BOX_EVP_PKEY_CTX_set1_id() for further info.
...
then output in DER format and then loaded back in and verified.
 ...
 int test_asn1_item_sign_verify(const char *mdname, EVP_PKEY *pkey, long version)
 {
    ...
    sctx = OPENSSL_BOX_EVP_MD_CTX_new();
    if (sctx == NULL
        || !OPENSSL_BOX_EVP_DigestSignInit_ex(sctx, NULL, mdname, NULL, NULL, pkey))
        goto err;
    ...
    vctx = OPENSSL_BOX_EVP_MD_CTX_new();
    if (vctx == NULL
        || !OPENSSL_BOX_EVP_DigestVerifyInit_ex(vctx, NULL, mdname, NULL, NULL, pkey))
        goto err;
...
err:
    ...
    MyObject_free(obj);
    OPENSSL_BOX_EVP_MD_CTX_free(sctx);
