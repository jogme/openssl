...
gen_server_credentials() {
    ...
    $OPENSSL x509 -new -subj "/CN=Root CA" -not_after 99991231235959Z -CA server_root-cert.pem -CAkey server_root-key.pem \
    ...
    $OPENSSL x509 -new -subj "/CN=Root CA" -not_after 99991231235959Z -CA newWithNew-cert.pem -CAkey newWithNew-key.pem \
...
genee_kem() {
    ...
    $OPENSSL x509 -new -subj "/CN=${cn}" -not_after 99991231235959Z -CA ${ca}.pem -CAkey ${cakey}.pem \
        -out ${cert}.pem -force_pubkey ${cn}-pubkey.pem \
        -extfile <(printf "basicConstraints=critical,CA:false\nkeyUsage=critical,keyEncipherment")
    $OPENSSL x509 -new -subj "/CN=${cn}-noSKID" -not_after 99991231235959Z -CA ${ca}.pem -CAkey ${cakey}.pem \
