                            ...
                            const uint8_t *seed, size_t seed_len)
{
    ...
    *p = OSSL_PARAM_construct_end();
    ...
    if (!TEST_ptr(ctx = OPENSSL_BOX_EVP_PKEY_CTX_new_from_name(lib_ctx, alg, NULL))
                                 ...
                                 int expected)
{
    ...
    *p = OSSL_PARAM_construct_end();
    ...
    if (!TEST_ptr(ctx = OPENSSL_BOX_EVP_PKEY_CTX_new_from_name(lib_ctx, name, NULL))
            || !TEST_int_eq(OPENSSL_BOX_EVP_PKEY_fromdata_init(ctx), 1)
            || !TEST_int_eq(OPENSSL_BOX_EVP_PKEY_fromdata(ctx, pkey, selection,
                                              params), expected))
        goto err;
...
static int ml_dsa_keygen_test(int tst_id)
{
    ...
    if (!TEST_ptr(pkey = do_gen_key(tst->name, tst->seed, tst->seed_len))
            || !TEST_true(OPENSSL_BOX_EVP_PKEY_get_octet_string_param(pkey, OSSL_PKEY_PARAM_ML_DSA_SEED,
                                                          seed, sizeof(seed), &seed_len))
            || !TEST_true(OPENSSL_BOX_EVP_PKEY_get_octet_string_param(pkey, OSSL_PKEY_PARAM_PRIV_KEY,
                                                          priv, sizeof(priv), &priv_len))
            || !TEST_true(OPENSSL_BOX_EVP_PKEY_get_octet_string_param(pkey, OSSL_PKEY_PARAM_PUB_KEY,
            ...
            || !TEST_true(OPENSSL_BOX_EVP_PKEY_get_int_param(pkey, OSSL_PKEY_PARAM_BITS, &bits))
            || !TEST_int_eq(bits, 1952 * 8)
            || !TEST_true(OPENSSL_BOX_EVP_PKEY_get_int_param(pkey, OSSL_PKEY_PARAM_SECURITY_BITS,
                                                 &sec_bits))
            || !TEST_int_eq(sec_bits, 192)
            || !TEST_true(OPENSSL_BOX_EVP_PKEY_get_int_param(pkey, OSSL_PKEY_PARAM_MAX_SIZE,
                                                 &sig_len))
...
static int ml_dsa_siggen_test(int tst_id)
{
    ...
    *p = OSSL_PARAM_construct_end();
    ...
    if (!TEST_true(ml_dsa_create_keypair(&pkey, td->alg, td->priv, td->priv_len,
                                         NULL, 0, 1))
            || !TEST_ptr(sctx = OPENSSL_BOX_EVP_PKEY_CTX_new_from_pkey(lib_ctx, pkey, NULL))
            || !TEST_ptr(sig_alg = OPENSSL_BOX_EVP_SIGNATURE_fetch(lib_ctx, td->alg, NULL))
            || !TEST_int_eq(OPENSSL_BOX_EVP_PKEY_sign_message_init(sctx, sig_alg, params), 1)
            || !TEST_int_eq(OPENSSL_BOX_EVP_PKEY_sign(sctx, NULL, &psig_len,
                                          td->msg, td->msg_len), 1)
            || !TEST_true(OPENSSL_BOX_EVP_PKEY_get_size_t_param(pkey, OSSL_PKEY_PARAM_MAX_SIZE,
                                                    &sig_len2))
            || !TEST_size_t_eq(sig_len2, psig_len)
            || !TEST_ptr(psig = OPENSSL_zalloc(psig_len))
            || !TEST_int_eq(OPENSSL_BOX_EVP_PKEY_sign(sctx, psig, &psig_len,
                                          td->msg, td->msg_len), 1)
            || !TEST_int_eq(OPENSSL_BOX_EVP_Q_digest(lib_ctx, "SHA256", NULL, psig, psig_len,
                                         digest, &digest_len), 1)
...
static int ml_dsa_sigver_test(int tst_id)
{
    ...
    *p = OSSL_PARAM_construct_end();
    ...
    if (!TEST_ptr(vctx = OPENSSL_BOX_EVP_PKEY_CTX_new_from_pkey(lib_ctx, pkey, NULL)))
        goto err;
    if (!TEST_ptr(sig_alg = OPENSSL_BOX_EVP_SIGNATURE_fetch(lib_ctx, td->alg, NULL)))
        goto err;
    if (!TEST_int_eq(OPENSSL_BOX_EVP_PKEY_verify_message_init(vctx, sig_alg, params), 1)
            || !TEST_int_eq(OPENSSL_BOX_EVP_PKEY_verify(vctx, td->sig, td->sig_len,
...
static int ml_dsa_siggen_upd_test(int tst_id)
{
    ...
    *p = OSSL_PARAM_construct_end();
    ...
    if (!TEST_true(ml_dsa_create_keypair(&pkey, td->alg, td->priv, td->priv_len,
            ...
            || !TEST_ptr(sig_alg = OPENSSL_BOX_EVP_SIGNATURE_fetch(lib_ctx, td->alg, NULL))
            || !TEST_int_eq(OPENSSL_BOX_EVP_PKEY_sign_message_init(sctx, sig_alg, params), 1)
            || !TEST_int_eq(OPENSSL_BOX_EVP_PKEY_sign_message_update(sctx, td->msg, 1), 1)
            || !TEST_int_eq(OPENSSL_BOX_EVP_PKEY_sign_message_update(sctx, td->msg + 1,
                                                         td->msg_len -1), 1)
            || !TEST_int_eq(OPENSSL_BOX_EVP_PKEY_sign_message_final(sctx, NULL, &psig_len), 1)
            || !TEST_true(OPENSSL_BOX_EVP_PKEY_get_size_t_param(pkey, OSSL_PKEY_PARAM_MAX_SIZE,
                                                    &sig_len2))
            || !TEST_size_t_eq(sig_len2, psig_len)
            || !TEST_ptr(psig = OPENSSL_zalloc(psig_len))
            || !TEST_int_eq(OPENSSL_BOX_EVP_PKEY_sign_message_final(sctx, psig, &psig_len), 1)
            || !TEST_int_eq(OPENSSL_BOX_EVP_Q_digest(lib_ctx, "SHA256", NULL, psig, psig_len,
                                         digest, &digest_len), 1)
            || !TEST_mem_eq(digest, digest_len, td->sig_digest, td->sig_digest_len))
        goto err;
...
static int ml_dsa_sigver_upd_test(int tst_id)
{
    ...
    *p = OSSL_PARAM_construct_end();
    ...
    if (!TEST_ptr(sig_alg = OPENSSL_BOX_EVP_SIGNATURE_fetch(lib_ctx, td->alg, NULL)))
        goto err;
    if (!TEST_int_eq(OPENSSL_BOX_EVP_PKEY_verify_message_init(vctx, sig_alg, params), 1)
            || !TEST_int_eq(OPENSSL_BOX_EVP_PKEY_CTX_set_signature(vctx, td->sig,
                                                       td->sig_len), 1)
            || !TEST_int_eq(OPENSSL_BOX_EVP_PKEY_verify_message_update(vctx, td->msg,
                                                           td->msg_len), 1)
            || !TEST_int_eq(OPENSSL_BOX_EVP_PKEY_verify_message_final(vctx), td->expected))
        goto err;
    ret = 1;
err:
    OPENSSL_BOX_EVP_SIGNATURE_free(sig_alg);
...
static int ml_dsa_key_dup_test(void)
{
    ...
    if (!TEST_ptr(pkey = do_gen_key(tst->name, tst->seed, tst->seed_len))
            || !TEST_ptr(pkey_copy = OPENSSL_BOX_EVP_PKEY_dup(pkey))
            || !TEST_int_eq(OPENSSL_BOX_EVP_PKEY_eq(pkey, pkey_copy), 1)
            || !TEST_ptr(ctx = OPENSSL_BOX_EVP_PKEY_CTX_new_from_pkey(lib_ctx, pkey_copy, NULL))
            || !TEST_int_eq(OPENSSL_BOX_EVP_PKEY_check(ctx), 1))
        goto err;
    ret = 1;
 err:
    OPENSSL_BOX_EVP_PKEY_free(pkey);
    OPENSSL_BOX_EVP_PKEY_free(pkey_copy);
    OPENSSL_BOX_EVP_PKEY_CTX_free(ctx);
    return ret;
}
...
static int from_data_bad_input_test(void)
{
    ...
    if (!TEST_ptr(ctx = OPENSSL_BOX_EVP_PKEY_CTX_new_from_name(lib_ctx, "ML-DSA-44", NULL))
            || !TEST_int_eq(OPENSSL_BOX_EVP_PKEY_fromdata_init(ctx), 1)
            || !TEST_ptr(OPENSSL_BOX_EVP_PKEY_fromdata_settable(ctx, OSSL_KEYMGMT_SELECT_KEYPAIR))
            || !TEST_ptr_null(OPENSSL_BOX_EVP_PKEY_fromdata_settable(ctx, 0)))
        goto err;

    params[0] = OSSL_PARAM_construct_uint32(OSSL_PKEY_PARAM_PRIV_KEY, &i);
    params[1] = OSSL_PARAM_construct_end();
    if (!TEST_int_ne(OPENSSL_BOX_EVP_PKEY_fromdata(ctx, &pkey, OSSL_KEYMGMT_SELECT_PRIVATE_KEY,
                                       params), 1))
        goto err;
    params[0] = OSSL_PARAM_construct_uint32(OSSL_PKEY_PARAM_PUB_KEY, &i);
    params[1] = OSSL_PARAM_construct_end();
    if (!TEST_int_ne(OPENSSL_BOX_EVP_PKEY_fromdata(ctx, &pkey, OSSL_KEYMGMT_SELECT_PUBLIC_KEY,
                                       params), 1))
        goto err;
    ret = 1;
 err:
...
static int ml_dsa_keygen_drbg_test(void)
{
    ...
    if (!TEST_ptr(pkey = do_gen_key("ML-DSA-44", NULL, 0))
            ...
            || !TEST_int_eq(OPENSSL_BOX_EVP_PKEY_eq(pkey, pkey2), 0)
            || !TEST_int_eq(OPENSSL_BOX_EVP_PKEY_eq(pkey, pkey3), -1)
            || !TEST_int_eq(OPENSSL_BOX_EVP_PKEY_get_raw_private_key(pkey, NULL, &len), 1)
            || !TEST_size_t_gt(len, 0)
            || !TEST_ptr(priv = OPENSSL_malloc(len))
            || !TEST_int_eq(OPENSSL_BOX_EVP_PKEY_get_raw_private_key(pkey, priv, &len), 1)
            || !TEST_int_eq(OPENSSL_BOX_EVP_PKEY_get_raw_public_key(pkey, NULL, &len), 1)
            || !TEST_size_t_gt(len, 0)
            || !TEST_ptr(pub = OPENSSL_malloc(len))
            || !TEST_int_eq(OPENSSL_BOX_EVP_PKEY_get_raw_public_key(pkey, pub, &len), 1)
            ...
            || !TEST_int_eq(OPENSSL_BOX_EVP_PKEY_eq(pkey_pub, pkey), 1))
        goto err;
    ret = 1;
 err:
    OPENSSL_free(pub);
    ...
    OPENSSL_BOX_EVP_PKEY_free(pkey_pub);
    return ret;
}
...
static int do_ml_dsa_sign_verify(const char *alg, int tstid)
{
    ...
    *p++ = OSSL_PARAM_construct_end();
    ...
    if (!TEST_ptr(sctx = OPENSSL_BOX_EVP_PKEY_CTX_new_from_pkey(lib_ctx, key, NULL))
            || !TEST_ptr(sig_alg = OPENSSL_BOX_EVP_SIGNATURE_fetch(lib_ctx, alg, NULL))
            || !TEST_int_eq(OPENSSL_BOX_EVP_PKEY_sign_message_init(sctx, sig_alg, params), sp->expected))
        goto err;
    if (sp->expected == 0) {
        ret = 1; /* return true as we expected to fail */
        goto err;
    }
    if (!TEST_int_eq(OPENSSL_BOX_EVP_PKEY_sign(sctx, NULL, &sig_len, sp->msg, sp->msg_len), 1)
            || !TEST_ptr(sig = OPENSSL_zalloc(sig_len)))
        goto err;
    sig_len--;
    if (!TEST_int_eq(OPENSSL_BOX_EVP_PKEY_sign(sctx, sig, &sig_len, sp->msg, sp->msg_len), 0))
        goto err;
    sig_len++;
    if (!TEST_int_eq(OPENSSL_BOX_EVP_PKEY_sign(sctx, sig, &sig_len, sp->msg, sp->msg_len), 1)
            || !TEST_ptr(vctx = OPENSSL_BOX_EVP_PKEY_CTX_new_from_pkey(lib_ctx, key, NULL))
            || !TEST_int_eq(OPENSSL_BOX_EVP_PKEY_verify_message_init(vctx, sig_alg, params), 1)
            || !TEST_int_eq(OPENSSL_BOX_EVP_PKEY_verify(vctx, sig, sig_len, sp->msg, sp->msg_len), 1))
        goto err;
    ret = 1;
err:
...
static int ml_dsa_digest_sign_verify_test(void)
{
    ...
    *p++ = OSSL_PARAM_construct_end();
    ...
    if (!TEST_ptr(mctx = OPENSSL_BOX_EVP_MD_CTX_new())
            || !TEST_int_eq(OPENSSL_BOX_EVP_DigestSignInit_ex(mctx, NULL, "SHA256",
                                                  lib_ctx, "?fips=true",
                                                  key, params), 0)
            || !TEST_int_eq(OPENSSL_BOX_EVP_DigestSignInit_ex(mctx, NULL, NULL, lib_ctx,
    ...
    if (!TEST_int_eq(OPENSSL_BOX_EVP_DigestSign(mctx, NULL, &sig_len, sp->msg, sp->msg_len), 1)
            || !TEST_ptr(sig = OPENSSL_zalloc(sig_len)))
        goto err;
    sig_len--;
    if (!TEST_int_eq(OPENSSL_BOX_EVP_DigestSign(mctx, sig, &sig_len, sp->msg, sp->msg_len), 0))
        goto err;
    sig_len++;
    if (!TEST_int_eq(OPENSSL_BOX_EVP_DigestSignInit_ex(mctx, NULL, NULL, lib_ctx, "?fips=true",
                                           key, params), 1)
            || !TEST_int_eq(OPENSSL_BOX_EVP_DigestSign(mctx, sig, &sig_len,
                                           sp->msg, sp->msg_len), 1)
            || !TEST_int_eq(OPENSSL_BOX_EVP_DigestVerifyInit_ex(mctx, NULL, "SHA256",
                                                    lib_ctx, "?fips=true",
                                                    key, params), 0)
            || !TEST_int_eq(OPENSSL_BOX_EVP_DigestVerifyInit_ex(mctx, NULL, NULL,
                                                    lib_ctx, "?fips=true",
                                                    key, params), 1)
            || !TEST_int_eq(OPENSSL_BOX_EVP_DigestVerify(mctx, sig, sig_len,
                                             sp->msg, sp->msg_len), 1))
        goto err;
    ret = 1;
err:
