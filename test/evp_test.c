...
#define OPENSSL_SUPPRESS_DEPRECATED /* OPENSSL_BOX_EVP_PKEY_new_CMAC_key */
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <ctype.h>
...
static int mac_check_fips_approved(EVP_MAC_CTX *ctx, EVP_TEST *t)
{
    ...
    if (OPENSSL_BOX_EVP_MAC_CTX_gettable_params(ctx) == NULL)
        return 1;

    params[0] = OSSL_PARAM_construct_int(OSSL_MAC_PARAM_FIPS_APPROVED_INDICATOR,
                                         &approved);
    if (!OPENSSL_BOX_EVP_MAC_CTX_get_params(ctx, params))
        return 0;
...
static int pkey_check_fips_approved(EVP_PKEY_CTX *ctx, EVP_TEST *t)
{
     ...
     */
    int approved = 1;
    const OSSL_PARAM *gettables = OPENSSL_BOX_EVP_PKEY_CTX_gettable_params(ctx);
    ...
    if (OPENSSL_BOX_EVP_PKEY_CTX_gettable_params(ctx) == NULL)
        return 1;

    params[0] = OSSL_PARAM_construct_int(OSSL_ALG_PARAM_FIPS_APPROVED_INDICATOR,
                                         &approved);
    if (!OPENSSL_BOX_EVP_PKEY_CTX_get_params(ctx, params))
        return 0;
    return check_fips_approved(t, approved);
}
...
static int rand_check_fips_approved(EVP_RAND_CTX *ctx, EVP_TEST *t)
{
    ...
    if (OPENSSL_BOX_EVP_RAND_CTX_gettable_params(ctx) == NULL)
        return 1;

    params[0] = OSSL_PARAM_construct_int(OSSL_DRBG_PARAM_FIPS_APPROVED_INDICATOR,
                                         &approved);
    if (!OPENSSL_BOX_EVP_RAND_CTX_get_params(ctx, params))
        return 0;
    return check_fips_approved(t, approved);
}
...
static int pkey_check_security_category(EVP_TEST *t, EVP_PKEY *pkey) {
    return check_security_category(t, pkey,
                                   (int (*)(void *, OSSL_PARAM *))OPENSSL_BOX_EVP_PKEY_get_params);
}
...
static int digest_test_init(EVP_TEST *t, const char *alg)
{
    ...
    if ((digest = fetched_digest = OPENSSL_BOX_EVP_MD_fetch(libctx, alg, propquery)) == NULL
        && (digest = OPENSSL_BOX_EVP_get_digestbyname(alg)) == NULL)
        return 0;
...
static void digest_test_cleanup(EVP_TEST *t)
{
    ...
    OPENSSL_BOX_EVP_MD_free(mdat->fetched_digest);
}
...
{
    return OPENSSL_BOX_EVP_DigestUpdate(ctx, buf, buflen);
}
...
static int test_duplicate_md_ctx(EVP_TEST *t, EVP_MD_CTX *mctx)
{
    ...
    if (!TEST_ptr(mctx))
        return 0;
    if (!OPENSSL_BOX_EVP_DigestFinalXOF(mctx, (unsigned char *)dont, 0)) {
...
static int digest_test_run(EVP_TEST *t)
{
    ...
    *p++ = OSSL_PARAM_construct_end();
    ...
    if (!OPENSSL_BOX_EVP_DigestInit_ex2(mctx, expected->digest, params)) {
    ...
    xof |= OPENSSL_BOX_EVP_MD_xof(expected->digest);
    if (xof) {
        EVP_MD_CTX *mctx_cpy;

        if (!TEST_ptr(mctx_cpy = OPENSSL_BOX_EVP_MD_CTX_new())) {
            goto err;
        }
        if (!TEST_true(OPENSSL_BOX_EVP_MD_CTX_copy(mctx_cpy, mctx))) {
            OPENSSL_BOX_EVP_MD_CTX_free(mctx_cpy);
            goto err;
        } else if (!test_duplicate_md_ctx(t, mctx_cpy)) {
            goto err;
        }

        if (!test_duplicate_md_ctx(t, OPENSSL_BOX_EVP_MD_CTX_dup(mctx)))
            goto err;

        got_len = (unsigned int)expected->output_len;
        if (!OPENSSL_BOX_EVP_DigestFinalXOF(mctx, got, expected->output_len)) {
            t->err = "DIGESTFINALXOF_ERROR";
            goto err;
        }
    } else {
        if (!OPENSSL_BOX_EVP_DigestFinal(mctx, got, &got_len)) {
    ...
    /* Test the OPENSSL_BOX_EVP_Q_digest interface as well */
    if (sk_EVP_TEST_BUFFER_num(expected->input) == 1
        ...
        if (!TEST_true(OPENSSL_BOX_EVP_Q_digest(libctx,
                                    OPENSSL_BOX_EVP_MD_get0_name(expected->fetched_digest),
                                    NULL, inbuf->buf, inbuf->buflen,
                                    got, &size))
                || !TEST_mem_eq(got, size,
                                expected->output, expected->output_len)) {
            t->err = "OPENSSL_BOX_EVP_Q_digest failed";
            goto err;
        }
    }
...
static int cipher_test_valid_fragmentation(CIPHER_DATA *cdat)
{
    return (cdat->aead == EVP_CIPH_CCM_MODE
            ...
            || (cdat->aead == -1
                && OPENSSL_BOX_EVP_CIPHER_get_mode(cdat->cipher) == EVP_CIPH_STREAM_CIPHER)
            || ((OPENSSL_BOX_EVP_CIPHER_get_flags(cdat->cipher) & EVP_CIPH_FLAG_CTS) != 0)
            || OPENSSL_BOX_EVP_CIPHER_get_mode(cdat->cipher) == EVP_CIPH_SIV_MODE
            || OPENSSL_BOX_EVP_CIPHER_get_mode(cdat->cipher) == EVP_CIPH_GCM_SIV_MODE
            || OPENSSL_BOX_EVP_CIPHER_get_mode(cdat->cipher) == EVP_CIPH_XTS_MODE
            || OPENSSL_BOX_EVP_CIPHER_get_mode(cdat->cipher) == EVP_CIPH_WRAP_MODE
            || OPENSSL_BOX_EVP_CIPHER_get_mode(cdat->cipher) == EVP_CIPH_CBC_MODE) ? 0 : 1;
}
...
static int cipher_test_init(EVP_TEST *t, const char *alg)
{
    ...
    ERR_set_mark();
    if ((cipher = fetched_cipher = OPENSSL_BOX_EVP_CIPHER_fetch(libctx, alg, propquery)) == NULL
        && (cipher = OPENSSL_BOX_EVP_get_cipherbyname(alg)) == NULL) {
    ...
    cdat->enc = -1;
    m = OPENSSL_BOX_EVP_CIPHER_get_mode(cipher);
    if (OPENSSL_BOX_EVP_CIPHER_get_flags(cipher) & EVP_CIPH_FLAG_AEAD_CIPHER)
        cdat->aead = m != 0 ? m : -1;
    else
        cdat->aead = 0;
...
static void cipher_test_cleanup(EVP_TEST *t)
{
    ...
    OPENSSL_BOX_EVP_CIPHER_free(cdat->fetched_cipher);
                           ...
                           const OSSL_PARAM initparams[])
{
    ...
    t->err = "TEST_FAILURE";
    if (!TEST_ptr(ctx_base = OPENSSL_BOX_EVP_CIPHER_CTX_new()))
        goto err;
    if (!TEST_ptr(ctx = OPENSSL_BOX_EVP_CIPHER_CTX_new()))
        goto err;
    OPENSSL_BOX_EVP_CIPHER_CTX_set_flags(ctx_base, EVP_CIPHER_CTX_FLAG_WRAP_ALLOW);
    if (enc) {
    ...
    if (!OPENSSL_BOX_EVP_CipherInit_ex2(ctx_base, expected->cipher, NULL, NULL, enc,
                            initparams)) {
    ...
    if (expected->cts_mode != NULL) {
        ...
        if (!OPENSSL_BOX_EVP_CIPHER_CTX_set_params(ctx_base, params)) {
    ...
    if (expected->iv) {
        if (expected->aead) {
            if (OPENSSL_BOX_EVP_CIPHER_CTX_ctrl(ctx_base, EVP_CTRL_AEAD_SET_IVLEN,
                                    (int)expected->iv_len, 0) <= 0) {
                t->err = "INVALID_IV_LENGTH";
                goto err;
            }
        } else if (expected->iv_len != (size_t)OPENSSL_BOX_EVP_CIPHER_CTX_get_iv_length(ctx_base)) {
            t->err = "INVALID_IV_LENGTH";
            goto err;
        }
    }
    if (expected->aead && !expected->tls_aad) {
        ...
        if (tag || expected->aead != EVP_CIPH_GCM_MODE) {
            if (OPENSSL_BOX_EVP_CIPHER_CTX_ctrl(ctx_base, EVP_CTRL_AEAD_SET_TAG,
    ...
    if (expected->rounds > 0) {
        ...
        if (OPENSSL_BOX_EVP_CIPHER_CTX_ctrl(ctx_base, EVP_CTRL_SET_RC5_ROUNDS, rounds, NULL) <= 0) {
            t->err = "INVALID_ROUNDS";
            goto err;
        }
    }

    if (!OPENSSL_BOX_EVP_CIPHER_CTX_set_key_length(ctx_base, (int)expected->key_len)) {
    ...
    if (expected->key_bits > 0) {
        ...
        if (OPENSSL_BOX_EVP_CIPHER_CTX_ctrl(ctx_base, EVP_CTRL_SET_RC2_KEY_BITS, bits, NULL) <= 0) {
            t->err = "INVALID KEY BITS";
            goto err;
        }
    }
    if (!OPENSSL_BOX_EVP_CipherInit_ex(ctx_base, NULL, NULL, expected->key, expected->iv, -1)) {
        t->err = "KEY_SET_ERROR";
        goto err;
    }
    ...
    if (expected->iv != NULL) {
        ...
        if (!TEST_true(OPENSSL_BOX_EVP_CIPHER_CTX_get_updated_iv(ctx_base, iv, sizeof(iv)))
            || ((OPENSSL_BOX_EVP_CIPHER_get_flags(expected->cipher) & EVP_CIPH_CUSTOM_IV) == 0
    ...
    ERR_set_mark();
    if (!OPENSSL_BOX_EVP_CIPHER_CTX_copy(ctx, ctx_base)) {
        if (fips_dupctx_supported) {
            TEST_info("Doing a copy of Cipher %s Fails!\n",
                      OPENSSL_BOX_EVP_CIPHER_get0_name(expected->cipher));
        ...
        OPENSSL_BOX_EVP_CIPHER_CTX_free(ctx_base);
        ctx_base = NULL;
    }
    /* Likewise for dup */
    duped = OPENSSL_BOX_EVP_CIPHER_CTX_dup(ctx);
    if (duped != NULL) {
        OPENSSL_BOX_EVP_CIPHER_CTX_free(ctx);
        ctx = duped;
    } else {
    ...
    if (expected->mac_key != NULL
        && OPENSSL_BOX_EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_MAC_KEY,
    ...
    if (expected->aead == EVP_CIPH_CCM_MODE) {
        if (!OPENSSL_BOX_EVP_CipherUpdate(ctx, NULL, &tmplen, NULL, (int)out_len)) {
        ...
        if (!frag) {
            ...
            for (i = 0; expected->aad[i] != NULL; i++) {
                ...
                do {
                    ...
                    if (!OPENSSL_BOX_EVP_CipherUpdate(ctx, NULL, &chunklen,
            ...
            for (i = 0; expected->aad[i] != NULL; i++) {
                if (expected->aad_len[i] > 0) {
                    if (!OPENSSL_BOX_EVP_CipherUpdate(ctx, NULL, &chunklen, expected->aad[i], 1))
                        goto err;
                    donelen++;
                }
                ...
                if (expected->aad_len[i] > 1
                    && !OPENSSL_BOX_EVP_CipherUpdate(ctx, NULL, &chunklen,
    ...
    if (!frag) {
        do {
            ...
            if (!OPENSSL_BOX_EVP_CipherUpdate(ctx, tmp + out_misalign + tmplen, &chunklen,
        ...
        if (in_len > 0) {
            if (!OPENSSL_BOX_EVP_CipherUpdate(ctx, tmp + out_misalign, &chunklen, in, 1))
                goto err;
            tmplen += chunklen;
            in++;
            in_len--;
    ...
    if (!OPENSSL_BOX_EVP_CipherFinal_ex(ctx, tmp + out_misalign + tmplen, &tmpflen)) {
    ...
    if (!enc && expected->tls_aad) {
        if (expected->tls_version >= TLS1_1_VERSION
            && (OPENSSL_BOX_EVP_CIPHER_is_a(expected->cipher, "AES-128-CBC-HMAC-SHA1")
                || OPENSSL_BOX_EVP_CIPHER_is_a(expected->cipher, "AES-256-CBC-HMAC-SHA1"))) {
    ...
    if (enc && expected->tag) {
        if (OPENSSL_BOX_EVP_CIPHER_is_a(expected->cipher, "AES-128-CBC-HMAC-SHA1-ETM")
            || OPENSSL_BOX_EVP_CIPHER_is_a(expected->cipher, "AES-128-CBC-HMAC-SHA256-ETM")
            || OPENSSL_BOX_EVP_CIPHER_is_a(expected->cipher, "AES-128-CBC-HMAC-SHA512-ETM")
            || OPENSSL_BOX_EVP_CIPHER_is_a(expected->cipher, "AES-192-CBC-HMAC-SHA1-ETM")
            || OPENSSL_BOX_EVP_CIPHER_is_a(expected->cipher, "AES-192-CBC-HMAC-SHA256-ETM")
            || OPENSSL_BOX_EVP_CIPHER_is_a(expected->cipher, "AES-192-CBC-HMAC-SHA512-ETM")
            || OPENSSL_BOX_EVP_CIPHER_is_a(expected->cipher, "AES-256-CBC-HMAC-SHA1-ETM")
            || OPENSSL_BOX_EVP_CIPHER_is_a(expected->cipher, "AES-256-CBC-HMAC-SHA256-ETM")
            || OPENSSL_BOX_EVP_CIPHER_is_a(expected->cipher, "AES-256-CBC-HMAC-SHA512-ETM")) {
            ...
            if (OPENSSL_BOX_EVP_CIPHER_is_a(expected->cipher, "AES-128-CBC-HMAC-SHA1-ETM")
                ...
                || OPENSSL_BOX_EVP_CIPHER_is_a(expected->cipher, "AES-256-CBC-HMAC-SHA1-ETM"))
                tag_len = 20;
            else if (OPENSSL_BOX_EVP_CIPHER_is_a(expected->cipher, "AES-128-CBC-HMAC-SHA256-ETM")
                     || OPENSSL_BOX_EVP_CIPHER_is_a(expected->cipher, "AES-192-CBC-HMAC-SHA256-ETM")
                     || OPENSSL_BOX_EVP_CIPHER_is_a(expected->cipher, "AES-256-CBC-HMAC-SHA256-ETM"))
                tag_len = 32;
            else if (OPENSSL_BOX_EVP_CIPHER_is_a(expected->cipher, "AES-128-CBC-HMAC-SHA512-ETM")
                     || OPENSSL_BOX_EVP_CIPHER_is_a(expected->cipher, "AES-192-CBC-HMAC-SHA512-ETM")
                     || OPENSSL_BOX_EVP_CIPHER_is_a(expected->cipher, "AES-256-CBC-HMAC-SHA512-ETM"))
                tag_len = 64;
    ...
    if (expected->next_iv != NULL) {
        /* Some (e.g., GCM) tests use IVs longer than EVP_MAX_IV_LENGTH. */
        unsigned char iv[128];
        if (!TEST_true(OPENSSL_BOX_EVP_CIPHER_CTX_get_updated_iv(ctx, iv, sizeof(iv)))
...
static int cipher_test_run(EVP_TEST *t)
{
    ...
    TEST_info("RUNNING TEST FOR CIPHER %s\n", OPENSSL_BOX_EVP_CIPHER_get0_name(cdat->cipher));
    if (!cdat->key) {
        t->err = "NO_KEY";
        return 0;
    }
    if (!cdat->iv && OPENSSL_BOX_EVP_CIPHER_get_iv_length(cdat->cipher) > 0) {
        /* IV is optional and usually omitted in wrap mode */
        if (OPENSSL_BOX_EVP_CIPHER_get_mode(cdat->cipher) != EVP_CIPH_WRAP_MODE) {
            t->err = "NO_IV";
            return 0;
        }
    }
...
static int mac_test_init(EVP_TEST *t, const char *alg)
{
    ...
    if ((mac = OPENSSL_BOX_EVP_MAC_fetch(libctx, alg, propquery)) == NULL) {
        /*
...
static void mac_test_cleanup(EVP_TEST *t)
{
    ...
    OPENSSL_BOX_EVP_MAC_free(mdat->mac);
    OPENSSL_free(mdat->mac_name);
                              ...
                              const char *value)
{
    ...
    if (p != NULL) {
        *p++ = '\0';
        rv = OPENSSL_BOX_EVP_PKEY_CTX_ctrl_str(pctx, tmpval, p);
    }
    if (rv == -2)
...
static int mac_test_run_pkey(EVP_TEST *t)
{
    ...
    if (expected->type == EVP_PKEY_CMAC) {
        ...
        if (!TEST_ptr(cipher = OPENSSL_BOX_EVP_CIPHER_fetch(libctx, expected->alg, propquery))) {
            t->err = "MAC_KEY_CREATE_ERROR";
            goto err;
        }
        tmpctx = OSSL_LIB_CTX_set0_default(libctx);
        key = OPENSSL_BOX_EVP_PKEY_new_CMAC_key(NULL, expected->key, expected->key_len,
                                    cipher);
        OSSL_LIB_CTX_set0_default(tmpctx);
#endif
    } else {
        key = OPENSSL_BOX_EVP_PKEY_new_raw_private_key_ex(libctx,
    ...
    if (!OPENSSL_BOX_EVP_DigestSignInit_ex(mctx, &pctx, mdname, libctx, NULL, key, NULL)) {
    ...
    do {
        ...
        if (!OPENSSL_BOX_EVP_DigestSignUpdate(mctx, expected->input + donelen, current_len)) {
    ...
    if (!OPENSSL_BOX_EVP_DigestSignFinal(mctx, NULL, &got_len)) {
    ...
    if (!OPENSSL_BOX_EVP_DigestSignFinal(mctx, got, &got_len)
...
static int mac_test_run_mac(EVP_TEST *t)
{
    ...
    const OSSL_PARAM *defined_params =
        OPENSSL_BOX_EVP_MAC_settable_ctx_params(expected->mac);
    int xof;
    int reinit = 1;
    size_t input_len, donelen ;
    ...
    if ((ctx = OPENSSL_BOX_EVP_MAC_CTX_new(expected->mac)) == NULL) {
    ...
    if (fips_provider_version_gt(libctx, 3, 2, 0)) {
        ...
        ERR_set_mark();
        size_before_init = OPENSSL_BOX_EVP_MAC_CTX_get_mac_size(ctx);
        ERR_pop_to_mark();
    }
    if (!OPENSSL_BOX_EVP_MAC_init(ctx, expected->key, expected->key_len, params)) {
        t->err = "MAC_INIT_ERROR";
        goto err;
    }
    size_after_init = OPENSSL_BOX_EVP_MAC_CTX_get_mac_size(ctx);
    ...
    if (psizes != sizes) {
        *psizes = OSSL_PARAM_construct_end();
        if (!TEST_true(OPENSSL_BOX_EVP_MAC_CTX_get_params(ctx, sizes))) {
            t->err = "INTERNAL_ERROR";
            goto err;
        }
        if (expected->output_size >= 0
 ...
 retry:
    ...
    do {
        ...
        if (data_chunk_size == 0 || (size_t) data_chunk_size > input_len)
            current_len = input_len;
        if (!OPENSSL_BOX_EVP_MAC_update(ctx, expected->input + donelen, current_len)) {
    ...
    if (xof) {
        ...
        if (!OPENSSL_BOX_EVP_MAC_finalXOF(ctx, got, expected->output_len)
            || !memory_err_compare(t, "TEST_MAC_ERR",
                                   expected->output, expected->output_len,
        ...
        if (!OPENSSL_BOX_EVP_MAC_final(ctx, NULL, &got_len, 0)) {
        ...
        if (!OPENSSL_BOX_EVP_MAC_final(ctx, got, &got_len, got_len)
            || !memory_err_compare(t, "TEST_MAC_ERR",
                                   expected->output, expected->output_len,
                                   got, got_len)) {
            t->err = "TEST_MAC_ERR";
    ...
    if (reinit-- && fips_provider_version_gt(libctx, 3, 0, 0)) {
        ...
        ERR_set_mark();
        ret = OPENSSL_BOX_EVP_MAC_init(ctx, NULL, 0, ivparams);
    ...
    /* Test the OPENSSL_BOX_EVP_Q_mac interface as well */
    if (!xof) {
        OPENSSL_cleanse(got, got_len);
        if (!TEST_true(OPENSSL_BOX_EVP_Q_mac(libctx, expected->mac_name, NULL,
                                 expected->alg, params,
                                 expected->key, expected->key_len,
                ...
                || !TEST_mem_eq(got, size,
                                expected->output, expected->output_len)) {
            t->err = "OPENSSL_BOX_EVP_Q_mac failed";
            goto err;
        }
    }
 err:
...
static void kem_test_cleanup(EVP_TEST *t)
{
    ...
    OPENSSL_BOX_EVP_PKEY_CTX_free(kdata->ctx);
}
                       ...
                       unsigned char **outsecret, size_t *outsecretlen)
{
    ...
    if (OPENSSL_BOX_EVP_PKEY_encapsulate_init(ctx, params) <= 0) {
        t->err = "TEST_ENCAPSULATE_INIT_ERROR";
        goto ok;
    }

    if (op != NULL && OPENSSL_BOX_EVP_PKEY_CTX_set_kem_op(ctx, op) <= 0) {
        t->err = "TEST_SET_KEM_OP_ERROR";
        goto ok;
    }
    if (OPENSSL_BOX_EVP_PKEY_encapsulate(ctx, NULL, &wrappedlen, NULL, &secretlen) <= 0) {
    ...
    if (OPENSSL_BOX_EVP_PKEY_encapsulate(ctx, wrapped, &wrappedlen, secret, &secretlen) <= 0) {
                       ...
                       const unsigned char *expected, size_t expectedlen)
{
    ...
    if (OPENSSL_BOX_EVP_PKEY_decapsulate_init(ctx, p) <= 0) {
    ...
    if (OPENSSL_BOX_EVP_PKEY_decapsulate(ctx, NULL, &outlen, in, inlen) <= 0) {
    ...
    if (OPENSSL_BOX_EVP_PKEY_decapsulate(ctx, out, &outlen, in, inlen) <= 0) {
...
static int kem_test_run(EVP_TEST *t)
{
    ...
    if (kdata->keyname != NULL) {
        ...
        /* Encoded public key */
        if ((pkey = OPENSSL_BOX_EVP_PKEY_new_raw_public_key_ex(libctx, kdata->algname,
                                                   propquery,
        ...
        /* Encoded private key */
        if ((pkey = OPENSSL_BOX_EVP_PKEY_new_raw_private_key_ex(libctx, kdata->algname,
                                                    propquery,
    ...
    if (!TEST_ptr(kdata->ctx = OPENSSL_BOX_EVP_PKEY_CTX_new_from_pkey(libctx, pkey, propquery)))
        goto err;
                                           ...
                                           size_t tbslen))
{
    ...
    if (!TEST_ptr(kdata->sigalgo
                  = OPENSSL_BOX_EVP_SIGNATURE_fetch(libctx, algoname, propquery))) {
...
static void pkey_test_cleanup(EVP_TEST *t)
{
    ...
    OPENSSL_free(kdata->output);
    OPENSSL_BOX_EVP_PKEY_CTX_free(kdata->ctx);
    OPENSSL_BOX_EVP_SIGNATURE_free(kdata->sigalgo);
}
...
static int pkey_test_run(EVP_TEST *t)
{
   ...
   if (!pkey_check_security_category(t, OPENSSL_BOX_EVP_PKEY_CTX_get0_pkey(expected->ctx)))
        goto err;

    /* Make a copy of the EVP_PKEY context, for repeat use further down */
    if (!TEST_ptr(copy = OPENSSL_BOX_EVP_PKEY_CTX_dup(expected->ctx))) {
        t->err = "INTERNAL_ERROR";
        goto err;
    }
 ...
 err:
    OPENSSL_free(got);
    OPENSSL_BOX_EVP_PKEY_CTX_free(copy);
    return 1;
}
...
static int pkey_fromdata_test_init(EVP_TEST *t, const char *name)
{
    ...
    if (!TEST_ptr(kdata = OPENSSL_zalloc(sizeof(*kdata))))
        return 0;
    kdata->ctx = OPENSSL_BOX_EVP_PKEY_CTX_new_from_name(libctx, name, "");
    if (kdata->ctx == NULL)
        goto err;
    if (OPENSSL_BOX_EVP_PKEY_fromdata_init(kdata->ctx) <= 0)
        goto err;
    kdata->controls = sk_OPENSSL_STRING_new_null();
...
static int pkey_fromdata_test_run(EVP_TEST *t)
{
    ...
    ret = 1;
    if (OPENSSL_BOX_EVP_PKEY_fromdata(kdata->ctx, &key, EVP_PKEY_KEYPAIR, p) <= 0) {
...
/*
 * "Sign" implies OPENSSL_BOX_EVP_PKEY_sign_init_ex2() if the argument is a colon-separated
 * pair, {algorithm}:{key}.  If not, it implies OPENSSL_BOX_EVP_PKEY_sign_init_ex()
 */
static int sign_test_init(EVP_TEST *t, const char *name)
{
    if (strchr(name, ':') != NULL)
        return pkey_test_init_ex2(t, name, 0,
                                  OPENSSL_BOX_EVP_PKEY_sign_init_ex2, OPENSSL_BOX_EVP_PKEY_sign);
    return pkey_test_init(t, name, 0, OPENSSL_BOX_EVP_PKEY_sign_init_ex, OPENSSL_BOX_EVP_PKEY_sign);
}
...
/*
 * "Sign-Message" is like "Sign", but uses OPENSSL_BOX_EVP_PKEY_sign_message_init()
...
/*
 * "VerifyRecover" implies OPENSSL_BOX_EVP_PKEY_verify_recover_init_ex2() if the argument is a
 * colon-separated pair, {algorithm}:{key}.
 * If not, it implies OPENSSL_BOX_EVP_PKEY_verify_recover_init_ex()
 */
static int verify_recover_test_init(EVP_TEST *t, const char *name)
{
    if (strchr(name, ':') != NULL)
        return pkey_test_init_ex2(t, name, 1,
                                  OPENSSL_BOX_EVP_PKEY_verify_recover_init_ex2,
                                  OPENSSL_BOX_EVP_PKEY_verify_recover);
    return pkey_test_init(t, name, 1, OPENSSL_BOX_EVP_PKEY_verify_recover_init_ex,
                          OPENSSL_BOX_EVP_PKEY_verify_recover);
}
...
{
    return pkey_test_init(t, name, 0, OPENSSL_BOX_EVP_PKEY_decrypt_init_ex,
                          OPENSSL_BOX_EVP_PKEY_decrypt);
}
...
/*
 * "Verify" implies OPENSSL_BOX_EVP_PKEY_verify_init_ex2() if the argument is a
 * colon-separated pair, {algorithm}:{key}.
 * If not, it implies OPENSSL_BOX_EVP_PKEY_verify_init_ex()
 */
static int verify_test_init(EVP_TEST *t, const char *name)
{
    if (strchr(name, ':') != NULL)
        return pkey_test_init_ex2(t, name, 1,
                                  OPENSSL_BOX_EVP_PKEY_verify_init_ex2, NULL);
    return pkey_test_init(t, name, 1, OPENSSL_BOX_EVP_PKEY_verify_init_ex, NULL);
}
...
static int verify_test_run(EVP_TEST *t)
{
    ...
    if (!pkey_test_run_init(t))
        goto err;
    if (!pkey_check_security_category(t, OPENSSL_BOX_EVP_PKEY_CTX_get0_pkey(kdata->ctx)))
        goto err;
    if (OPENSSL_BOX_EVP_PKEY_verify(kdata->ctx, kdata->output, kdata->output_len,
...
/*
 * "Verify-Message" is like "Verify", but uses OPENSSL_BOX_EVP_PKEY_verify_message_init()
 * The argument must be a colon separated pair, {algorithm}:{key}
 */
static int verify_message_test_init(EVP_TEST *t, const char *name)
{
    return pkey_test_init_ex2(t, name, 0,
                              OPENSSL_BOX_EVP_PKEY_verify_message_init, NULL);
}
...
{
    return pkey_test_init(t, name, 0, OPENSSL_BOX_EVP_PKEY_derive_init_ex, 0);
}
...
static int pderive_test_run(EVP_TEST *t)
{
    ...
    if (!pkey_check_security_category(t, OPENSSL_BOX_EVP_PKEY_CTX_get0_pkey(expected->ctx)))
        goto err;
    if (OPENSSL_BOX_EVP_PKEY_derive_set_peer_ex(expected->ctx, expected->peer,
                                    expected->validate) <= 0) {
        t->err = "DERIVE_SET_PEER_ERROR";
        goto err;
    }

    if (!TEST_ptr(dctx = OPENSSL_BOX_EVP_PKEY_CTX_dup(expected->ctx))) {
        t->err = "DERIVE_ERROR";
        goto err;
    }

    if (OPENSSL_BOX_EVP_PKEY_derive(dctx, NULL, &got_len) <= 0
    ...
    if (OPENSSL_BOX_EVP_PKEY_derive(dctx, got, &got_len) <= 0) {
        t->err = "DERIVE_ERROR";
        goto err;
    }
                             ...
                             const char *keyword, const char *value)
{
    ...
    if (strcmp(keyword, "MD") == 0) {
        pdata->md = OPENSSL_BOX_EVP_get_digestbyname(value);
        if (pdata->md == NULL)
            return -1;
        return 1;
    }
...
static int pbe_test_run(EVP_TEST *t)
{
    ...
    if (expected->pbe_type == PBE_TYPE_PBKDF2) {
        if (OPENSSL_BOX_PKCS5_PBKDF2_HMAC((char *)expected->pass, (int)expected->pass_len,
        ...
        if (OPENSSL_BOX_EVP_PBE_scrypt((const char *)expected->pass, expected->pass_len,
        ...
        fetched_digest = OPENSSL_BOX_EVP_MD_fetch(libctx, OPENSSL_BOX_EVP_MD_get0_name(expected->md),
                                      propquery);
        if (fetched_digest == NULL) {
            t->err = "PKCS12_ERROR";
            goto err;
...
static int encode_test_run(EVP_TEST *t)
{
    ...
    if (!TEST_ptr(decode_ctx = OPENSSL_BOX_EVP_ENCODE_CTX_new())) {
        t->err = "INTERNAL_ERROR";
        goto err;
    }
    ...
    if (expected->encoding == BASE64_CANONICAL_ENCODING) {
        ...
        if (!TEST_ptr(encode_ctx = OPENSSL_BOX_EVP_ENCODE_CTX_new())
                || !TEST_ptr(encode_out =
                        OPENSSL_malloc(EVP_ENCODE_LENGTH(expected->input_len))))
            goto err;

        OPENSSL_BOX_EVP_EncodeInit(encode_ctx);
        ...
        do {
            ...
            if (data_chunk_size == 0 || current_len > input_len)
                current_len = input_len;
            if (!TEST_true(OPENSSL_BOX_EVP_EncodeUpdate(encode_ctx, encode_out, &chunk_len,
        ...
        OPENSSL_BOX_EVP_EncodeFinal(encode_ctx, encode_out + output_len, &chunk_len);
        output_len += chunk_len;
    ...
    output_len = 0;
    OPENSSL_BOX_EVP_DecodeInit(decode_ctx);
    ...
    do {
        ...
        if (data_chunk_size == 0 || (size_t) data_chunk_size > input_len)
            current_len = input_len;
        if (OPENSSL_BOX_EVP_DecodeUpdate(decode_ctx, decode_out + output_len, &chunk_len,
    ...
    if (OPENSSL_BOX_EVP_DecodeFinal(decode_ctx, decode_out + output_len, &chunk_len) != 1) {
        t->err = "DECODE_ERROR";
        goto err;
    }
    output_len += chunk_len;
 ...
 err:
    ...
    OPENSSL_BOX_EVP_ENCODE_CTX_free(decode_ctx);
    OPENSSL_BOX_EVP_ENCODE_CTX_free(encode_ctx);
    return 1;
}
...
static int rand_test_init(EVP_TEST *t, const char *name)
{
    ...
    rand = OPENSSL_BOX_EVP_RAND_fetch(libctx, "TEST-RAND", "-fips");
    if (rand == NULL)
        goto err;
    rdata->parent = OPENSSL_BOX_EVP_RAND_CTX_new(rand, NULL);
    OPENSSL_BOX_EVP_RAND_free(rand);
    if (rdata->parent == NULL)
        goto err;

    *params = OSSL_PARAM_construct_uint(OSSL_RAND_PARAM_STRENGTH, &strength);
    if (!OPENSSL_BOX_EVP_RAND_CTX_set_params(rdata->parent, params))
        goto err;

    rand = OPENSSL_BOX_EVP_RAND_fetch(libctx, name, propquery);
    if (rand == NULL)
        goto err;
    rdata->ctx = OPENSSL_BOX_EVP_RAND_CTX_new(rand, rdata->parent);
    OPENSSL_BOX_EVP_RAND_free(rand);
    if (rdata->ctx == NULL)
        goto err;
    ...
    return 1;
 err:
    OPENSSL_BOX_EVP_RAND_CTX_free(rdata->parent);
    OPENSSL_free(rdata);
    return 0;
}
...
static void rand_test_cleanup(EVP_TEST *t)
{
    ...
    OPENSSL_BOX_EVP_RAND_CTX_free(rdata->ctx);
    OPENSSL_BOX_EVP_RAND_CTX_free(rdata->parent);
}
...
static int rand_test_run(EVP_TEST *t)
{
    ...
    *p = OSSL_PARAM_construct_end();
    if (!OPENSSL_BOX_EVP_RAND_CTX_set_params(expected->ctx, params)) {
        if (t->expect_unapproved == 0) {
            t->err = "OPENSSL_BOX_EVP_RAND_CTX_set_params";
            ret = 1;
        }
        goto err;
    }
    ...
    strength = OPENSSL_BOX_EVP_RAND_get_strength(expected->ctx);
    for (i = 0; i <= expected->n; i++) {
        ...
        *p++ = OSSL_PARAM_construct_octet_string(OSSL_RAND_PARAM_TEST_NONCE,
                                                 z, item->nonce_len);
        *p = OSSL_PARAM_construct_end();
        if (!TEST_true(OPENSSL_BOX_EVP_RAND_instantiate(expected->parent, strength,
                                            0, NULL, 0, params)))
            goto err;

        z = item->pers != NULL ? item->pers : (unsigned char *)"";
        if (!TEST_true(OPENSSL_BOX_EVP_RAND_instantiate
        ...
        if (item->reseed_entropy != NULL) {
            params[0] = OSSL_PARAM_construct_octet_string
                           (OSSL_RAND_PARAM_TEST_ENTROPY, item->reseed_entropy,
                            item->reseed_entropy_len);
            params[1] = OSSL_PARAM_construct_end();
            if (!TEST_true(OPENSSL_BOX_EVP_RAND_CTX_set_params(expected->parent, params)))
                goto err;

            if (!TEST_true(OPENSSL_BOX_EVP_RAND_reseed
        ...
        if (!TEST_true(OPENSSL_BOX_EVP_RAND_generate
        ...
        if (!rand_check_fips_approved(expected->ctx, t))
            goto err;
        if (!TEST_true(OPENSSL_BOX_EVP_RAND_uninstantiate(expected->ctx))
                || !TEST_true(OPENSSL_BOX_EVP_RAND_uninstantiate(expected->parent))
                || !TEST_true(OPENSSL_BOX_EVP_RAND_verify_zeroization(expected->ctx))
                || !TEST_int_eq(OPENSSL_BOX_EVP_RAND_get_state(expected->ctx),
...
static int pkey_kdf_test_init(EVP_TEST *t, const char *name)
{
    ...
    kdata->ctx = OPENSSL_BOX_EVP_PKEY_CTX_new_from_name(libctx, name, propquery);
    if (kdata->ctx == NULL
        || OPENSSL_BOX_EVP_PKEY_derive_init(kdata->ctx) <= 0)
        goto err;
...
static int pkey_kdf_test_run(EVP_TEST *t)
{
    ...
    if (fips_provider_version_eq(libctx, 3, 0, 0)) {
        ...
        if (OPENSSL_BOX_EVP_PKEY_derive(expected->ctx, NULL, &got_len) <= 0) {
            t->err = "INTERNAL_ERROR";
            goto err;
        }
         ...
         * OPENSSL_BOX_EVP_PKEY_derive() can cope with it.
         */
    ...
    if (OPENSSL_BOX_EVP_PKEY_derive(expected->ctx, got, &got_len) <= 0) {
        t->err = "KDF_DERIVE_ERROR";
        goto err;
    }
...
static int keypair_test_run(EVP_TEST *t)
{
    ...
    if ((rv = OPENSSL_BOX_EVP_PKEY_eq(pair->privk, pair->pubk)) != 1) {
        if (0 == rv) {
                                      ...
                                      STACK_OF(OPENSSL_STRING) *out_controls)
{
    ...
    if (sk_OPENSSL_STRING_num(out_controls) > 0) {
        ...
        if (!ctrl2params(t, out_controls,
                         OPENSSL_BOX_EVP_PKEY_gettable_params(pkey),
        ...
        for (p = out_params; p->key != NULL; ++p) {
            if (p->data_type == OSSL_PARAM_OCTET_STRING) {
                ...
                if (data == NULL)
                    goto err;
                ret = OPENSSL_BOX_EVP_PKEY_get_octet_string_param(pkey, p->key, data,
                                                      p->data_size, &len)
                    && len == p->data_size
...
static int keygen_test_run(EVP_TEST *t)
{
    ...
    if (keygen->paramname != NULL) {
        ...
        if (!TEST_ptr(genctx = OPENSSL_BOX_EVP_PKEY_CTX_new_from_pkey(libctx, keyparams,
                                                          propquery)))
            goto err;

    } else {
        if (!TEST_ptr(genctx = OPENSSL_BOX_EVP_PKEY_CTX_new_from_name(libctx, keygen->alg,
                                                          propquery)))
            goto err;
    }
    ...
    if (sk_OPENSSL_STRING_num(keygen->in_controls) > 0) {
        ...
        if (!ctrl2params(t, keygen->in_controls,
                         OPENSSL_BOX_EVP_PKEY_CTX_settable_params(genctx),
                         params, 4, &params_n))
            goto err;
    ...
    if (OPENSSL_BOX_EVP_PKEY_keygen(genctx, &pkey) <= 0) {
                                  ...
                                  int is_oneshot)
{
    ...
    if (strcmp(alg, "NULL") != 0) {
        ...
        md = OPENSSL_BOX_EVP_get_digestbyname(alg);
        if (md == NULL)
            return 0;
    }
    if (!TEST_ptr(mdat = OPENSSL_zalloc(sizeof(*mdat))))
    ...
    mdat->md = md;
    if (!TEST_ptr(mdat->ctx = OPENSSL_BOX_EVP_MD_CTX_new())) {
        OPENSSL_free(mdat);
        return 0;
    }
...
static void digestsigver_test_cleanup(EVP_TEST *t)
{
    ...
    OPENSSL_BOX_EVP_MD_CTX_free(mdata->ctx);
    ...
    if (mdata->deterministic_noncetype == 1) {
        ...
        params[0] =
            OSSL_PARAM_construct_uint(OSSL_SIGNATURE_PARAM_NONCE_TYPE,
                                      &nonce_type);
        params[1] = OSSL_PARAM_construct_end();
        if (!OPENSSL_BOX_EVP_PKEY_CTX_set_params(mdata->pctx, params))
            t->err = "EVP_PKEY_CTX_set_params_ERROR";
        else if (!OPENSSL_BOX_EVP_PKEY_CTX_get_params(mdata->pctx, params))
...
{
    const char *name = data->md == NULL ? NULL : OPENSSL_BOX_EVP_MD_get0_name(data->md);
    OSSL_PARAM params[5];
    OSSL_PARAM *p = NULL;
    int i, ret = 0;
    size_t params_n = 0, params_allocated_n = 0;
    ...
    if (data->is_verify) {
        if (!OPENSSL_BOX_EVP_DigestVerifyInit_ex(data->ctx, &data->pctx, name, libctx,
                                     NULL, data->key, p)) {
            t->err = "DIGESTVERIFYINIT_ERROR";
            goto err;
        }
    } else {
        if (!OPENSSL_BOX_EVP_DigestSignInit_ex(data->ctx, &data->pctx, name, libctx, NULL,
                                   data->key, p)) {
            t->err = "DIGESTSIGNINIT_ERROR";
            goto err;
        }
...
static int digestsign_update_fn(void *ctx, const unsigned char *buf,
                                size_t buflen)
{
    return OPENSSL_BOX_EVP_DigestSignUpdate(ctx, buf, buflen);
}
...
static int digestsign_test_run(EVP_TEST *t)
{
    ...
    if (!OPENSSL_BOX_EVP_DigestSignFinal(expected->ctx, NULL, &got_len)) {
        t->err = "DIGESTSIGNFINAL_LENGTH_ERROR";
        goto err;
    }
    if (!TEST_ptr(got = OPENSSL_malloc(got_len))) {
    ...
    got_len *= 2;
    if (!OPENSSL_BOX_EVP_DigestSignFinal(expected->ctx, got, &got_len)) {
...
static int digestverify_update_fn(void *ctx, const unsigned char *buf,
                                  size_t buflen)
{
    return OPENSSL_BOX_EVP_DigestVerifyUpdate(ctx, buf, buflen);
}
...
static int digestverify_test_run(EVP_TEST *t)
{
    ...
    if (OPENSSL_BOX_EVP_DigestVerifyFinal(mdata->ctx, mdata->output,
                              mdata->output_len) <= 0)
        t->err = "VERIFY_ERROR";
    return 1;
}
...
static int oneshot_digestsign_test_run(EVP_TEST *t)
{
    ...
    if (!OPENSSL_BOX_EVP_DigestSign(expected->ctx, NULL, &got_len,
    ...
    got_len *= 2;
    if (!OPENSSL_BOX_EVP_DigestSign(expected->ctx, got, &got_len,
                        expected->osin, expected->osin_len)) {
...
static int oneshot_digestverify_test_run(EVP_TEST *t)
{
    ...
    if (OPENSSL_BOX_EVP_DigestVerify(mdata->ctx, mdata->output, mdata->output_len,
...
static void free_key_list(KEY_LIST *lst)
{
    while (lst != NULL) {
        ...
        OPENSSL_BOX_EVP_PKEY_free(lst->key);
...
start:
    if (strcmp(pp->key, "PrivateKey") == 0) {
        ...
        if (klist == &private_keys)
            pkey = OPENSSL_BOX_EVP_PKEY_new_raw_private_key_ex(libctx, strnid, NULL, keybin,
                                                   keylen);
        else
            pkey = OPENSSL_BOX_EVP_PKEY_new_raw_public_key_ex(libctx, strnid, NULL, keybin,
                                                  keylen);
        if (pkey == NULL && !key_unsupported()) {
