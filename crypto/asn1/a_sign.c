              ...
              const EVP_MD *type)
{
    ...
    outll = outl = OPENSSL_BOX_EVP_PKEY_get_size(pkey);
                       ...
                       const void *data, EVP_MD_CTX *ctx)
{
    ...
    if (pkey->ameth == NULL) {
        EVP_PKEY_CTX *pctx = OPENSSL_BOX_EVP_MD_CTX_get_pkey_ctx(ctx);
        OSSL_PARAM params[2];
        ...
        if (OPENSSL_BOX_EVP_PKEY_CTX_get_params(pctx, params) <= 0)
            goto err;
    ...
    if (rv == 2) {
        ...
        pkey_id =
            ...
            OPENSSL_BOX_EVP_PKEY_get_id(pkey) == NID_sm2 ? NID_sm2 :
#endif
            pkey->ameth->pkey_id;
