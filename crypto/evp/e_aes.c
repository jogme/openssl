                          ...
                          const unsigned char *iv, int enc)
{
    ...
    const int keylen = OPENSSL_BOX_EVP_CIPHER_CTX_get_key_length(ctx) * 8;
                            ...
                            const unsigned char *in, size_t len)
{
    aesni_cbc_encrypt(in, out, len, &EVP_C_DATA(EVP_AES_KEY,ctx)->ks.ks,
                      ctx->iv, OPENSSL_BOX_EVP_CIPHER_CTX_is_encrypting(ctx));
                              ...
                              const unsigned char *iv, int enc)
{
    ...
    if (key) {
        ...
        const int keylen = OPENSSL_BOX_EVP_CIPHER_CTX_get_key_length(ctx);
                           ...
                           const unsigned char *iv, int enc)
{
    ...
    mode = EVP_CIPHER_CTX_get_mode(ctx);
    bits = OPENSSL_BOX_EVP_CIPHER_CTX_get_key_length(ctx) * 8;
    if (bits <= 0) {
        ERR_raise(ERR_LIB_EVP, EVP_R_INVALID_KEY_LENGTH);
        return 0;
    }
                               ...
                               const unsigned char *iv, int enc)
{
    ...
    if (iv == NULL && key == NULL)
        return 1;
    if (key) {
        const int bits = OPENSSL_BOX_EVP_CIPHER_CTX_get_key_length(ctx) * 8;
                                  ...
                                  const unsigned char *ivec, int enc)
{
    ...
    const int keylen = OPENSSL_BOX_EVP_CIPHER_CTX_get_key_length(ctx);
    const int ivlen = OPENSSL_BOX_EVP_CIPHER_CTX_get_iv_length(ctx);
                                ...
                                const unsigned char *in, size_t len)
{
    S390X_AES_CFB_CTX *cctx = EVP_C_DATA(S390X_AES_CFB_CTX, ctx);
    const int keylen = OPENSSL_BOX_EVP_CIPHER_CTX_get_key_length(ctx);
    const int enc = OPENSSL_BOX_EVP_CIPHER_CTX_is_encrypting(ctx);
    const int ivlen = OPENSSL_BOX_EVP_CIPHER_CTX_get_iv_length(ctx);
    unsigned char *iv = OPENSSL_BOX_EVP_CIPHER_CTX_iv_noconst(ctx);
    int n = ctx->num;
    int rem;
...
static int s390x_aes_gcm_ctrl(EVP_CIPHER_CTX *c, int type, int arg, void *ptr)
{
    ...
    switch (type) {
    case EVP_CTRL_INIT:
        ivlen = OPENSSL_BOX_EVP_CIPHER_get_iv_length(c->cipher);
        gctx->key_set = 0;
        gctx->iv_set = 0;
        gctx->ivlen = ivlen;
        gctx->iv = c->iv;
    ...
    case EVP_CTRL_AEAD_SET_TAG:
        buf = OPENSSL_BOX_EVP_CIPHER_CTX_buf_noconst(c);
        enc = OPENSSL_BOX_EVP_CIPHER_CTX_is_encrypting(c);
        if (arg <= 0 || arg > 16 || enc)
            return 0;
...
{
    S390X_AES_GCM_CTX *gctx = EVP_C_DATA(S390X_AES_GCM_CTX, ctx);
    const unsigned char *buf = OPENSSL_BOX_EVP_CIPHER_CTX_buf_noconst(ctx);
    const int enc = OPENSSL_BOX_EVP_CIPHER_CTX_is_encrypting(ctx);
    int rv = -1;
    ...
    if (OPENSSL_BOX_EVP_CIPHER_CTX_ctrl(ctx, enc ? EVP_CTRL_GCM_IV_GEN
                                ...
                                const unsigned char *in, size_t len)
{
    ...
    if (in != NULL) {
        ...
        enc = OPENSSL_BOX_EVP_CIPHER_CTX_is_encrypting(ctx);
        if (enc) {
            gctx->taglen = 16;
        } else {
            if (gctx->taglen < 0)
            ...
            buf = OPENSSL_BOX_EVP_CIPHER_CTX_buf_noconst(ctx);
                                    ...
                                    const unsigned char *in, size_t len)
{
    ...
    unsigned char *buf = OPENSSL_BOX_EVP_CIPHER_CTX_buf_noconst(ctx);
    const int enc = OPENSSL_BOX_EVP_CIPHER_CTX_is_encrypting(ctx);
...
static int aes_gcm_ctrl(EVP_CIPHER_CTX *c, int type, int arg, void *ptr)
{
    ...
    case EVP_CTRL_INIT:
        gctx->key_set = 0;
        gctx->iv_set = 0;
        gctx->ivlen = OPENSSL_BOX_EVP_CIPHER_get_iv_length(c->cipher);
        gctx->iv = c->iv;
        gctx->taglen = -1;
        gctx->iv_gen = 0;
        gctx->tls_aad_len = -1;
                              ...
                              const unsigned char *in, size_t len)
{
     ...
     * side only.
     */
    if (OPENSSL_BOX_EVP_CIPHER_CTX_is_encrypting(ctx) && ++gctx->tls_enc_records == 0) {
        ERR_raise(ERR_LIB_EVP, EVP_R_TOO_MANY_RECORDS);
        goto err;
    }
     ...
     * buffer.
     */
    if (OPENSSL_BOX_EVP_CIPHER_CTX_ctrl(ctx,
                            OPENSSL_BOX_EVP_CIPHER_CTX_is_encrypting(ctx) ?
                                EVP_CTRL_GCM_IV_GEN : EVP_CTRL_GCM_SET_IV_INV,
                            EVP_GCM_TLS_EXPLICIT_IV_LEN, out) <= 0)
        goto err;
    /* Use saved AAD */
    if (CRYPTO_gcm128_aad(&gctx->gcm, OPENSSL_BOX_EVP_CIPHER_CTX_buf_noconst(ctx),
                          gctx->tls_aad_len))
        goto err;
    ...
    if (OPENSSL_BOX_EVP_CIPHER_CTX_is_encrypting(ctx)) {
        ...
        /* Retrieve tag */
        CRYPTO_gcm128_tag(&gctx->gcm, OPENSSL_BOX_EVP_CIPHER_CTX_buf_noconst(ctx),
                          EVP_GCM_TLS_TAG_LEN);
        /* If tag mismatch wipe buffer */
        if (CRYPTO_memcmp(OPENSSL_BOX_EVP_CIPHER_CTX_buf_noconst(ctx), in + len,
                          EVP_GCM_TLS_TAG_LEN)) {
            OPENSSL_cleanse(out, len);
            goto err;
        }
                          ...
                          const unsigned char *in, size_t len)
{
     ...
     */
    if (!gctx->iv_set) {
        if (!OPENSSL_BOX_EVP_CIPHER_CTX_is_encrypting(ctx) || !aes_gcm_iv_generate(gctx, 0))
            return -1;
        CRYPTO_gcm128_setiv(&gctx->gcm, gctx->iv, gctx->ivlen);
        gctx->iv_set = 1;
        gctx->iv_gen_rand = 1;
    ...
    if (in) {
        ...
        CRYPTO_gcm128_tag(&gctx->gcm, OPENSSL_BOX_EVP_CIPHER_CTX_buf_noconst(ctx), 16);
        gctx->taglen = 16;
        /* Don't reuse the IV */
        gctx->iv_set = 0;
        return 0;
...
static int aes_ccm_ctrl(EVP_CIPHER_CTX *c, int type, int arg, void *ptr)
{
    ...
    case EVP_CTRL_AEAD_TLS1_AAD:
        ...
        if (arg != EVP_AEAD_TLS1_AAD_LEN)
            return 0;
        memcpy(OPENSSL_BOX_EVP_CIPHER_CTX_buf_noconst(c), ptr, arg);
        cctx->tls_aad_len = arg;
        {
            uint16_t len =
                OPENSSL_BOX_EVP_CIPHER_CTX_buf_noconst(c)[arg - 2] << 8
                | OPENSSL_BOX_EVP_CIPHER_CTX_buf_noconst(c)[arg - 1];
            /* Correct length for explicit IV */
            if (len < EVP_CCM_TLS_EXPLICIT_IV_LEN)
                return 0;
            len -= EVP_CCM_TLS_EXPLICIT_IV_LEN;
            /* If decrypting correct for tag too */
            if (!OPENSSL_BOX_EVP_CIPHER_CTX_is_encrypting(c)) {
                if (len < cctx->M)
                    return 0;
                len -= cctx->M;
            }
            OPENSSL_BOX_EVP_CIPHER_CTX_buf_noconst(c)[arg - 2] = len >> 8;
            OPENSSL_BOX_EVP_CIPHER_CTX_buf_noconst(c)[arg - 1] = len & 0xff;
        }
        /* Extra padding: tag appended to record */
        return cctx->M;
    ...
    case EVP_CTRL_AEAD_SET_TAG:
        if ((arg & 1) || arg < 4 || arg > 16)
            return 0;
        if (OPENSSL_BOX_EVP_CIPHER_CTX_is_encrypting(c) && ptr)
            return 0;
        if (ptr) {
            cctx->tag_set = 1;
            memcpy(OPENSSL_BOX_EVP_CIPHER_CTX_buf_noconst(c), ptr, arg);
    ...
    case EVP_CTRL_AEAD_GET_TAG:
        if (!OPENSSL_BOX_EVP_CIPHER_CTX_is_encrypting(c) || !cctx->tag_set)
            return 0;
                              ...
                              const unsigned char *in, size_t len)
{
    ...
    if (OPENSSL_BOX_EVP_CIPHER_CTX_is_encrypting(ctx))
        memcpy(out, OPENSSL_BOX_EVP_CIPHER_CTX_buf_noconst(ctx),
    ...
    if (CRYPTO_ccm128_setiv(ccm, ctx->iv, 15 - cctx->L,
            ...
            return -1;
    /* Use saved AAD */
    CRYPTO_ccm128_aad(ccm, OPENSSL_BOX_EVP_CIPHER_CTX_buf_noconst(ctx),
                      cctx->tls_aad_len);
                          ...
                          const unsigned char *in, size_t len)
{
    ...
    /* The tag must be set before actually decrypting data */
    if (!OPENSSL_BOX_EVP_CIPHER_CTX_is_encrypting(ctx) && !cctx->tag_set)
        return -1;
    ...
    if (OPENSSL_BOX_EVP_CIPHER_CTX_is_encrypting(ctx)) {
        ...
        if (cctx->str ? !CRYPTO_ccm128_decrypt_ccm64(ccm, in, out, len,
            ...
            unsigned char tag[16];
            if (CRYPTO_ccm128_tag(ccm, tag, cctx->M)) {
                if (!CRYPTO_memcmp(tag, OPENSSL_BOX_EVP_CIPHER_CTX_buf_noconst(ctx),
                                   cctx->M))
                    rv = (int)len;
            }
        }
                             ...
                             const unsigned char *iv, int enc)
{
    ...
    if (iv != NULL) {
        if ((len = OPENSSL_BOX_EVP_CIPHER_CTX_get_iv_length(ctx)) < 0)
            return 0;
                           ...
                           const unsigned char *in, size_t inlen)
{
    ...
    int pad = OPENSSL_BOX_EVP_CIPHER_CTX_get_iv_length(ctx) == 4;
    ...
    if (!OPENSSL_BOX_EVP_CIPHER_CTX_is_encrypting(ctx) && (inlen < 16 || inlen & 0x7))
        return -1;
...
static int aes_ocb_ctrl(EVP_CIPHER_CTX *c, int type, int arg, void *ptr)
{
    ...
    case EVP_CTRL_INIT:
        octx->key_set = 0;
        octx->iv_set = 0;
        octx->ivlen = OPENSSL_BOX_EVP_CIPHER_get_iv_length(c->cipher);
        octx->iv = c->iv;
        octx->taglen = 16;
        octx->data_buf_len = 0;
        octx->aad_buf_len = 0;
    ...
    case EVP_CTRL_AEAD_SET_TAG:
        ...
        if (arg != octx->taglen || OPENSSL_BOX_EVP_CIPHER_CTX_is_encrypting(c))
            return 0;
