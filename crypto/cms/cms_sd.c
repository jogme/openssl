...
static int cms_signature_nomd(EVP_PKEY *pkey)
{
    ...
    return OPENSSL_BOX_EVP_PKEY_get_default_digest_name(pkey, def_md, sizeof(def_md)) == 2
...
static int cms_generic_sign(CMS_SignerInfo *si, int verify)
{
    ...
    if (!verify) {
        ...
        int snid, hnid, pknid = OPENSSL_BOX_EVP_PKEY_get_id(pkey);
        ...
        if (pknid <= 0) { /* check whether a provider registered a NID */
            const char *typename = OPENSSL_BOX_EVP_PKEY_get0_type_name(pkey);
...
static int cms_sd_asn1_ctrl(CMS_SignerInfo *si, int cmd)
{
    ...
    if (OPENSSL_BOX_EVP_PKEY_is_a(pkey, "DSA") || OPENSSL_BOX_EVP_PKEY_is_a(pkey, "EC"))
        return cms_generic_sign(si, cmd) > 0;
    else if (OPENSSL_BOX_EVP_PKEY_is_a(pkey, "RSA") || OPENSSL_BOX_EVP_PKEY_is_a(pkey, "RSA-PSS"))
                                ...
                                unsigned int flags)
{
    ...
    si->pkey = pk;
    si->signer = signer;
    si->mctx = OPENSSL_BOX_EVP_MD_CTX_new();
    si->pctx = NULL;
    ...
    if (md == NULL) {
        ...
        if (OPENSSL_BOX_EVP_PKEY_get_default_digest_nid(pk, &def_nid) <= 0) {
            ERR_raise_data(ERR_LIB_CMS, CMS_R_NO_DEFAULT_DIGEST,
                           "pkey nid=%d", OPENSSL_BOX_EVP_PKEY_get_id(pk));
            goto err;
        }
    ...
    for (i = 0; i < sk_X509_ALGOR_num(sd->digestAlgorithms); i++) {
        ...
        if (OPENSSL_BOX_EVP_MD_is_a(md, name))
            break;
    }
    ...
    if (!(flags & CMS_NOATTR)) {
        ...
        if (flags & CMS_CADES) {
            ...
            if (md == NULL || OPENSSL_BOX_EVP_MD_is_a(md, SN_sha1)) {
    ...
    if (flags & CMS_KEY_PARAM) {
        if (flags & CMS_NOATTR) {
            si->pctx = OPENSSL_BOX_EVP_PKEY_CTX_new_from_pkey(ossl_cms_ctx_get0_libctx(ctx),
                                                  si->pkey,
                                                  ossl_cms_ctx_get0_propq(ctx));
            if (si->pctx == NULL)
                goto err;
            if (OPENSSL_BOX_EVP_PKEY_sign_init(si->pctx) <= 0)
                goto err;
            if (OPENSSL_BOX_EVP_PKEY_CTX_set_signature_md(si->pctx, md) <= 0)
                goto err;
        } else if (OPENSSL_BOX_EVP_DigestSignInit_ex(si->mctx, &si->pctx,
                                         OPENSSL_BOX_EVP_MD_get0_name(md),
            ...
            OPENSSL_BOX_EVP_MD_CTX_set_flags(si->mctx, EVP_MD_CTX_FLAG_KEEP_PKEY_CTX);
        }
    }
                                       ...
                                       unsigned int mdlen)
{
    ...
    if (CMS_signed_get_attr_count(si) >= 0) {
        ...
        if (md == NULL) {
            if (!OPENSSL_BOX_EVP_DigestFinal_ex(mctx, computed_md, &mdlen))
                goto err;
            md = computed_md;
        }
        if (!CMS_signed_add1_attr_by_NID(si, NID_pkcs9_messageDigest,
        ...
        siglen = OPENSSL_BOX_EVP_PKEY_get_size(si->pkey);
        if (siglen == 0 || (sig = OPENSSL_malloc(siglen)) == NULL)
            goto err;
        if (OPENSSL_BOX_EVP_PKEY_sign(pctx, sig, &siglen, md, mdlen) <= 0) {
            OPENSSL_free(sig);
            goto err;
        }
        ...
        if (siglen == 0 || (sig = OPENSSL_malloc(siglen)) == NULL)
            goto err;
        if (!OPENSSL_BOX_EVP_SignFinal_ex(mctx, sig, &siglen, si->pkey,
                              ossl_cms_ctx_get0_libctx(ctx),
                              ossl_cms_ctx_get0_propq(ctx))) {
...
int CMS_SignerInfo_sign(CMS_SignerInfo *si)
{
    ...
    if (si->pctx) {
        ...
        OPENSSL_BOX_EVP_MD_CTX_reset(mctx);
        if (OPENSSL_BOX_EVP_DigestSignInit_ex(mctx, &pctx, md_name,
                                  ossl_cms_ctx_get0_libctx(ctx),
                                  ossl_cms_ctx_get0_propq(ctx), si->pkey,
                                  NULL) <= 0)
            goto err;
        OPENSSL_BOX_EVP_MD_CTX_set_flags(mctx, EVP_MD_CTX_FLAG_KEEP_PKEY_CTX);
        si->pctx = pctx;
    }
...
int CMS_SignerInfo_verify(CMS_SignerInfo *si)
{
    ...
    (void)ERR_set_mark();
    fetched_md = OPENSSL_BOX_EVP_MD_fetch(libctx, name, propq);
    ...
    if (si->mctx == NULL && (si->mctx = OPENSSL_BOX_EVP_MD_CTX_new()) == NULL) {
        ERR_raise(ERR_LIB_CMS, ERR_R_EVP_LIB);
        goto err;
    }
    mctx = si->mctx;
    ...
    if (OPENSSL_BOX_EVP_DigestVerifyInit_ex(mctx, &si->pctx, OPENSSL_BOX_EVP_MD_get0_name(md), libctx,
    ...
    if (abuf == NULL || alen < 0)
        goto err;
    r = OPENSSL_BOX_EVP_DigestVerifyUpdate(mctx, abuf, alen);
    OPENSSL_free(abuf);
    if (r <= 0) {
        r = -1;
        goto err;
    }
    r = OPENSSL_BOX_EVP_DigestVerifyFinal(mctx,
...
int CMS_SignerInfo_verify_content(CMS_SignerInfo *si, BIO *chain)
{
    ...
    if (OPENSSL_BOX_EVP_DigestFinal_ex(mctx, mval, &mlen) <= 0) {
    ...
    if (os != NULL) {
        ...
        const EVP_MD *md = OPENSSL_BOX_EVP_MD_CTX_get0_md(mctx);
        const CMS_CTX *ctx = si->cms_ctx;

        pkctx = OPENSSL_BOX_EVP_PKEY_CTX_new_from_pkey(ossl_cms_ctx_get0_libctx(ctx),
                                           si->pkey,
                                           ossl_cms_ctx_get0_propq(ctx));
        if (pkctx == NULL)
            goto err;
        if (OPENSSL_BOX_EVP_PKEY_verify_init(pkctx) <= 0)
            goto err;
        if (OPENSSL_BOX_EVP_PKEY_CTX_set_signature_md(pkctx, md) <= 0)
            goto err;
        si->pctx = pkctx;
        if (!cms_sd_asn1_ctrl(si, 1)) {
            si->pctx = NULL;
        ...
        si->pctx = NULL;
        r = OPENSSL_BOX_EVP_PKEY_verify(pkctx, si->signature->data,
